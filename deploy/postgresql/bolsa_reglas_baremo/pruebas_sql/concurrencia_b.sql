...
DECLARE
    ...
    bytes_resultado bytea := convert_to(
        '{"estado":"publicada","revision":2,"ganador":"b"}', 'UTF8'
    );
    decision bytea := convert_to(jsonb_build_object(
        'decision_ref', 'decision:reglas:concurrencia:b',
...
BEGIN
    ...
    PERFORM * FROM vec_bolsa_reglas_baremo.confirmar_cambio_v1(
        jsonb_build_object(
            ...
            'operacion', 'publicar',
            'intencion_ref', 'intencion:reglas:concurrencia:b',
            'intencion_version', 1,
            'intencion_huella_sha256', repeat('4', 64),
            'esperado_revision', 1,
            ...
            'prueba_ref', 'prueba:reglas:concurrencia:b',
            'prueba_version', 1, 'prueba_huella_sha256', repeat('5', 64),
            'accion', 'bolsa.reglas_baremo.publicar',
            'recurso_ref', 'reglas-baremo:' || huella_resultado,
            'correlacion_ref',
                'correlacion_8123456789abcdef0123456789abcdef',
