-- Frontera estrecha instalada en el nucleo de autorizacion. Solo el
-- propietario de la persistencia Bolsa puede invocarla desde sus funciones
-- SECURITY DEFINER; la identidad runtime nunca recibe este privilegio.
BEGIN;
    ...
    IF to_regclass('vec_autorizacion.decision_autorizacion') IS NULL
       ...
       OR to_regprocedure(
        ...
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'estado incompatible para instalar la frontera Bolsa';
    END IF;
END
$prevalidacion$;

CREATE FUNCTION vec_autorizacion.revalidar_decision_bolsa_baremacion_v1(
    p_prueba jsonb,
...
BEGIN
    IF p_prueba IS NULL OR jsonb_typeof(p_prueba) <> 'object'
       ...
       OR NOT (p_prueba ?& ARRAY[
           ...
           'verificada_en', 'sujeto_ref'
       ])
       OR p_prueba ->> 'esquema_huella' <>
       ...
       OR vec_autorizacion.texto_positivo_valido(p_accion, 256) IS NOT TRUE
       OR vec_autorizacion.texto_positivo_valido(
           p_clase_recurso, 128
       ) IS NOT TRUE
       OR vec_autorizacion.texto_positivo_valido(
    ...
    BEGIN
        decision_canonica := convert_from(
    ...
    decision_canonica_esperada := jsonb_build_object(
        ...
        'vinculo_autenticacion_actor',
            decision.documento -> 'vinculo_autenticacion_actor',
    ...
    IF decision_canonica IS DISTINCT FROM decision_canonica_esperada
       OR campos_ordenados IS DISTINCT FROM (
           SELECT COALESCE(jsonb_agg(valor ORDER BY valor), '[]'::jsonb)
    ...
    SELECT count(*) INTO concesiones_coincidentes
     ...
     WHERE c ->> 'accion' = decision.accion
       AND c ->> 'modulo_id' = 'bolsa'
       AND c ->> 'tipo_recurso' = decision.tipo_recurso
       ...
       ) IS NOT DISTINCT FROM campos_ordenados
       AND EXISTS (
           SELECT 1
             FROM jsonb_array_elements_text(c -> 'finalidades') AS finalidad
            WHERE finalidad = decision.finalidad
       );
...
GRANT USAGE ON SCHEMA vec_autorizacion
    TO vec_bolsa_baremacion_propietario;
...
COMMENT ON FUNCTION
    ...
    'Revalida una decision Bolsa exacta, su huella reforzada, recurso, RBAC, catalogo, sesion y ContextoActor; no acredita por si sola procedencia COSE del PDP.';
COMMIT;
