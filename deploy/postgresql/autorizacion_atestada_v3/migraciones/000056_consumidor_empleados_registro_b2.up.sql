...
-- Consumidor nominal de la lista de empleados del organismo (selector RRHH de
-- la ficha B2). Amplía el perfil registro_empleado_b2 ya segregado en AD3-54
-- con una sola operación de lectura, sin datos civiles. Requiere AD3-54 y se
-- instala después de ella, en serie con cualquier otra reescritura del núcleo.
BEGIN;
...
SELECT pg_advisory_xact_lock(hashtextextended('vec_autorizacion_atestada_v3:migracion:000056',0));
...
BEGIN
 IF current_user<>'vec_autorizacion_atestada_v3_propietario'
    ...
    OR to_regprocedure('vec_autorizacion_atestada_v3.consumir_empleados_registro_b2_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
 THEN RAISE EXCEPTION 'AD3-56: preimagen incompatible' USING ERRCODE='55000'; END IF;
 ...
 IF propietario<>'vec_autorizacion_atestada_v3_propietario'::regrole OR NOT definidora
    ...
    OR strpos(original,'personal.registro_empleado.empleados.consultar')<>0
    OR strpos(original,'personal.registro_empleado.catalogo.publicar')=0
    OR strpos(original,'registro_empleado_b2')=0
 THEN RAISE EXCEPTION 'AD3-56: núcleo incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-56: núcleo alterado fuera de contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;

LOCK TABLE vec_autorizacion_atestada_v3.clave_capacidad_version IN ACCESS EXCLUSIVE MODE;
DO $audiencias$
DECLARE d text; nueva text; audiencia text;
 lista text[]:=ARRAY['vec_personal.registro_empleado.empleados.v1'];
BEGIN
 ...
 IF strpos(d,'CHECK (audiencia_consumo = ANY (ARRAY[')<>1 OR right(d,3)<>']))'
    OR strpos(d,'vec_personal.registro_empleado.catalogo.consultar.v1')=0
 THEN RAISE EXCEPTION 'AD3-56: audiencias previas incompatibles' USING ERRCODE='55000'; END IF;
 nueva:=left(d,length(d)-3);
 FOREACH audiencia IN ARRAY lista LOOP
  IF strpos(d,quote_literal(audiencia))<>0 THEN
   RAISE EXCEPTION 'AD3-56: audiencia ya registrada' USING ERRCODE='55000'; END IF;
...
CREATE FUNCTION vec_autorizacion_atestada_v3.consumir_empleados_registro_b2_v3_atestada(
...
BEGIN
 ...
 EXCEPTION WHEN others THEN RAISE EXCEPTION 'AD3-56: material inválido' USING ERRCODE='22023'; END;
 IF d->>'modulo_id' IS DISTINCT FROM 'personal'
    ...
    OR d->>'finalidad' IS DISTINCT FROM 'consultar_empleados'
    OR d->'campos_permitidos' IS DISTINCT FROM '["corte","cursor","cursor_siguiente","empleados","evidencia","limite","organismo_ref"]'::jsonb
 THEN RAISE EXCEPTION 'AD3-56: operación denegada' USING ERRCODE='42501'; END IF;
 ...
 IF x.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'AD3-56: requiere consumo nuevo' USING ERRCODE='42501'; END IF;
...
END $f$;
REVOKE ALL ON FUNCTION vec_autorizacion_atestada_v3.consumir_empleados_registro_b2_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) FROM PUBLIC;
