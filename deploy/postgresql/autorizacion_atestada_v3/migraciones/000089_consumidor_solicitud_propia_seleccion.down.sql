...
-- AD3-89 DOWN: solo sin historia. Se rechaza si AD3-90 o Selección 000001
-- siguen instaladas, o si ya existe alguna clave de capacidad de sus
-- audiencias; sin historia deja el núcleo, las audiencias y las ACL como
-- antes de AD3-89.
BEGIN;
...
DO $pre$
BEGIN
 IF to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_solicitud_propia_seleccion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NULL
 THEN RAISE EXCEPTION 'AD3-89 DOWN: AD3-89 no instalada' USING ERRCODE='55000'; END IF;
 IF to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_consulta_solicitudes_seleccion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
 THEN RAISE EXCEPTION 'AD3-89 DOWN: AD3-90 sigue instalada' USING ERRCODE='55000'; END IF;
 IF to_regnamespace('vec_seleccion') IS NOT NULL
 THEN RAISE EXCEPTION 'AD3-89 DOWN: Selección 000001 sigue instalada' USING ERRCODE='55000'; END IF;
 IF EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.clave_capacidad_version
            WHERE audiencia_consumo LIKE 'vec\_seleccion.solicitudes\_propias.%')
 THEN RAISE EXCEPTION 'AD3-89 DOWN: no admitido con historia de sus audiencias' USING ERRCODE='55000'; END IF;
END $pre$;

DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_solicitud_propia_seleccion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
REVOKE USAGE ON SCHEMA vec_autorizacion_atestada_v3 FROM vec_seleccion_propietario;
...
BEGIN
 ...
 resto:=d;
 FOREACH a IN ARRAY ARRAY['vec_seleccion.solicitudes_propias.consultar.v1','vec_seleccion.solicitudes_propias.guardar_borrador.v1','vec_seleccion.solicitudes_propias.presentar.v1'] LOOP
  IF length(resto)-length(replace(resto,', '||quote_literal(a)||'::text',''))<>length(', '||quote_literal(a)||'::text')
  THEN RAISE EXCEPTION 'AD3-89 DOWN: audiencia no localizada' USING ERRCODE='55000'; END IF;
  resto:=replace(resto,', '||quote_literal(a)||'::text','');
 END LOOP;
 IF strpos(resto,'vec_seleccion.solicitudes_propias.')<>0 THEN
  RAISE EXCEPTION 'AD3-89 DOWN: audiencias incompatibles' USING ERRCODE='55000';
 END IF;
...
DECLARE
 ...
 excl text:=E'               AND p_perfil_mutacion IS DISTINCT FROM ''solicitud_propia_seleccion''\n';
 guarda text:=$g$           OR (
...
BEGIN
 ...
 THEN RAISE EXCEPTION 'AD3-89 DOWN: extensión no localizada en el núcleo' USING ERRCODE='55000'; END IF;
 nuevo:=replace(replace(replace(original,extension,''),guarda,''),excl,'');
 IF strpos(nuevo,'solicitud_propia_seleccion')<>0 OR strpos(nuevo,'vec_seleccion.solicitudes_propias')<>0 THEN
  RAISE EXCEPTION 'AD3-89 DOWN: restos de la extensión en el núcleo' USING ERRCODE='55000';
 END IF;
 EXECUTE nuevo;
 ...
 THEN RAISE EXCEPTION 'AD3-89 DOWN: núcleo alterado fuera del contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;
COMMIT;
