...
-- Amplía el único núcleo: permiso CT por etapa/material y acción Bolsa propia.
-- Conserva literalmente 16/17/18, criptografía y comparación de instantes.
DO $ampliar$
DECLARE
...
BEGIN
    IF to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_continuacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
       OR to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_resolucion_manual_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NULL THEN
        RAISE EXCEPTION 'dependencia incompatible para continuación' USING ERRCODE='55000';
    END IF;
    ...
    IF length(v_def)-length(replace(v_def,v_marca,''))<>length(v_marca)
       OR length(v_def)-length(replace(v_def,v_anterior,''))<>length(v_anterior)
       OR strpos(v_def,'continuacion_llamamiento_ct')<>0 OR strpos(v_def,'bolsa.llamamiento.siguiente.abrir')<>0
       ...
       OR strpos(v_def,'p_perfil_mutacion IS NOT DISTINCT FROM ''consulta_justificante_respuesta_ct''')=0 THEN
        RAISE EXCEPTION 'núcleo incompatible para continuación' USING ERRCODE='55000';
    END IF;
    EXECUTE replace(replace(v_def,v_marca,v_extension||v_marca),v_anterior,v_nuevo);
...
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_continuacion_ct_v3_atestada(
...
BEGIN
    SELECT * INTO STRICT v_consumo
      FROM vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna(
        'continuacion_llamamiento_ct',p_capacidad,p_decision,p_motivo,p_contexto,
        p_persona_version,p_perfil_version,p_payload,p_sobre,p_evidencia,p_raiz);
    IF v_consumo.consumo_nuevo IS NOT TRUE THEN
        RAISE EXCEPTION 'continuación requiere consumo nuevo' USING ERRCODE='P0603';
    END IF;
...
END
$funcion$;
REVOKE ALL ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_continuacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)
...
GRANT EXECUTE ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_continuacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) TO vec_contratacion_temporal_propietario;
COMMIT;
