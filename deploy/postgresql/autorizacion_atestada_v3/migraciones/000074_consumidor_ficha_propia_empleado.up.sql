...
-- Consumidor nominal de la ficha propia de la persona empleada («mis datos»:
-- relaciones y servicios del propio empleado). Amplía el perfil registro_empleado_b2
-- de AD3-54 con una sola operación de lectura. Requiere AD3-54, 55 y 56 y se
-- instala después, en serie con cualquier otra reescritura del núcleo. Número
-- libre fuera de los reservados; no fija orden. Sin DOWN tras historia.
BEGIN;
...
SELECT pg_advisory_xact_lock(hashtextextended('vec_autorizacion_atestada_v3:migracion:000074',0));
...
BEGIN
 ...
 THEN RAISE EXCEPTION 'AD3-74: preimagen incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-74: núcleo incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-74: núcleo alterado fuera de contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;

LOCK TABLE vec_autorizacion_atestada_v3.clave_capacidad_version IN ACCESS EXCLUSIVE MODE;
DO $audiencias$
DECLARE d text; nueva text; audiencia text;
 lista text[]:=ARRAY['vec_personal.registro_empleado.ficha_propia.v1'];
BEGIN
 ...
 IF strpos(d,'CHECK (audiencia_consumo = ANY (ARRAY[')<>1 OR right(d,3)<>']))'
    OR strpos(d,'vec_personal.registro_empleado.empleados.v1')=0
 THEN RAISE EXCEPTION 'AD3-74: audiencias previas incompatibles' USING ERRCODE='55000'; END IF;
 nueva:=left(d,length(d)-3);
 FOREACH audiencia IN ARRAY lista LOOP
  IF strpos(d,quote_literal(audiencia))<>0 THEN
   RAISE EXCEPTION 'AD3-74: audiencia ya registrada' USING ERRCODE='55000'; END IF;
...
BEGIN
 ...
 EXCEPTION WHEN others THEN RAISE EXCEPTION 'AD3-74: material inválido' USING ERRCODE='22023'; END;
 ...
 THEN RAISE EXCEPTION 'AD3-74: operación denegada' USING ERRCODE='42501'; END IF;
 ...
 IF x.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'AD3-74: requiere consumo nuevo' USING ERRCODE='42501'; END IF;
...
END $f$;
REVOKE ALL ON FUNCTION vec_autorizacion_atestada_v3.consumir_ficha_propia_empleado_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) FROM PUBLIC;
