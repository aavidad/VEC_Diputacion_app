...
-- Extensión nominal sobre el núcleo instalado, conservando 000011/000012.
DO $ampliar$
DECLARE
...
$perfil$;
BEGIN
    IF to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_reanudacion_seleccion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
       OR to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_comunicacion_llamamiento_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NULL THEN
        RAISE EXCEPTION 'estado incompatible para reanudación' USING ERRCODE = '55000';
    END IF;
    ...
    IF length(v_def)-length(replace(v_def,v_marca,'')) <> length(v_marca)
       OR strpos(v_def, 'p_perfil_mutacion IS NOT DISTINCT FROM ''reanudacion_seleccion''') <> 0 THEN
        RAISE EXCEPTION 'núcleo incompatible para reanudación' USING ERRCODE = '55000';
    END IF;
...
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_reanudacion_seleccion_v3_atestada(
...
BEGIN
    SELECT * INTO STRICT v_consumo
      FROM vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna(
        'reanudacion_seleccion',p_capacidad,p_decision,p_motivo,p_contexto,
        p_persona_version,p_perfil_version,p_payload,p_sobre,p_evidencia,p_raiz);
    -- Un consumo histórico no renueva autorización ni recupera una reserva.
...
END
$funcion$;
REVOKE ALL ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_reanudacion_seleccion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)
...
GRANT EXECUTE ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_reanudacion_seleccion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)
