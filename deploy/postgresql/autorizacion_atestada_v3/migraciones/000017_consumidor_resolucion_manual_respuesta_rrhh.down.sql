...
$perfil$;
BEGIN
    -- Catálogos únicamente: ninguna lectura CT ni USAGE de su esquema.
    -- Cualquier sobrecarga nominal impide retirar el consumidor todavía vivo.
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
        WHERE n.nspname='vec_contratacion_temporal'
          AND p.proname='registrar_resolucion_manual_respuesta_rrhh_v1')
       OR EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.atestacion_decision_v3 a
          WHERE convert_from(a.capacidad_canonica,'UTF8')::jsonb->>'operacion'=
              'contratacion_temporal.llamamiento.respuesta.validacion_manual.registrar') THEN
        RAISE EXCEPTION 'reversión denegada: consumidor o historia de resolución manual' USING ERRCODE='55000';
    END IF;
    ...
    IF length(v_def)-length(replace(v_def,v_extension,''))<>length(v_extension) THEN
        RAISE EXCEPTION 'núcleo incompatible para retirar resolución manual' USING ERRCODE='55000';
    END IF;
...
END
$retirar$;
DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_resolucion_manual_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
COMMIT;
