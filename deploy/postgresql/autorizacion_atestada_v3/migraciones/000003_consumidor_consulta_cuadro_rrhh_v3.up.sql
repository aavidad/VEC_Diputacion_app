...
SELECT pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
        'vec_autorizacion_atestada_v3:migracion:000003', 0
    )
);
DO $prevalidacion$
DECLARE
...
BEGIN
    IF pg_catalog.to_regprocedure(
       ...
       OR pg_catalog.to_regrole(
        ...
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'estado incompatible para consulta cuadro RRHH VEC-AD-3';
    END IF;
    ...
    IF v_definicion IS NULL
       OR v_definicion NOT IN (
          'CHECK (audiencia_consumo = ''vec_contratacion_temporal.confirmar_alta_atestada.v1''::text)',
          'CHECK (audiencia_consumo = ANY (ARRAY[''vec_contratacion_temporal.confirmar_alta_atestada.v1''::text, ''vec_bolsa_llamamientos.confirmar_integracion_desarrollo.v1''::text]))'
       ) THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'gobierno de audiencias VEC-AD-3 inesperado';
    END IF;
    ...
    IF v_definicion =
          'CHECK (audiencia_consumo = ''vec_contratacion_temporal.confirmar_alta_atestada.v1''::text)' THEN
        ALTER TABLE vec_autorizacion_atestada_v3.clave_capacidad_version
...
END
$prevalidacion$;
CREATE FUNCTION vec_autorizacion_atestada_v3.consumir_consulta_rrhh_v3_interna(
...
DECLARE
    ...
    v_replay_coincidencias integer;
BEGIN
    IF pg_catalog.current_setting('transaction_isolation') <> 'serializable'
       ...
       OR EXISTS (
           SELECT 1
            ...
            WHERE u.rolname = session_user
              AND (
                  u.rolsuper OR u.rolcreatedb OR u.rolcreaterole
                  OR u.rolreplication OR u.rolbypassrls
                  OR NOT u.rolcanlogin OR NOT u.rolinherit
              )
       )
       OR EXISTS (
           SELECT 1
       ...
       OR coalesce((
        ...
        RAISE EXCEPTION USING
            ERRCODE = '42501',
            MESSAGE = 'consulta RRHH VEC-AD-3 rechazada';
    END IF;
    ...
    IF vec_autorizacion_atestada_v3.capacidad_cruda_prevalida(
       ...
       OR p_contexto_actor_canonico IS NULL OR p_persona_version IS NULL
       OR p_perfil_version IS NULL OR p_payload_vec_ad_3 IS NULL
       OR p_sobre_cose_sign1 IS NULL OR p_evidencia_verificacion IS NULL
       ...
       OR pg_catalog.octet_length(p_evidencia_verificacion)
          NOT BETWEEN 1 AND 262144
       OR pg_catalog.octet_length(p_raiz_publica_spki) IS DISTINCT FROM 44
       OR pg_catalog.substr(p_raiz_publica_spki, 1, 12) IS DISTINCT FROM
    ...
    IF vec_autorizacion_atestada_v3.capacidad_tipos_validos(c) IS NOT TRUE
       ...
       OR NOT (
           (
               ...
               AND
               c ->> 'audiencia_consumo' IS NOT DISTINCT FROM
                   'vec_contratacion_temporal.consultar_cuadro_rrhh_atestado.v1'
    ...
    SELECT pg_catalog.count(*)::integer
      INTO v_replay_coincidencias
      FROM vec_autorizacion_atestada_v3.consumo_decision_v3 a
     WHERE a.decision_ref = c ->> 'decision_ref'
        OR a.nonce = c ->> 'nonce';
    IF v_replay_coincidencias > 1 THEN
        RAISE EXCEPTION USING
            ERRCODE = '23505',
            MESSAGE = 'conflicto múltiple VEC-AD-3';
    END IF;
    SELECT a.decision_ref, a.efecto_ref, a.huella_efecto_sha256,
           a.consumo_huella_sha256, u.auditoria_ref,
           u.huella_sha256 AS auditoria_huella_sha256, a.consumida_en,
    ...
    IF FOUND THEN
        ...
        RETURN QUERY SELECT
            ...
            v_replay.auditoria_huella_sha256,
    ...
    IF v_revalidada_en > v_ahora
       ...
       OR EXISTS (
        ...
        RAISE EXCEPTION USING
            ERRCODE = '42501',
            MESSAGE = 'vigencia final de consulta RRHH agotada';
    END IF;
    ...
    v_auditoria_huella := pg_catalog.encode(
...
BEGIN
    IF vec_autorizacion_atestada_v3.capacidad_cruda_prevalida(
        ...
        RAISE EXCEPTION USING
            ERRCODE = '22023',
            MESSAGE = 'consulta cuadro RRHH VEC-AD-3 inválida';
    END IF;
    BEGIN
    ...
    IF c ->> 'audiencia_consumo' IS DISTINCT FROM
       ...
       OR d ->> 'contexto_recurso_huella_sha256' IS DISTINCT FROM
        ...
        RAISE EXCEPTION USING
            ERRCODE = '42501',
            MESSAGE = 'consulta cuadro RRHH VEC-AD-3 rechazada';
    END IF;
    RETURN QUERY
    SELECT *
      FROM vec_autorizacion_atestada_v3
           .consumir_consulta_rrhh_v3_interna(
          'cuadro', p_capacidad_canonica, p_decision_canonica,
