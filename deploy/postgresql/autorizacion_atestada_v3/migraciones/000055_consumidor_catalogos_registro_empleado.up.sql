...
-- de Personal sobre el perfil ya segregado en AD3-54. Se instala después de
-- AD3-54, en serie con cualquier otra reescritura del núcleo.
BEGIN;
...
SELECT pg_advisory_xact_lock(hashtextextended('vec_autorizacion_atestada_v3:migracion:000055',0));
...
BEGIN
 ...
 THEN RAISE EXCEPTION 'AD3-55: preimagen incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-55: núcleo incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-55: núcleo alterado fuera de contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;
...
BEGIN
 ...
 THEN RAISE EXCEPTION 'AD3-55: audiencias previas incompatibles' USING ERRCODE='55000'; END IF;
 nueva:=left(d,length(d)-3);
 FOREACH audiencia IN ARRAY lista LOOP
  IF strpos(d,quote_literal(audiencia))<>0 THEN
   RAISE EXCEPTION 'AD3-55: audiencia ya registrada' USING ERRCODE='55000'; END IF;
...
BEGIN
 ...
 EXCEPTION WHEN others THEN RAISE EXCEPTION 'AD3-55: material inválido' USING ERRCODE='22023'; END;
 ...
 THEN RAISE EXCEPTION 'AD3-55: operación denegada' USING ERRCODE='42501'; END IF;
 ...
 IF x.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'AD3-55: requiere consumo nuevo' USING ERRCODE='42501'; END IF;
