...
-- Un permiso propio para consulta/confirmación de propuesta, ligado al material.
-- Extensión del único núcleo; conserva 19 y todos sus perfiles, sin nueva criptografía.
DO $ampliar$
DECLARE
...
$perfil$;
BEGIN
    IF to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_propuesta_formalizacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
       OR to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_continuacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NULL THEN
        RAISE EXCEPTION 'dependencia incompatible para propuesta' USING ERRCODE='55000';
    END IF;
    ...
    IF length(v_def)-length(replace(v_def,v_marca,''))<>length(v_marca)
       OR strpos(v_def,'propuesta_formalizacion_ct')<>0
       OR strpos(v_def,'p_perfil_mutacion IS NOT DISTINCT FROM ''continuacion_llamamiento_ct''')=0
       OR strpos(v_def,'p_perfil_mutacion IS NOT DISTINCT FROM ''resolucion_manual_respuesta_ct''')=0
       OR strpos(v_def,'bolsa.llamamiento.siguiente.abrir')=0 THEN
        RAISE EXCEPTION 'núcleo incompatible para propuesta' USING ERRCODE='55000';
    END IF;
...
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_propuesta_formalizacion_ct_v3_atestada(
...
BEGIN
    SELECT * INTO STRICT v_consumo
      FROM vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna(
        'propuesta_formalizacion_ct',p_capacidad,p_decision,p_motivo,p_contexto,
        p_persona_version,p_perfil_version,p_payload,p_sobre,p_evidencia,p_raiz);
    IF v_consumo.consumo_nuevo IS NOT TRUE THEN
        RAISE EXCEPTION 'propuesta requiere consumo nuevo' USING ERRCODE='P0613';
    END IF;
...
END
$funcion$;
REVOKE ALL ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_propuesta_formalizacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)
...
GRANT EXECUTE ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_propuesta_formalizacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) TO vec_contratacion_temporal_propietario;
COMMIT;
