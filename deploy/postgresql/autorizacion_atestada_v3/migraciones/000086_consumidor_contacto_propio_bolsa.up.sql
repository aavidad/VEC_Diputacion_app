...
-- AD3-86. Acción propia del candidato en «Mi bolsa»: confirmar que el
-- contacto que RRHH trajo de CONVOCA sigue siendo el suyo (duda 45). Mismo
-- patrón que AD3-84, sin tocarla: el mismo perfil nominal
-- 'portal_candidato_bolsa', una operación nueva con su propia audiencia en la
-- lista única de audiencias, el recurso propio 'mi-bolsa:<candidato>' y un
-- único consumidor, Bolsa (000040), que coteja al candidato con los vínculos
-- del contexto. Toma el cerrojo común del núcleo antes de leer su preimagen.
BEGIN;
...
BEGIN
 IF current_user<>'vec_autorizacion_atestada_v3_propietario'
    ...
    OR to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_contacto_propio_bolsa_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
 ...
 THEN RAISE EXCEPTION 'AD3-86: preimagen incompatible (AD3-84 requerida)' USING ERRCODE='55000'; END IF;
END $pre$;
...
BEGIN
 ...
 -- El núcleo lleva el perfil del portal (AD3-84) y ninguna versión previa de
 -- esta operación; la marca aparece exactamente una vez.
 IF propietario<>'vec_autorizacion_atestada_v3_propietario'::regrole OR NOT definidora
    ...
    OR strpos(original,'participaciones_propias.confirmar_contacto')<>0
 THEN RAISE EXCEPTION 'AD3-86: núcleo incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-86: núcleo alterado fuera del contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;

LOCK TABLE vec_autorizacion_atestada_v3.clave_capacidad_version IN ACCESS EXCLUSIVE MODE;
DO $audiencias$
DECLARE d text; audiencia text:='vec_bolsa_llamamientos.participaciones_propias.confirmar_contacto.v1';
BEGIN
 ...
 IF strpos(d,'CHECK (audiencia_consumo = ANY (ARRAY[')<>1 OR right(d,3)<>']))'
    OR strpos(d,'vec_bolsa_llamamientos.participaciones_propias.manifestar_disposicion.v1')=0
    OR strpos(d,quote_literal(audiencia))<>0
 THEN RAISE EXCEPTION 'AD3-86: audiencias previas incompatibles' USING ERRCODE='55000'; END IF;
...
-- Fachada única de la confirmación: además de la guarda del núcleo exige el
-- recurso propio 'mi-bolsa:<candidato>', sin campos ni obligaciones, y un
-- consumo nuevo.
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_contacto_propio_bolsa_v3_atestada(
...
BEGIN
 ...
 EXCEPTION WHEN others THEN RAISE EXCEPTION 'AD3-86: material de confirmación de contacto inválido' USING ERRCODE='22023'; END;
 IF c->>'operacion' IS DISTINCT FROM 'bolsa.participaciones_propias.confirmar_contacto'
    OR c->>'audiencia_consumo' IS DISTINCT FROM 'vec_bolsa_llamamientos.participaciones_propias.confirmar_contacto.v1'
 ...
 THEN RAISE EXCEPTION 'AD3-86: material de confirmación de contacto rechazado' USING ERRCODE='42501'; END IF;
 ...
 IF x.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'AD3-86: la confirmación requiere consumo nuevo' USING ERRCODE='42501'; END IF;
...
BEGIN
 ...
 THEN RAISE EXCEPTION 'AD3-86: propietario o entorno de fachada incompatible' USING ERRCODE='55000'; END IF;
 FOR x IN SELECT a.grantee,a.privilege_type,a.is_grantable,p.proowner
  ...
  THEN RAISE EXCEPTION 'AD3-86: ACL de fachada abierta' USING ERRCODE='55000'; END IF;
 END LOOP;
END $acl$;
COMMIT;
