...
-- AD3-82: consumidores nominales del cese del nombramiento o contrato y del
-- cierre del expediente de Contratación temporal (petición RRHH p. 1, punto 3;
-- dudas 11 y 12). Dos perfiles y dos audiencias propias; cada fachada exige
-- consumo nuevo y el único llamante es el propietario de CT (CT 000115).
...
SELECT pg_advisory_xact_lock(hashtextextended('vec_autorizacion_atestada_v3:migracion:000082',0));
...
BEGIN
 IF current_user<>'vec_autorizacion_atestada_v3_propietario'
    OR to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_cese_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
    OR to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_cierre_expediente_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
 ...
 THEN RAISE EXCEPTION 'AD3-82: preimagen incompatible' USING ERRCODE='55000'; END IF;
 ...
 IF propietario<>'vec_autorizacion_atestada_v3_propietario'::regrole OR NOT definidora
    ...
    OR strpos(original,'cierre_expediente_ct')<>0
    OR strpos(original,'contratacion_temporal.seguimiento.cesar')<>0
 THEN RAISE EXCEPTION 'AD3-82: núcleo incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-82: núcleo alterado fuera de contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;

LOCK TABLE vec_autorizacion_atestada_v3.clave_capacidad_version IN ACCESS EXCLUSIVE MODE;
DO $audiencias$
DECLARE d text; nueva text; audiencia text;
 lista text[]:=ARRAY['vec_contratacion_temporal.cese.v1','vec_contratacion_temporal.cierre_expediente.v1'];
BEGIN
 ...
 IF strpos(d,'CHECK (audiencia_consumo = ANY (ARRAY[')<>1 OR right(d,3)<>']))'
    OR strpos(d,'vec_contratacion_temporal.cierre_administrativo_sin_cese.v1')=0
 THEN RAISE EXCEPTION 'AD3-82: audiencias previas incompatibles' USING ERRCODE='55000'; END IF;
 nueva:=left(d,length(d)-3);
 FOREACH audiencia IN ARRAY lista LOOP
  IF strpos(d,quote_literal(audiencia))<>0 THEN
   RAISE EXCEPTION 'AD3-82: audiencia ya registrada' USING ERRCODE='55000'; END IF;
...
-- Cese: el recurso es el expediente en nombramiento con incorporación.
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_cese_ct_v3_atestada(
...
BEGIN
 ...
 EXCEPTION WHEN others THEN RAISE EXCEPTION 'AD3-82: material de cese inválido' USING ERRCODE='22023'; END;
 IF c->>'operacion' IS DISTINCT FROM 'contratacion_temporal.seguimiento.cesar'
    OR c->>'audiencia_consumo' IS DISTINCT FROM 'vec_contratacion_temporal.cese.v1'
    OR d->>'accion' IS DISTINCT FROM c->>'operacion'
    OR d->>'modulo_id' IS DISTINCT FROM 'contratacion_temporal'
    OR d->>'tipo_recurso' IS DISTINCT FROM 'cese_contratacion_temporal'
 ...
 IF x.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'AD3-82: el cese requiere consumo nuevo' USING ERRCODE='42501'; END IF;
...
-- Cierre: el recurso es el expediente con cese registrado.
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_cierre_expediente_ct_v3_atestada(
 p_capacidad bytea,p_decision bytea,p_motivo bytea,p_contexto bytea,p_persona_version numeric,p_perfil_version numeric,p_payload bytea,p_sobre bytea,p_evidencia bytea,p_raiz bytea)
RETURNS TABLE(decision_ref text,efecto_ref text,huella_efecto_sha256 text,consumo_huella_sha256 text,auditoria_ref text,consumida_en timestamptz,consumo_nuevo boolean)
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog SET lock_timeout='2s' AS $f$
DECLARE c jsonb; d jsonb; x record;
BEGIN
 BEGIN c:=convert_from(p_capacidad,'UTF8')::jsonb; d:=convert_from(p_decision,'UTF8')::jsonb;
 EXCEPTION WHEN others THEN RAISE EXCEPTION 'AD3-82: material de cierre inválido' USING ERRCODE='22023'; END;
 IF c->>'operacion' IS DISTINCT FROM 'contratacion_temporal.expediente.cerrar'
    OR c->>'audiencia_consumo' IS DISTINCT FROM 'vec_contratacion_temporal.cierre_expediente.v1'
    OR d->>'accion' IS DISTINCT FROM c->>'operacion'
    OR d->>'modulo_id' IS DISTINCT FROM 'contratacion_temporal'
    OR d->>'tipo_recurso' IS DISTINCT FROM 'cierre_expediente_contratacion_temporal'
 ...
 IF x.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'AD3-82: el cierre requiere consumo nuevo' USING ERRCODE='42501'; END IF;
 RETURN QUERY SELECT x.decision_ref,x.efecto_ref,x.huella_efecto_sha256,x.consumo_huella_sha256,x.auditoria_ref,x.consumida_en,true;
END $f$;
...
BEGIN
 FOREACH f IN ARRAY ARRAY[
  'vec_autorizacion_atestada_v3.registrar_y_consumir_cese_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure,
  'vec_autorizacion_atestada_v3.registrar_y_consumir_cierre_expediente_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure] LOOP
  ...
  THEN RAISE EXCEPTION 'AD3-82: propietario o entorno de fachada incompatible' USING ERRCODE='55000'; END IF;
  FOR x IN SELECT a.grantee,a.privilege_type,a.is_grantable,p.proowner
   ...
   THEN RAISE EXCEPTION 'AD3-82: ACL de fachada abierta' USING ERRCODE='55000'; END IF;
  END LOOP;
 END LOOP;
END $acl$;
COMMIT;
