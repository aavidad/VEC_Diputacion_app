...
-- AD3-83 DOWN: solo sin historia. Exige que CT 000116 esté retirada y que no
-- exista ninguna clave de capacidad publicada para su audiencia: sin
...
BEGIN
 IF current_user<>'vec_autorizacion_atestada_v3_propietario'
    OR EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
               WHERE n.nspname='vec_contratacion_temporal' AND c.relname='modificacion_nombramiento_v1')
    ...
    OR EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.clave_capacidad_version
               WHERE audiencia_consumo='vec_contratacion_temporal.modificacion_tras_nombramiento.v1')
 ...
 IF actual IS DISTINCT FROM nuevo OR strpos(actual,'modificacion_tras_nombramiento_ct')<>0
...
DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_modificacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
...
BEGIN
 ...
 nueva:=replace(d,', ''vec_contratacion_temporal.modificacion_tras_nombramiento.v1''::text','');
 IF length(d)-length(nueva)<>length(', ''vec_contratacion_temporal.modificacion_tras_nombramiento.v1''::text')
 THEN RAISE EXCEPTION 'AD3-83: audiencias no localizadas al revertir' USING ERRCODE='55000'; END IF;
