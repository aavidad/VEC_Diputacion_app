...
SELECT pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
        'vec_autorizacion_atestada_v3:migracion:000008', 0
    )
);
    ...
    IF pg_catalog.to_regprocedure(
       ...
       OR pg_catalog.to_regrole(
        ...
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'estado incompatible para asignación VEC-AD-3';
    END IF;
END
$prevalidacion$;
...
BEGIN
    IF vec_autorizacion_atestada_v3.capacidad_cruda_prevalida(
       ...
       OR pg_catalog.octet_length(p_decision_canonica)
          NOT BETWEEN 1 AND 524288 THEN
        RAISE EXCEPTION USING
            ERRCODE = '22023',
            MESSAGE = 'alta VEC-AD-3 inválida';
    END IF;
    BEGIN
    ...
    IF c ->> 'audiencia_consumo' IS DISTINCT FROM
       ...
       OR d ->> 'contexto_recurso_huella_sha256' IS DISTINCT FROM
        ...
        RAISE EXCEPTION USING
            ERRCODE = '42501',
            MESSAGE = 'alta VEC-AD-3 rechazada';
    END IF;
    RETURN QUERY
    SELECT *
      FROM vec_autorizacion_atestada_v3
           .consumir_decision_mutacion_v3_interna(
          'alta', p_capacidad_canonica, p_decision_canonica,
...
CREATE FUNCTION
    vec_autorizacion_atestada_v3
    .registrar_y_consumir_asignacion_v3_atestada(
    p_capacidad_canonica bytea,
    p_decision_canonica bytea,
    p_motivo_canonico bytea,
    p_contexto_actor_canonico bytea,
...
BEGIN
    IF vec_autorizacion_atestada_v3.capacidad_cruda_prevalida(
       ...
       OR pg_catalog.octet_length(p_decision_canonica)
        ...
        RAISE EXCEPTION USING
            ERRCODE = '22023',
            MESSAGE = 'asignación VEC-AD-3 inválida';
    END IF;
    BEGIN
        c := pg_catalog.convert_from(
            p_capacidad_canonica, 'UTF8'
    ...
    IF c ->> 'audiencia_consumo' IS DISTINCT FROM
       ...
       OR d ->> 'contexto_recurso_huella_sha256' IS DISTINCT FROM
        ...
        RAISE EXCEPTION USING
            ERRCODE = '42501',
            MESSAGE = 'asignación VEC-AD-3 rechazada';
    END IF;
    RETURN QUERY
    SELECT *
      FROM vec_autorizacion_atestada_v3
           .consumir_decision_mutacion_v3_interna(
          'asignacion', p_capacidad_canonica, p_decision_canonica,
          p_motivo_canonico, p_contexto_actor_canonico,
          p_persona_version, p_perfil_version,
          p_payload_vec_ad_3, p_sobre_cose_sign1,
          p_evidencia_verificacion, p_raiz_publica_spki
