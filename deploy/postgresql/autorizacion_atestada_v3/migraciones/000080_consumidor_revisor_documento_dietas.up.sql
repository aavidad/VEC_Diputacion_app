...
-- AD3-80: consumo nominal de la lectura del documento de Dietas por quien lo
-- revisa en su etapa del circuito (administrativo, responsable, gestión RRHH
-- o Intervención). Una audiencia propia, distinta de la del titular, y un
-- único consumidor: Dietas 000008. Requiere AD3-59; no depende de AD3-53 y
-- admite instalarse antes o después de ella, siempre en serie: toma el
-- consultivo común del núcleo antes de leer su preimagen.
-- Numeración: el corte paralelo de Cronos no usa 000080 en adelante.
BEGIN;
...
SELECT pg_advisory_xact_lock(hashtextextended('vec_autorizacion_atestada_v3:migracion:000080',0));
...
BEGIN
 IF current_user<>'vec_autorizacion_atestada_v3_propietario'
    ...
    OR to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_dietas_revisor_documento_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
 ...
 THEN RAISE EXCEPTION 'AD3-80: preimagen incompatible' USING ERRCODE='55000'; END IF;
 ...
 -- El núcleo debe llevar AD3-59 (circuito de Dietas) y ninguna versión previa
 ...
 IF propietario<>'vec_autorizacion_atestada_v3_propietario'::regrole OR NOT definidora
    ...
    OR strpos(original,$x$'circuito_dietas'$x$)=0
    OR strpos(original,'revisor_documento_dietas')<>0
 THEN RAISE EXCEPTION 'AD3-80: núcleo incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-80: núcleo alterado fuera del contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;

LOCK TABLE vec_autorizacion_atestada_v3.clave_capacidad_version IN ACCESS EXCLUSIVE MODE;
DO $audiencias$
DECLARE d text; audiencia text:='vec_dietas.circuito.documento.consultar.v1';
BEGIN
 ...
 IF strpos(d,'CHECK (audiencia_consumo = ANY (ARRAY[')<>1 OR right(d,3)<>']))'
    OR strpos(d,'vec_dietas.documento.revisar.v1')=0
    OR strpos(d,quote_literal(audiencia))<>0
 THEN RAISE EXCEPTION 'AD3-80: audiencias previas incompatibles' USING ERRCODE='55000'; END IF;
...
-- Los campos son exactamente los que devuelve Dietas 000008: el documento, sus
-- líneas y justificantes, sin relación jurídica, unidad ni validadores.
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_dietas_revisor_documento_v3_atestada(
...
BEGIN
 ...
 EXCEPTION WHEN others THEN RAISE EXCEPTION 'AD3-80: lectura del revisor Dietas inválida' USING ERRCODE='22023'; END;
 IF c->>'operacion' IS DISTINCT FROM 'dietas.circuito.documento.consultar'
    OR c->>'audiencia_consumo' IS DISTINCT FROM 'vec_dietas.circuito.documento.consultar.v1'
    ...
    OR d->>'finalidad' IS DISTINCT FROM 'revisar_documento_circuito_dietas'
    OR d->>'recurso_ref' IS DISTINCT FROM c->>'efecto_ref'
    OR d->>'contexto_recurso_huella_sha256' IS DISTINCT FROM c->>'huella_efecto_sha256'
    OR d->'campos_permitidos' IS DISTINCT FROM campos
    OR d->'obligaciones' IS DISTINCT FROM '[]'::jsonb
 THEN RAISE EXCEPTION 'AD3-80: lectura del revisor Dietas denegada' USING ERRCODE='42501'; END IF;
 SELECT * INTO STRICT x FROM vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna(
  'revisor_documento_dietas',p_capacidad,p_decision,p_motivo,p_contexto,p_persona_version,p_perfil_version,p_payload,p_sobre,p_evidencia,p_raiz);
 IF x.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'AD3-80: lectura del revisor Dietas requiere consumo nuevo' USING ERRCODE='42501'; END IF;
...
DO $acl$
DECLARE f regprocedure:='vec_autorizacion_atestada_v3.registrar_y_consumir_dietas_revisor_documento_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure;
 permitido oid:='vec_dietas_propietario'::regrole::oid; x record;
BEGIN
 EXECUTE format('REVOKE ALL ON FUNCTION %s FROM PUBLIC',f::text);
 EXECUTE format('GRANT EXECUTE ON FUNCTION %s TO vec_dietas_propietario',f::text);
 ...
 THEN RAISE EXCEPTION 'AD3-80: propietario o entorno de fachada incompatible' USING ERRCODE='55000'; END IF;
 FOR x IN SELECT a.grantee,a.privilege_type,a.is_grantable,p.proowner
  ...
  THEN RAISE EXCEPTION 'AD3-80: ACL de fachada abierta' USING ERRCODE='55000'; END IF;
 END LOOP;
END $acl$;
COMMIT;
