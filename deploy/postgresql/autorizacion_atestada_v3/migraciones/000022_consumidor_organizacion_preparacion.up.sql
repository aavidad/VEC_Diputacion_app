...
SELECT pg_advisory_xact_lock(hashtextextended('vec_autorizacion_atestada_v3:migracion:000022',0));
-- Un permiso de edición preparatoria; no publica organización ni asigna roles.
DO $ampliar$
DECLARE
...
$perfil$;
BEGIN
    IF to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_organizacion_preparacion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL THEN
        RAISE EXCEPTION 'organización ya instalada' USING ERRCODE='55000';
    END IF;
    ...
    IF length(v_def)-length(replace(v_def,v_marca,''))<>length(v_marca)
       OR strpos(v_def,'organizacion_preparacion')<>0
       OR strpos(v_def,'p_perfil_mutacion IS NOT DISTINCT FROM ''propuesta_formalizacion_ct''')=0
       OR strpos(v_def,'(d ->> ''valida_hasta'')::timestamptz')=0 THEN
        RAISE EXCEPTION 'núcleo incompatible para organización' USING ERRCODE='55000';
    END IF;
    ...
    IF (SELECT proacl FROM pg_proc WHERE oid='vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna(text,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure) IS DISTINCT FROM v_acl THEN
        RAISE EXCEPTION 'organización alteró permisos del núcleo' USING ERRCODE='55000';
    END IF;
END
$ampliar$;

CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_organizacion_preparacion_v3_atestada(
...
BEGIN
    SELECT * INTO STRICT v_consumo
      FROM vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna(
        'organizacion_preparacion',p_capacidad,p_decision,p_motivo,p_contexto,
        p_persona_version,p_perfil_version,p_payload,p_sobre,p_evidencia,p_raiz);
    IF v_consumo.consumo_nuevo IS NOT TRUE THEN
        RAISE EXCEPTION 'organización requiere consumo nuevo' USING ERRCODE='P0663';
    END IF;
...
END
$funcion$;
REVOKE ALL ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_organizacion_preparacion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)
...
GRANT EXECUTE ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_organizacion_preparacion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) TO vec_contratacion_temporal_propietario;
COMMIT;
