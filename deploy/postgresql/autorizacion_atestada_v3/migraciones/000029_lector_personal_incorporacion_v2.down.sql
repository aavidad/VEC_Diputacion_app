...
DO $proteger$
DECLARE v_oid oid := 'vec_autorizacion_atestada_v3.registrar_y_consumir_lectura_personal_incorporacion_v2_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure;
BEGIN
 -- Fotografía exacta del wrapper instalado por UP29, antes de cambiar núcleo,
 -- audiencias o función. Los OID de tipos/lenguaje se resuelven en catálogo.
 IF NOT EXISTS (
   SELECT 1 FROM pg_proc p JOIN pg_language l ON l.oid=p.prolang
   WHERE p.oid=v_oid
     AND p.proowner='vec_autorizacion_atestada_v3_propietario'::regrole
     AND p.pronamespace='vec_autorizacion_atestada_v3'::regnamespace
     AND p.proname='registrar_y_consumir_lectura_personal_incorporacion_v2_atestada'
     AND p.prokind='f' AND l.lanname='plpgsql'
     AND p.prosecdef AND p.proretset AND p.prorettype='record'::regtype
     AND p.provolatile='v' AND p.proparallel='u'
     AND NOT p.proisstrict AND NOT p.proleakproof
     AND p.pronargs=10 AND p.pronargdefaults=0 AND p.provariadic=0
     AND p.proargtypes=ARRAY['bytea'::regtype::oid,'bytea'::regtype::oid,
       'bytea'::regtype::oid,'bytea'::regtype::oid,'numeric'::regtype::oid,
       'numeric'::regtype::oid,'bytea'::regtype::oid,'bytea'::regtype::oid,
       'bytea'::regtype::oid,'bytea'::regtype::oid]::oidvector
     AND p.proallargtypes=ARRAY['bytea'::regtype::oid,'bytea'::regtype::oid,
       'bytea'::regtype::oid,'bytea'::regtype::oid,'numeric'::regtype::oid,
       'numeric'::regtype::oid,'bytea'::regtype::oid,'bytea'::regtype::oid,
       'bytea'::regtype::oid,'bytea'::regtype::oid,'text'::regtype::oid,
       'text'::regtype::oid,'text'::regtype::oid,'text'::regtype::oid,
       'text'::regtype::oid,'timestamptz'::regtype::oid,'boolean'::regtype::oid]
     AND p.proargmodes=ARRAY['i','i','i','i','i','i','i','i','i','i',
       't','t','t','t','t','t','t']::"char"[]
     ...
     AND p.proargdefaults IS NULL AND p.probin IS NULL AND p.prosqlbody IS NULL
     AND p.protrftypes IS NULL AND p.prosupport=0 AND p.procost=100 AND p.prorows=1000
     AND obj_description(p.oid,'pg_proc') IS NULL
     AND octet_length(p.prosrc)=546
     AND encode(sha256(convert_to(p.prosrc,'UTF8')),'hex')=
       'e5893a21e932583f2c1ffb49f2195a0507f1fff201f097555c3ea337ef618b02'
 ) THEN
   RAISE EXCEPTION 'retirada AD3-29 denegada: definición o metadatos alterados' USING ERRCODE='55000';
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=v_oid AND p.proowner='vec_autorizacion_atestada_v3_propietario'::regrole AND p.prosecdef)
    OR EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='vec_autorizacion_atestada_v3' AND p.proname='registrar_y_consumir_lectura_personal_incorporacion_v2_atestada' AND p.oid<>v_oid)
    OR EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.clave_capacidad_version WHERE audiencia_consumo='vec_personal.lectura_incorporacion.v2')
    OR EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.atestacion_decision_v3 WHERE convert_from(capacidad_canonica,'UTF8')::jsonb->>'audiencia_consumo'='vec_personal.lectura_incorporacion.v2')
    OR EXISTS (SELECT 1 FROM pg_depend d WHERE d.refclassid='pg_proc'::regclass AND d.refobjid=v_oid AND d.deptype<>'i')
    OR EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid<>v_oid AND p.prosrc LIKE '%registrar_y_consumir_lectura_personal_incorporacion_v2_atestada%') THEN
  RAISE EXCEPTION 'retirada AD3-29 denegada: función, sobrecarga, clave, historia o dependencia' USING ERRCODE='55000';
 END IF;
 IF EXISTS (
...
BEGIN
 ...
 FOREACH v_par IN ARRAY ARRAY[v_perfil1||v_perfil2,v_lector1||v_lector2,v_exclusion2] LOOP
 ...
 v_nueva := replace(replace(replace(v_def,v_perfil1||v_perfil2,v_perfil1),v_lector1||v_lector2,v_lector1),v_exclusion2,v_exclusion1);
 EXECUTE v_nueva;
...
BEGIN
 ...
 IF v_def='CHECK (audiencia_consumo = ANY (ARRAY['||array_to_string(ARRAY(SELECT quote_literal(a)||'::text' FROM unnest(v_a7) WITH ORDINALITY u(a,n) ORDER BY n),', ')||']))' THEN v_a:=v_a7[1:7]; ELSIF v_def='CHECK (audiencia_consumo = ANY (ARRAY['||array_to_string(ARRAY(SELECT quote_literal(a)||'::text' FROM unnest(v_a11) WITH ORDINALITY u(a,n) ORDER BY n),', ')||']))' THEN v_a:=v_a11[1:11]; ELSE RAISE EXCEPTION 'audiencias incompatibles; no retirar AD3-29' USING ERRCODE='55000'; END IF;
...
END $audiencia$;
DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_lectura_personal_incorporacion_v2_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
COMMIT;
