...
-- AD3-83: consumidor nominal de la modificación de fechas o jornada de un
-- expediente ya nombrado (petición RRHH p. 1, punto 3; duda 12). Perfil y
-- audiencia propios; la fachada exige consumo nuevo y el único llamante es el
-- propietario de CT (CT 000116).
...
SELECT pg_advisory_xact_lock(hashtextextended('vec_autorizacion_atestada_v3:migracion:000083',0));
...
BEGIN
 IF current_user<>'vec_autorizacion_atestada_v3_propietario'
    OR to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_modificacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL
 ...
 THEN RAISE EXCEPTION 'AD3-83: preimagen incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-83: núcleo incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-83: núcleo alterado fuera de contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;

LOCK TABLE vec_autorizacion_atestada_v3.clave_capacidad_version IN ACCESS EXCLUSIVE MODE;
DO $audiencias$
DECLARE d text; nueva text; audiencia text;
 lista text[]:=ARRAY['vec_contratacion_temporal.modificacion_tras_nombramiento.v1'];
BEGIN
 ...
 THEN RAISE EXCEPTION 'AD3-83: audiencias previas incompatibles' USING ERRCODE='55000'; END IF;
 nueva:=left(d,length(d)-3);
 FOREACH audiencia IN ARRAY lista LOOP
  IF strpos(d,quote_literal(audiencia))<>0 THEN
   RAISE EXCEPTION 'AD3-83: audiencia ya registrada' USING ERRCODE='55000'; END IF;
...
-- Modificación: el recurso es el expediente nombrado cuya versión de análisis cambia.
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_modificacion_ct_v3_atestada(
...
BEGIN
 ...
 EXCEPTION WHEN others THEN RAISE EXCEPTION 'AD3-83: material de modificación inválido' USING ERRCODE='22023'; END;
 IF c->>'operacion' IS DISTINCT FROM 'contratacion_temporal.expediente.modificar_tras_nombramiento'
    OR c->>'audiencia_consumo' IS DISTINCT FROM 'vec_contratacion_temporal.modificacion_tras_nombramiento.v1'
    OR d->>'accion' IS DISTINCT FROM c->>'operacion'
    OR d->>'modulo_id' IS DISTINCT FROM 'contratacion_temporal'
    OR d->>'tipo_recurso' IS DISTINCT FROM 'modificacion_contratacion_temporal'
 ...
 IF x.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'AD3-83: la modificación requiere consumo nuevo' USING ERRCODE='42501'; END IF;
...
BEGIN
 FOREACH f IN ARRAY ARRAY[
  'vec_autorizacion_atestada_v3.registrar_y_consumir_modificacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure] LOOP
  ...
  THEN RAISE EXCEPTION 'AD3-83: propietario o entorno de fachada incompatible' USING ERRCODE='55000'; END IF;
  FOR x IN SELECT a.grantee,a.privilege_type,a.is_grantable,p.proowner
   ...
   THEN RAISE EXCEPTION 'AD3-83: ACL de fachada abierta' USING ERRCODE='55000'; END IF;
  END LOOP;
 END LOOP;
END $acl$;
COMMIT;
