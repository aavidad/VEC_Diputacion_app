...
-- AD3-85 DOWN: solo sin historia. Se rechaza si CT118 sigue instalada o si
-- ya existe alguna clave de capacidad de la audiencia de firma; en ese caso
-- la reversión exige un procedimiento supervisado que conserve decisiones,
-- consumos y auditoría. Sin historia deja el núcleo, las audiencias y las
-- ACL exactamente como antes de AD3-85.
BEGIN;
...
DO $pre$
BEGIN
 IF to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_firma_documento_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NULL
 THEN RAISE EXCEPTION 'AD3-85 DOWN: AD3-85 no instalada' USING ERRCODE='55000'; END IF;
 IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
            WHERE n.nspname='vec_contratacion_temporal' AND c.relname='firma_documento_v1')
 THEN RAISE EXCEPTION 'AD3-85 DOWN: CT118 sigue instalada' USING ERRCODE='55000'; END IF;
 IF EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.clave_capacidad_version
            WHERE audiencia_consumo='vec_contratacion_temporal.firma_documento.v1')
 THEN RAISE EXCEPTION 'AD3-85 DOWN: no admitido con historia de la audiencia de firma' USING ERRCODE='55000'; END IF;
END $pre$;

DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_firma_documento_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
...
BEGIN
 ...
 -- Solo se retira la audiencia de firma; las añadidas después se conservan.
 IF strpos(d,'CHECK (audiencia_consumo = ANY (ARRAY[')<>1 OR right(d,3)<>']))'
    OR length(d)-length(replace(d,quote_literal('vec_contratacion_temporal.firma_documento.v1'),''))
       <>length(quote_literal('vec_contratacion_temporal.firma_documento.v1'))
 THEN RAISE EXCEPTION 'AD3-85 DOWN: audiencias incompatibles' USING ERRCODE='55000'; END IF;
 resto:=replace(d,', '||quote_literal('vec_contratacion_temporal.firma_documento.v1')||'::text','');
 IF resto=d OR strpos(resto,'firma_documento.v1')<>0 THEN
  RAISE EXCEPTION 'AD3-85 DOWN: audiencia de firma no localizada' USING ERRCODE='55000';
 END IF;
...
BEGIN
 ...
 THEN RAISE EXCEPTION 'AD3-85 DOWN: núcleo alterado fuera del contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;
COMMIT;
