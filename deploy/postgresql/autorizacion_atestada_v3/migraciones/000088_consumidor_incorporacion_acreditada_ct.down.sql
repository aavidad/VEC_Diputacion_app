...
BEGIN
 ...
 IF actual IS DISTINCT FROM nuevo OR strpos(actual,'confirmacion_ginpix_ct')<>0 OR strpos(actual,'incorporacion_centro_ct')<>0 OR strpos(actual,'no_incorporacion_ct')<>0
...
DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_no_incorporacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
...
BEGIN
 ...
 nueva:=replace(d,', ''vec_contratacion_temporal.confirmacion_ginpix.v1''::text, ''vec_contratacion_temporal.no_incorporacion.v1''::text','');
 IF length(d)-length(nueva)<>length(', ''vec_contratacion_temporal.confirmacion_ginpix.v1''::text, ''vec_contratacion_temporal.no_incorporacion.v1''::text')
