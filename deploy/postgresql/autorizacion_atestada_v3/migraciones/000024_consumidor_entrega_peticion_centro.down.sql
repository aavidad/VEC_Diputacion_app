    ...
    IF EXISTS (
        SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
         WHERE n.nspname='vec_contratacion_temporal'
           AND p.proname='gestionar_entrega_peticion_centro_v1'
    ) OR EXISTS (
        ...
        RAISE EXCEPTION 'reversión denegada: consumidor o historia de entrega de petición' USING ERRCODE='55000';
    END IF;
...
END
$retirar$;
DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_entrega_peticion_centro_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
COMMIT;
