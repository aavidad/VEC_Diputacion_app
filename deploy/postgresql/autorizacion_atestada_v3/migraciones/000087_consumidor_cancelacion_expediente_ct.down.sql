...
-- AD3-87 DOWN: solo sin historia. Se rechaza si CT 000122 sigue instalada o
-- si ya existe alguna clave de capacidad de la audiencia de cancelación; sin
-- historia deja el núcleo, las audiencias y las ACL como antes de AD3-87.
BEGIN;
...
DO $pre$
BEGIN
 IF to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_cancelacion_expediente_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NULL
 THEN RAISE EXCEPTION 'AD3-87 DOWN: AD3-87 no instalada' USING ERRCODE='55000'; END IF;
 IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
            WHERE n.nspname='vec_contratacion_temporal' AND c.relname='cancelacion_expediente_v1')
 THEN RAISE EXCEPTION 'AD3-87 DOWN: CT 000122 sigue instalada' USING ERRCODE='55000'; END IF;
 IF EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.clave_capacidad_version
            WHERE audiencia_consumo='vec_contratacion_temporal.cancelacion_expediente.v1')
 THEN RAISE EXCEPTION 'AD3-87 DOWN: no admitido con historia de la audiencia de cancelación' USING ERRCODE='55000'; END IF;
END $pre$;

DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_cancelacion_expediente_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);

DO $audiencias$
DECLARE d text; resto text; audiencia text:='vec_contratacion_temporal.cancelacion_expediente.v1';
BEGIN
 ...
 -- Solo se retira la audiencia de cancelación; las añadidas después se conservan.
 ...
 THEN RAISE EXCEPTION 'AD3-87 DOWN: audiencias incompatibles' USING ERRCODE='55000'; END IF;
 resto:=replace(d,', '||quote_literal(audiencia)||'::text','');
 IF resto=d OR strpos(resto,'cancelacion_expediente.v1')<>0 THEN
  RAISE EXCEPTION 'AD3-87 DOWN: audiencia de cancelación no localizada' USING ERRCODE='55000';
 END IF;
...
BEGIN
 ...
 IF length(original)-length(replace(original,extension,''))<>length(extension)
    ...
    OR length(original)-length(replace(original,'cancelacion_expediente_ct',''))<>length('cancelacion_expediente_ct')
 THEN RAISE EXCEPTION 'AD3-87 DOWN: extensión no localizada en el núcleo' USING ERRCODE='55000'; END IF;
 nuevo:=replace(original,extension,'');
 EXECUTE nuevo;
 SELECT pg_get_functiondef(f) INTO STRICT actual;
 IF actual IS DISTINCT FROM nuevo OR strpos(actual,'cancelacion_expediente_ct')<>0
 ...
 THEN RAISE EXCEPTION 'AD3-87 DOWN: núcleo alterado fuera del contrato' USING ERRCODE='55000'; END IF;
END $nucleo$;
COMMIT;
