...
BEGIN
    SELECT pg_get_functiondef(p.oid),to_jsonb(p)-'prosrc' INTO STRICT def,metadata FROM pg_proc p
        ...
        AND encode(sha256(convert_to(prosrc,'UTF8')),'hex')='2f15b9ca8b64d453e79063fe706a5ac87ec3453e5c9451e071ecfc6bc30b2ff0';
    ...
    nueva:=replace(nueva,$antes$p_perfil_consulta NOT IN ('cuadro', 'detalle', 'contacto_usuario')$antes$,$despues$p_perfil_consulta NOT IN ('cuadro', 'detalle')$despues$);
    nueva:=replace(nueva,$antes$IF p_perfil_consulta = 'contacto_usuario' THEN
    ...
    nueva:=replace(nueva,$antes$d ->> 'modulo_id' IS DISTINCT FROM CASE WHEN p_perfil_consulta='contacto_usuario'
