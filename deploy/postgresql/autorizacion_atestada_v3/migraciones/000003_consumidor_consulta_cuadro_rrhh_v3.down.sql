...
BEGIN
    IF pg_catalog.to_regprocedure(
       ...
       OR pg_catalog.to_regprocedure(
        ...
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'orden de reversión de consulta RRHH inválido';
    END IF;
    ...
    IF v_definicion IS NULL
       OR v_definicion <>
        ...
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'gobierno de audiencias de cuadro inesperado';
    END IF;
    SELECT
        EXISTS (
            SELECT 1
              FROM vec_autorizacion_atestada_v3.clave_capacidad_version k
             WHERE k.audiencia_consumo IN (
    ...
    IF v_hay_historia THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'reversión protegida: existe historia de consulta RRHH';
    END IF;
    ALTER TABLE
