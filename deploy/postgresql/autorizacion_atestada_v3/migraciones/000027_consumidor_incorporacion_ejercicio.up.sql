...
-- Primero: compartido con CT000070 y el futuro negocio CT, no con Personal.
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
       ...
       OR NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='vec_contratacion_temporal_propietario'
        ...
        RAISE EXCEPTION 'estado incompatible para AD3-27' USING ERRCODE='55000';
    END IF;
END
$precondicion$;
...
DO $ampliar$
DECLARE
    v_def text; v_acl aclitem[]; v_owner oid; v_config text[];
...
BEGIN
    ...
    IF length(v_def)-length(replace(v_def,v_marca,''))<>length(v_marca)
       ...
       OR strpos(v_def,'p_perfil_mutacion IS NOT DISTINCT FROM ''resolucion_formalizacion_ct''')=0 THEN
        RAISE EXCEPTION 'núcleo incompatible para incorporación de ejercicio' USING ERRCODE='55000';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid='vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna(text,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure
               AND (p.proacl IS DISTINCT FROM v_acl OR p.proowner IS DISTINCT FROM v_owner OR p.proconfig IS DISTINCT FROM v_config OR NOT p.prosecdef)) THEN
        RAISE EXCEPTION 'AD3-27 alteró núcleo o autoridad ajena' USING ERRCODE='55000';
    END IF;
END
$ampliar$;
...
DO $audiencia$
DECLARE v_def text; v_a5 text[] := ARRAY['vec_contratacion_temporal.confirmar_alta_atestada.v1','vec_bolsa_llamamientos.confirmar_integracion_desarrollo.v1','vec_contratacion_temporal.consultar_cuadro_rrhh_atestado.v1','vec_contratacion_temporal.consultar_detalle_rrhh_atestado.v1','vec_personal.alta_ejercicio.v1']; v_a9 text[] := ARRAY['vec_contratacion_temporal.confirmar_alta_atestada.v1','vec_contratacion_temporal.consultar_cuadro_rrhh_atestado.v1','vec_contratacion_temporal.consultar_detalle_rrhh_atestado.v1','vec_contexto_actor.publicar_organizacion_corporativa_fuente.v1','vec_contexto_actor.revocar_organizacion_corporativa_fuente.v1','vec_contexto_actor.publicar_vinculo_corporativo_fuente.v1','vec_contexto_actor.revocar_vinculo_corporativo_fuente.v1','vec_bolsa_llamamientos.confirmar_integracion_desarrollo.v1','vec_personal.alta_ejercicio.v1']; v_a text[];
BEGIN
...
CREATE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_incorporacion_ejercicio_ct_v3_atestada(p_capacidad bytea,p_decision bytea,p_motivo bytea,p_contexto bytea,p_persona_version numeric,p_perfil_version numeric,p_payload bytea,p_sobre bytea,p_evidencia bytea,p_raiz bytea)
RETURNS TABLE(decision_ref text,efecto_ref text,huella_efecto_sha256 text,consumo_huella_sha256 text,auditoria_ref text,consumida_en timestamptz,consumo_nuevo boolean)
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog SET lock_timeout='2s' AS $f$
DECLARE v record;
BEGIN
 SELECT * INTO STRICT v FROM vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna('incorporacion_ejercicio_ct',p_capacidad,p_decision,p_motivo,p_contexto,p_persona_version,p_perfil_version,p_payload,p_sobre,p_evidencia,p_raiz);
 IF v.consumo_nuevo IS NOT TRUE THEN RAISE EXCEPTION 'incorporación de ejercicio requiere consumo nuevo' USING ERRCODE='P1102'; END IF;
...
END $f$;
ALTER FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_incorporacion_ejercicio_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) OWNER TO vec_autorizacion_atestada_v3_propietario;
REVOKE ALL ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_incorporacion_ejercicio_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) FROM PUBLIC,vec_autorizacion_atestada_v3_consumidor,vec_autorizacion_atestada_v3_emisor,vec_autorizacion_atestada_v3_migrador,vec_personal_propietario,vec_personal_ejecutor,vec_personal_migrador,vec_contratacion_temporal_ejecutor,vec_contratacion_temporal_migrador,vec_bolsa_llamamientos_propietario,vec_bolsa_llamamientos_ejecutor,vec_bolsa_llamamientos_migrador;
GRANT EXECUTE ON FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_incorporacion_ejercicio_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) TO vec_contratacion_temporal_propietario;
COMMIT;
