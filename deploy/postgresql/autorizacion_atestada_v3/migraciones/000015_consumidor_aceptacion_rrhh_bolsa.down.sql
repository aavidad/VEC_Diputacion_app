...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
        WHERE n.nspname='vec_bolsa_llamamientos' AND p.proname='guardar_integracion_desarrollo_v1'
          AND strpos(pg_get_functiondef(p.oid),'bolsa.llamamiento.aceptacion_rrhh.registrar')>0)
       OR EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.atestacion_decision_v3 a
          WHERE convert_from(a.capacidad_canonica,'UTF8')::jsonb->>'operacion'='bolsa.llamamiento.aceptacion_rrhh.registrar') THEN
        RAISE EXCEPTION 'reversión denegada: consumidor o historia terminal de Bolsa' USING ERRCODE='55000';
    END IF;
    ...
    IF length(v_def)-length(replace(v_def,v_nuevo,''))<>length(v_nuevo)
       OR strpos(v_def,v_anterior)<>0 THEN
        RAISE EXCEPTION 'núcleo incompatible para retirar aceptación RRHH' USING ERRCODE='55000';
    END IF;
    ...
    IF (SELECT proacl FROM pg_proc WHERE oid='vec_autorizacion_atestada_v3.consumir_decision_mutacion_v3_interna(text,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure) IS DISTINCT FROM v_acl THEN
        RAISE EXCEPTION 'reversión de aceptación alteró permisos del núcleo' USING ERRCODE='55000';
    END IF;
END
$retirar$;
COMMIT;
