...
-- AD3-82 DOWN: solo sin historia. Exige que CT 000115 esté retirada y que no
-- exista ninguna clave de capacidad publicada para las dos audiencias: sin
...
BEGIN
 IF current_user<>'vec_autorizacion_atestada_v3_propietario'
    OR EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
               WHERE n.nspname='vec_contratacion_temporal' AND c.relname='cese_nombramiento_v1')
    ...
    OR EXISTS (SELECT 1 FROM vec_autorizacion_atestada_v3.clave_capacidad_version
               WHERE audiencia_consumo IN ('vec_contratacion_temporal.cese.v1','vec_contratacion_temporal.cierre_expediente.v1'))
 THEN RAISE EXCEPTION 'AD3-82: DOWN no admitido (CT 000115 instalada o historia de claves)' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'AD3-82: extensión del núcleo no localizada exactamente una vez' USING ERRCODE='55000'; END IF;
 nuevo:=replace(original,extension,'');
 EXECUTE nuevo;
 SELECT pg_get_functiondef(f) INTO STRICT actual;
 IF actual IS DISTINCT FROM nuevo OR strpos(actual,'''cese_ct''')<>0 OR strpos(actual,'cierre_expediente_ct')<>0
 ...
 THEN RAISE EXCEPTION 'AD3-82: núcleo alterado fuera de contrato al revertir' USING ERRCODE='55000'; END IF;
END $nucleo$;

DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_cese_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_cierre_expediente_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
...
BEGIN
 ...
 nueva:=replace(replace(d,', ''vec_contratacion_temporal.cese.v1''::text',''),', ''vec_contratacion_temporal.cierre_expediente.v1''::text','');
 IF length(d)-length(nueva)<>length(', ''vec_contratacion_temporal.cese.v1''::text')+length(', ''vec_contratacion_temporal.cierre_expediente.v1''::text')
 THEN RAISE EXCEPTION 'AD3-82: audiencias no localizadas al revertir' USING ERRCODE='55000'; END IF;
