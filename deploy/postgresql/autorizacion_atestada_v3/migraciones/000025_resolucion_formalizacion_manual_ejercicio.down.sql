    ...
    IF EXISTS (
        SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
         WHERE n.nspname='vec_contratacion_temporal'
           AND p.proname='registrar_resolucion_formalizacion_v1'
    ) OR EXISTS (
        ...
        RAISE EXCEPTION 'reversión denegada: consumidor o historia de resolución manual de ejercicio' USING ERRCODE='55000';
    END IF;
...
END
$retirar$;
DROP FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_resolucion_formalizacion_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
COMMIT;
