#!/usr/bin/env bash
# AD3-57 sobre la PREIMAGEN SINTÉTICA de AD3-51/52 (stub del núcleo) con
# AD3-53 y AD3-70 reales: rechazo sin AD3-70, ROLLBACK, COMMIT, anclas,
# audiencias, ACL de las cuatro fachadas, contratos previos intactos,
...
container="vec-ad3-57-${RANDOM}${RANDOM}"
...
m57="$mig/000057_consumidores_cronos_resolucion_avisos.up.sql"
...
if run < "$m57" >/dev/null 2>&1; then echo 'FALLO AD3-57 aceptada sin AD3-70' >&2; exit 1; fi
printf 'OK AD3-57 rechazada sin AD3-70\n'
...
sed '$s/^COMMIT;$/ROLLBACK;/' "$m57" | run >/dev/null
...
comprobar "SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='vec_autorizacion_atestada_v3' AND (p.proname LIKE '%cronos_bandeja%' OR p.proname LIKE '%cronos_resolucion%' OR p.proname LIKE '%cronos_avisos%' OR p.proname LIKE '%cronos_archivo%')" 0 'ROLLBACK sin fachadas'

run < "$m57" >/dev/null
contar() { printf "SELECT (length(d)-length(replace(d,%s,'')))/length(%s) FROM pg_get_functiondef('%s'::regprocedure) d" "$1" "$1" "$nucleo"; }
for perfil in cronos_marcaje_propio cronos_saldo_propio cronos_movimientos_propio cronos_correccion_solicitar cronos_permisos_propio cronos_permiso_solicitar cronos_permisos_bandeja cronos_permiso_resolver cronos_avisos_propio cronos_aviso_archivar; do
...
comprobar "$(contar "'''cronos_aviso_archivar'')'")" 2 'guarda de sesión y contrato con la lista ampliada'
comprobar "$(contar "'''cronos_permiso_solicitar'')'")" 0 'lista previa sustituida en ambos sitios'
for audiencia in saldo_propio.consultar.v1 permiso_propio.solicitar.v1 permisos_bandeja.consultar.v1 permiso.resolver.v1 avisos_propio.consultar.v1 aviso_propio.archivar.v1; do
...
for fachada in consumir_cronos_bandeja_permisos_v3_atestada registrar_y_consumir_cronos_resolucion_permiso_v3_atestada consumir_cronos_avisos_propio_v3_atestada registrar_y_consumir_cronos_archivo_aviso_v3_atestada registrar_y_consumir_cronos_permiso_v3_atestada; do
...
printf 'PG18.4: AD3-57 ROLLBACK/COMMIT, ACL y reinicio sobre PREIMAGEN SINTÉTICA con AD3-53/70; NO acredita la cadena AD3 real.\n'
