...
-- Ejecutar por la sesión administrativa de catálogo separada del login que
-- realiza los consumos CT86; aquí no se llama a ningún consumidor V3.
-- Sólo una base DESECHABLE vec_ct86_prueba_<12 hex>, identificada además por
-- pg_temp.ct86_entorno(base name PRIMARY KEY, solo_sinteticos boolean), una fila
-- current_database()/true. Nunca la base conservada; este archivo no instala SQL.
-- El runner autorizado conserva en LA MISMA SESIÓN antes de instalar AD3-30:
--   CREATE TEMP TABLE ad3_30_preimagen AS
...
DO $catalogos$
DECLARE
 nuevo text:='vec_autorizacion_atestada_v3.registrar_y_consumir_anotacion_administrativa_ct_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)';
...
BEGIN
 IF current_database() !~ '^vec_ct86_prueba_[a-f0-9]{12}$'
  ...
  OR to_regclass('pg_temp.ad3_30_preimagen') IS NULL THEN
  RAISE EXCEPTION 'AD3-30: falta entorno desechable/preimagen anterior'; END IF;
 IF (SELECT count(*) FROM pg_temp.ct86_entorno)<>1 OR NOT EXISTS(
  ...
  RAISE EXCEPTION 'AD3-30: entorno no identificado como sintético'; END IF;
 ...
 IF f IS NULL OR to_regprocedure(nucleo) IS NULL OR to_regprocedure(alta) IS NULL THEN
  RAISE EXCEPTION 'AD3-30: consumidores requeridos ausentes'; END IF;
 IF EXISTS(SELECT 1 FROM pg_temp.ad3_30_preimagen WHERE identidad=nuevo)
  OR NOT EXISTS(SELECT 1 FROM pg_temp.ad3_30_preimagen WHERE identidad=nucleo)
  OR NOT EXISTS(SELECT 1 FROM pg_temp.ad3_30_preimagen WHERE identidad=alta)
  OR EXISTS(SELECT identidad FROM pg_temp.ad3_30_preimagen GROUP BY identidad HAVING count(*)<>1)
  OR (SELECT count(*) FROM pg_proc WHERE pronamespace='vec_autorizacion_atestada_v3'::regnamespace)
     <> 1+(SELECT count(*) FROM pg_temp.ad3_30_preimagen) THEN
  RAISE EXCEPTION 'AD3-30: preimagen/inventario incompatible'; END IF;
 ...
 FOR b IN SELECT * FROM pg_temp.ad3_30_preimagen LOOP
  IF b.identidad IS NULL OR b.metadatos IS NULL OR b.definicion IS NULL
   OR to_regprocedure(b.identidad) IS NULL THEN
   RAISE EXCEPTION 'AD3-30: preimagen incompleta o función previa retirada'; END IF;
  ...
  IF b.identidad=nucleo THEN
   IF (to_jsonb(p)-'prosrc') IS DISTINCT FROM (b.metadatos-'prosrc')
    ...
    RAISE EXCEPTION 'AD3-30: metadatos núcleo alterados o extensión ausente'; END IF;
  ELSIF to_jsonb(p) IS DISTINCT FROM b.metadatos OR pg_get_functiondef(p.oid) IS DISTINCT FROM b.definicion THEN
   RAISE EXCEPTION 'AD3-30: definición/ACL/autoridad previa alterada';
  END IF;
 END LOOP;
 ...
 IF p.proowner<>propietario OR NOT p.prosecdef OR p.provolatile<>'v'
  ...
  RAISE EXCEPTION 'AD3-30: contrato catalogal del consumidor nuevo incorrecto'; END IF;
 ...
 IF EXISTS(SELECT 1 FROM aclexplode(p.proacl) a
  ...
  RAISE EXCEPTION 'AD3-30: consumidor nuevo accesible fuera del propietario CT'; END IF;
 FOREACH identidad IN ARRAY ARRAY['vec_autorizacion_atestada_v3_consumidor','vec_autorizacion_atestada_v3_emisor',
  ...
  IF has_function_privilege(identidad,f,'EXECUTE') THEN
   RAISE EXCEPTION 'AD3-30: ejecución heredada no prevista para %',identidad; END IF;
 END LOOP;
 IF EXISTS(SELECT 1 FROM pg_roles WHERE oid IN (propietario,ct)
  ...
  RAISE EXCEPTION 'AD3-30: rol propietario privilegiado o con login'; END IF;
 RAISE NOTICE 'AD3-30: catálogo y preimagen comprobados; no acredita consumo V3 dinámico';
END $catalogos$;
ROLLBACK;
