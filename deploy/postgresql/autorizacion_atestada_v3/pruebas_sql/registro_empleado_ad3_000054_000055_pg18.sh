#!/usr/bin/env bash
# Ensayo estructural PG18.4 desechable de AD3-54 (registro de empleado B2) y
# AD3-55 (catálogos B2) sobre la postimagen de main: núcleo 051/052 con stub
...
contenedor="vec-b2-ad3-54-55-${BASHPID}"
...
for operacion in alta.registrar hecho.registrar ficha.consultar vacantes.consultar catalogo.publicar catalogo.retirar catalogo.consultar; do
...
for audiencia in alta.v1 hecho.v1 ficha.v1 vacantes.v1 catalogo.publicar.v1 catalogo.retirar.v1 catalogo.consultar.v1; do
...
ok 'ACL nominal: solo vec_personal_propietario ejecuta las dos fachadas'
psql_admin <<'SQL'
DO $neg$
BEGIN
 BEGIN
...
esperar
[[ $(huella) == "$antes" && $(valor "SELECT to_regprocedure('$f54') IS NOT NULL AND to_regprocedure('$f55') IS NOT NULL") == t ]] || fallo 'estado perdido tras reinicio'
ok 'núcleo, audiencias y fachadas persisten tras reinicio'
printf 'PG18.4: AD3-54/55 sobre postimagen main (51/52 stub; 53/59/61/70/80 reales): ROLLBACK, COMMIT, ACL, negativos y reinicio; NO acredita decisión V3 firmada.\n'
