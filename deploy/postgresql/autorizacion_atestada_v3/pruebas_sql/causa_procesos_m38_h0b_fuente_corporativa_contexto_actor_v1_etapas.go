...
// La autorización limita efectos; nunca afirma cuántos se intentaron.
...
// El resultado conserva la cardinalidad real. O4a solo consume este sobre;
// la observación física y los efectos pertenecen a O4b.
...
func autoridadBaseEtapasExactaO4aM38(a *autoridadCausaO4aM38) bool {
	if a == nil || a.auto != a || a.origen == nil || a.origen.auto != a.origen ||
		...
		a.sellos.autoridad.auto != a.sellos.autoridad || a.sellos.custodia == nil ||
		a.sellos.custodia != a.origen.custodia || a.sellos.lease == nil || a.sellos.observador == nil ||
		a.sellos.registro == nil || a.sellos.lease != a.sellos.custodia.lease ||
		a.sellos.observador != a.sellos.custodia.observador || a.sellos.control != a.sellos.custodia.control ||
		a.sellos.controlFD != a.sellos.custodia.controlFD || a.sellos.terminal != a.sellos.custodia.terminal ||
		a.sellos.cmd != a.sellos.custodia.cmd || a.sellos.control == nil || a.sellos.controlFD == nil ||
		a.sellos.terminal == nil || a.sellos.cmd == nil || a.sellos.cmd.Process == nil ||
		a.sellos.proceso != a.sellos.cmd.Process ||
		...
		a.sellos.lease.generacion != a.sellos.generacionLease ||
		a.sellos.observador.generacion != a.sellos.generacionObservador ||
		a.sellos.lease.tid != a.sellos.tid || a.sellos.registro.tid != a.sellos.tid ||
		a.sellos.custodia.tid != a.sellos.tid || a.sellos.ppid != a.sellos.custodia.ppid ||
		...
		a.sellos.retornoCont != a.origen.retornoCont ||
		a.sellos.baselineSenal != a.sellos.custodia.baselineSenal ||
		a.sellos.baselineSenal&mascaraEstadoObservadorO3aM38 != 2 ||
		uint8(a.sellos.baselineSenal>>2) != 0 || a.sellos.identidad.estado != 'T' ||
		a.sellos.identidad.pid != a.sellos.proceso.Pid || a.sellos.identidad.ppid != a.sellos.ppid ||
		a.sellos.identidad.pgid != a.sellos.proceso.Pid || a.sellos.identidad.sid <= 0 ||
		a.sellos.identidad.inicio == 0 || a.sellos.ahoraCaso != a.origen.ahoraCaso ||
		a.sellos.finCaso != a.origen.finCaso || !tiempoMonotonoO3cM38(a.sellos.custodia.finBootstrap) ||
		!a.sellos.ahoraCaso.Before(a.sellos.custodia.finBootstrap) || !tiempoSelladoArbitrajeO4aM38(a) ||
		a.sellos.fisico.mapa == nil || a.sellos.fisico.limite == 0 ||
		!snapshotsIgualesO3aM38(a.sellos.fisico, a.sellos.lease.fisico) ||
		!a.sellos.huellaControl.abierto || !a.sellos.huellaTerminal.abierto ||
		a.sellos.huellaControl.identidad == a.sellos.huellaTerminal.identidad {
		return false
	}
	p := a.sellos.pidfd
...
func emitirEtapaO4aM38(e *autoridadEtapasO4aM38, desde estadoCausaO4aM38, etapa etapaO4bM38, limite time.Time, clase claseLimiteEtapaO4bM38) (*autorizacionEtapaO4aM38, bool) {
	operacion, maxima, valida := especificacionEtapaO4aM38(etapa, limite, clase)
	...
	p.tid, p.etapa, p.cardinalidadMaxima, p.operacion = e.causa.sellos.tid, etapa, maxima, operacion
...
func autorizacionExactaEtapaO4aM38(e *autoridadEtapasO4aM38, p *autorizacionEtapaO4aM38) bool {
	...
	operacion, maxima, valida := especificacionEtapaO4aM38(p.etapa, p.limite, p.claseLimite)
	if !valida || p.operacion != operacion || p.cardinalidadMaxima != maxima {
		return false
	}
...
func resultadoExactoEtapaO4aM38(e *autoridadEtapasO4aM38, r *resultadoEtapaO4bM38) bool {
	p := e.pendiente
	return autorizacionExactaEtapaO4aM38(e, p) && r != nil && r.auto == r &&
		r.autorizacion == p && p.resultado == r && r.generacion == p.generacion &&
		r.etapa == p.etapa && r.limite == p.limite && r.claseLimite == p.claseLimite &&
		r.cardinalidad <= p.cardinalidadMaxima && r.rawPrimero >= 0 && r.rawSegundo >= 0 &&
		p.estado.Load() == uint32(autorizacionConsumidaO4bM38) &&
		e.causa.estado.Load() == uint32(causaA5EsperandoResultadoM38)
}

func marcaResultadoHastaO4aM38(e *autoridadEtapasO4aM38, r *resultadoEtapaO4bM38, limite time.Time) bool {
	return tiempoMonotonoO3cM38(r.observado) && !r.observado.Before(e.causa.sellos.ahoraCaso) &&
		!r.observado.After(limite)
}

func marcaResultadoAntesO4aM38(e *autoridadEtapasO4aM38, r *resultadoEtapaO4bM38, limite time.Time) bool {
	return marcaResultadoHastaO4aM38(e, r, limite) && r.observado.Before(limite)
}
...
func emitirKillO4aM38(e *autoridadEtapasO4aM38, ahora, limite time.Time, clase claseLimiteEtapaO4bM38) *autorizacionEtapaO4aM38 {
	if !tiempoMonotonoO3cM38(ahora) || ahora.Before(e.causa.sellos.ahoraCaso) || !ahora.Before(limite) {
		fatalEtapasO4aM38(e)
	}
	p, ok := emitirEtapaO4aM38(e, causaA5EsperandoResultadoM38, etapaMatarGrupoO4bM38, limite, clase)
	if !ok {
		fatalEtapasO4aM38(e)
	}
	return p
...
func emitirParadaFinalO4aM38(e *autoridadEtapasO4aM38, desde estadoCausaO4aM38, ahora time.Time) *autorizacionEtapaO4aM38 {
	if !tiempoMonotonoO3cM38(ahora) || ahora.Before(e.causa.sellos.ahoraCaso) ||
		ahora.Before(e.plazos.finGracia) || !ahora.Before(e.plazos.finParadaFinal) {
		fatalEtapasO4aM38(e)
	}
	p, ok := emitirEtapaO4aM38(e, desde, etapaParadaFinalO4bM38, e.plazos.finParadaFinal, limiteParadaFinalO4bM38)
	if !ok {
		fatalEtapasO4aM38(e)
	}
	return p
...
func entregarTerminalO4aM38(e *autoridadEtapasO4aM38) *autorizacionEtapaO4aM38 {
	e.terminalidad = true
...
func aplicarResultadoEnO4aM38(e *autoridadEtapasO4aM38, r *resultadoEtapaO4bM38, ahora time.Time) *autorizacionEtapaO4aM38 {
	...
	case etapaParadaInicialO4bM38:
		...
		estable := valido && r.rawPrimero == 0 && r.evidencia == evidenciaEstableO4bM38 &&
			marcaResultadoAntesO4aM38(e, r, e.plazos.finParadaInicial)
		noEstable := valido && r.rawPrimero == 0 && r.evidencia == evidenciaNoEstableO4bM38 &&
		...
		return emitirKillO4aM38(e, ahora, e.plazos.finDrenajeRapido, limiteDrenajeRapidoO4bM38)
	case etapaTerminarReanudarO4bM38:
		falloTerm := r.cardinalidad == 1 && r.rawPrimero != 0 && r.rawSegundo == 0 &&
			r.evidencia == evidenciaSinO4bM38 && r.observado.IsZero()
		falloCont := r.cardinalidad == 2 && r.rawPrimero == 0 && r.rawSegundo != 0 &&
			r.evidencia == evidenciaSinO4bM38 && r.observado.IsZero()
		terminal := r.cardinalidad == 2 && r.rawPrimero == 0 && r.rawSegundo == 0 &&
			r.evidencia == evidenciaTerminalO4bM38 && marcaResultadoHastaO4aM38(e, r, e.plazos.finGracia)
		presente := r.cardinalidad == 2 && r.rawPrimero == 0 && r.rawSegundo == 0 &&
			r.evidencia == evidenciaGrupoPresenteO4bM38 && marcaResultadoHastaO4aM38(e, r, e.plazos.finGracia)
		if !falloTerm && !falloCont && !terminal && !presente {
			fatalEtapasO4aM38(e)
		}
		consumirResultadoO4aM38(e, r)
		if terminal {
			return entregarTerminalO4aM38(e)
		}
		if presente && r.observado == e.plazos.finGracia {
			return emitirParadaFinalO4aM38(e, causaA5EsperandoResultadoM38, ahora)
		}
		if presente {
		...
		enclavarIncidenteCierreO4aM38(e)
		return emitirKillO4aM38(e, ahora, e.plazos.finDrenajeCooperativo, limiteDrenajeCooperativoO4bM38)
	case etapaParadaFinalO4bM38:
		terminal := (r.cardinalidad == 0 || r.cardinalidad == 1) && r.rawPrimero == 0 && r.rawSegundo == 0 &&
			r.evidencia == evidenciaTerminalO4bM38 && marcaResultadoHastaO4aM38(e, r, e.plazos.finParadaFinal)
		valido := r.cardinalidad == 1 && r.rawSegundo == 0
		estable := valido && r.rawPrimero == 0 && r.evidencia == evidenciaEstableO4bM38 &&
		...
		fallo := valido && r.rawPrimero != 0 && r.evidencia == evidenciaSinO4bM38 && r.observado.IsZero()
		if !terminal && !estable && !noEstable && !fallo {
			fatalEtapasO4aM38(e)
		}
		consumirResultadoO4aM38(e, r)
		if terminal {
...
func aplicarResultadoEtapaO4aM38(e *autoridadEtapasO4aM38, entrada **resultadoEtapaO4bM38) (*autorizacionEtapaO4aM38, error) {
	...
	r := *entrada
	*entrada = nil
	estado := r.estado.Load()
	if estado == uint32(resultadoConsumiendoO4bM38) || estado == uint32(resultadoConsumidoO4bM38) {
		return nil, errEtapasConsumidasO4aM38
	}
...
func continuarEtapasEnO4aM38(e *autoridadEtapasO4aM38, ahora time.Time) *autorizacionEtapaO4aM38 {
	...
	return emitirParadaFinalO4aM38(e, causaA3CausaFijadaM38, ahora)
}
