...
container="vec-personal-d7c-000015-$RANDOM"
...
for source in \
 ...
 "$base_dir/rectificacion_dietas_000015_preparar.sql" \
 "$repo_dir/deploy/postgresql/personal/migraciones/000015_solicitud_rectificacion_dietas.up.sql" \
 "$repo_dir/deploy/postgresql/personal/migraciones/000015_solicitud_rectificacion_dietas.down.sql" \
 "$base_dir/rectificacion_dietas_000015.sql" \
 "$base_dir/rectificacion_dietas_000015_operaciones.sql"; do
...
run_sql /tmp/preparar_asignacion_dietas_000012.sql
run_sql /tmp/000012_asignacion_dietas.up.sql
run_sql /tmp/000013_auditoria_frontera_asignacion_dietas.up.sql
run_sql /tmp/rectificacion_dietas_000015_preparar.sql
run_sql /tmp/000014_competencias_asignacion_dietas.up.sql
run_sql /tmp/000015_solicitud_rectificacion_dietas.up.sql
run_sql /tmp/rectificacion_dietas_000015.sql
run_sql /tmp/rectificacion_dietas_000015_operaciones.sql
if down_output=$(run_sql /tmp/000015_solicitud_rectificacion_dietas.down.sql 2>&1); then
  echo 'ERROR: DOWN 000015 borró historia sintética' >&2
  exit 1
fi
if [[ "$down_output" != *'Personal 000015 DOWN protege historia'* ]]; then
  echo "$down_output" >&2
  exit 1
fi
echo 'OK: Personal 000015 PG18 aislado; estructura, ACL, solicitud/consulta/rechazo/confirmación/replay con stub AD3.'
