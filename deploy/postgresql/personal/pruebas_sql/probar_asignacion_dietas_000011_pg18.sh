...
container="vec-personal-d7-000011-$RANDOM"
...
for source in \
  ...
  "$repo_dir/deploy/postgresql/personal/migraciones/000011_asignacion_dietas.down.sql" \
  "$base_dir/asignacion_dietas_000011.sql"; do
...
echo 'PG18 aislado: preimagen TEST-ONLY por ausencia de Personal 000010 y fachadas AD3 nominales TEST-ONLY'
psql_file /tmp/preparar_asignacion_dietas_000011.sql
psql_file /tmp/000011_asignacion_dietas.up.sql
echo 'PG18 aislado: DOWN vacío de 000011 conserva 000009 y retira sólo 000011'
psql_file /tmp/000011_asignacion_dietas.down.sql
...
psql_file /tmp/000011_asignacion_dietas.up.sql
psql_file /tmp/asignacion_dietas_000011.sql
echo 'PG18 aislado: DOWN con alta confirmada debe rechazar y preservar historia'
if psql_file /tmp/000011_asignacion_dietas.down.sql >/tmp/vec-personal-d7-000011-down-historia.out 2>&1; then
  echo 'ERROR: DOWN 000011 aceptó una alta confirmada' >&2
  exit 1
fi
rg -q 'Personal 000011 DOWN protege historia o consumidor' /tmp/vec-personal-d7-000011-down-historia.out
...
echo 'OK: Personal D7 000011 PostgreSQL 18 aislado; ACL nominal, consulta, correcciones separadas, replay/conflicto, historia y sello.'
