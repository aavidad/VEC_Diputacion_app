...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname='vec_personal_localizador_solicitud_alta')
...
END $pre$;
CREATE ROLE vec_personal_localizador_solicitud_alta NOLOGIN NOINHERIT NOSUPERUSER
 NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
DO $conexion$
BEGIN
 EXECUTE format('GRANT CONNECT ON DATABASE %I TO vec_personal_localizador_solicitud_alta',current_database());
END $conexion$;
COMMIT;
