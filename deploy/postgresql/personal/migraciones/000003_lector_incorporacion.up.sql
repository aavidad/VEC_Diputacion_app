...
-- Compartido con AD3-28; no se adquiere este advisory lock en runtime.
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname='vec_personal' AND nspowner=current_user::regrole)
       OR EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
           WHERE n.nspname='vec_personal' AND p.proname='acreditar_alta_ejercicio_v1')
       OR to_regclass('vec_personal.auditoria_lectura_incorporacion') IS NOT NULL THEN
        RAISE EXCEPTION 'estado incompatible Personal 000003' USING ERRCODE='55000';
    END IF;
    FOREACH r IN ARRAY ARRAY['vec_personal_propietario','vec_personal_migrador','vec_personal_ejecutor',
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname=r AND NOT rolcanlogin AND NOT rolsuper
            ...
            RAISE EXCEPTION 'roles incompatibles Personal 000003' USING ERRCODE='55000';
        END IF;
    END LOOP;
    -- Solo se admite USAGE CT directo no delegable, o su ausencia original.
    IF has_schema_privilege('vec_contratacion_temporal_propietario','vec_personal','CREATE')
       ...
       OR pg_has_role('vec_personal_ejecutor','vec_contratacion_temporal_ejecutor','MEMBER') THEN
        RAISE EXCEPTION 'separación o ACL incompatible Personal 000003' USING ERRCODE='55000';
    END IF;
    ...
    SELECT p.oid INTO f FROM pg_proc p WHERE p.oid=to_regprocedure(
        'vec_autorizacion_atestada_v3.registrar_y_consumir_lectura_personal_incorporacion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)')
    ...
    IF f IS NULL OR NOT has_schema_privilege(current_user,'vec_autorizacion_atestada_v3','USAGE')
       OR EXISTS (SELECT 1 FROM pg_proc p, LATERAL aclexplode(COALESCE(p.proacl,acldefault('f',p.proowner))) a
           WHERE p.oid=f AND (a.grantee NOT IN (p.proowner,current_user::regrole) OR
               (a.grantee<>p.proowner AND (a.privilege_type<>'EXECUTE' OR a.is_grantable)))) THEN
        RAISE EXCEPTION 'wrapper AD3-28 propietario requerido' USING ERRCODE='55000';
    END IF;
END
$precondiciones$;

-- Solo historia de lectura: ni alta, ni ocupación, ni relación, ni outbox nuevos.
CREATE TABLE vec_personal.auditoria_lectura_incorporacion (
    auditoria_lectura_ref text PRIMARY KEY CHECK (auditoria_lectura_ref ~ '^auditoria:personal:lectura:[0-9a-f-]{36}$'),
    resultado_ref text NOT NULL REFERENCES vec_personal.registro_alta_ejercicio,
    decision_lectura_ref text NOT NULL UNIQUE,
    consumo_huella_sha256 text NOT NULL UNIQUE CHECK (consumo_huella_sha256 ~ '^[a-f0-9]{64}$'),
    auditoria_v3_ref text NOT NULL UNIQUE,
    contexto_lectura_sha256 text NOT NULL CHECK (contexto_lectura_sha256 ~ '^[a-f0-9]{64}$'),
    ...
    leida_en timestamptz(6) NOT NULL CHECK (isfinite(leida_en) AND leida_en>=consumida_en)
);
ALTER TABLE vec_personal.auditoria_lectura_incorporacion ENABLE ROW LEVEL SECURITY;
ALTER TABLE vec_personal.auditoria_lectura_incorporacion FORCE ROW LEVEL SECURITY;
CREATE POLICY propietario ON vec_personal.auditoria_lectura_incorporacion TO vec_personal_propietario
    USING (true) WITH CHECK (true);
CREATE TRIGGER inmutable BEFORE UPDATE OR DELETE OR TRUNCATE ON vec_personal.auditoria_lectura_incorporacion
    FOR EACH STATEMENT EXECUTE FUNCTION vec_personal.rechazar_mutacion_alta_ejercicio_v1();
-- Marcador cerrado de restauración; no es autoridad ni dato de negocio.
DO $acl$
DECLARE a record;
BEGIN
    EXECUTE format('COMMENT ON TABLE vec_personal.auditoria_lectura_incorporacion IS %L',
        CASE WHEN has_schema_privilege('vec_contratacion_temporal_propietario','vec_personal','USAGE')
          THEN 'Personal000003:v1:ct_usage=presente' ELSE 'Personal000003:v1:ct_usage=ausente' END);
    REVOKE ALL ON vec_personal.auditoria_lectura_incorporacion FROM PUBLIC;
    FOR a IN SELECT DISTINCT grantee FROM pg_class c,LATERAL aclexplode(c.relacl)
        WHERE c.oid='vec_personal.auditoria_lectura_incorporacion'::regclass AND grantee<>c.relowner LOOP
        EXECUTE format('REVOKE ALL ON vec_personal.auditoria_lectura_incorporacion FROM %I',pg_get_userbyid(a.grantee));
    END LOOP;
END
$acl$;

CREATE FUNCTION vec_personal.acreditar_alta_ejercicio_v1(
    p_organizacion_ref text,p_solicitud_ref text,p_expediente_ref text,p_version_expediente bigint,
    p_resultado_ref text,p_recibo_ref text,p_relacion_ref text,p_ocupacion_ref text,p_material_sha256 text,
...
BEGIN
    ...
    END IF;
    -- Única canonicalización nueva: el mapa 1/9 de Recurso.HuellaContextoAutorizacionSHA256.
    ...
    IF jsonb_typeof(d) IS DISTINCT FROM 'object' OR jsonb_typeof(c) IS DISTINCT FROM 'object'
       ...
       OR d->>'tipo_recurso' IS DISTINCT FROM 'registro_alta_ejercicio_incorporacion'
       ...
       OR c->>'audiencia_consumo' IS DISTINCT FROM 'vec_personal.lectura_incorporacion.v1'
    ...
    SELECT * INTO STRICT consumo FROM
        vec_autorizacion_atestada_v3.registrar_y_consumir_lectura_personal_incorporacion_v3_atestada(
...
DO $acl_funcion$
DECLARE a record; f oid:= 'vec_personal.acreditar_alta_ejercicio_v1(text,text,text,bigint,text,text,text,text,text,bytea,bytea,bytea,bytea,bigint,bigint,bytea,bytea,bytea,bytea)'::regprocedure;
BEGIN
...
END
$acl_funcion$;
GRANT USAGE ON SCHEMA vec_personal TO vec_contratacion_temporal_propietario;
COMMIT;
