...
DECLARE
    ...
    origen text:=$antes$ref_relacion:='relacion:personal:ejercicio:'||id;$antes$;
    destino text:=$despues$ref_relacion:='ref:'||encode(sha256(convert_to('vec.personal.relacion.ejercicio.v1:'||id,'UTF8')),'hex');$despues$;
...
BEGIN
    ...
    IF encode(sha256(convert_to(p.prosrc,'UTF8')),'hex') IS DISTINCT FROM 'cc96d9b84f7f47f22b85ad3876b8e4662c0fb0396338cc76e70960516e76a17d'
    ...
    IF despues IS DISTINCT FROM antes
       ...
       OR (SELECT encode(sha256(convert_to(prosrc,'UTF8')),'hex') FROM pg_proc WHERE oid=f)
          IS DISTINCT FROM 'dd8d3d102200cd7e65d31d94cf4a5a9512869eafab60cafbf4eb083f2d35ff9d'
