...
DO $guardas$
DECLARE f oid:=to_regprocedure('vec_personal.localizar_solicitud_alta_ejercicio_v1(text,text,text)');
 r oid:='vec_personal_localizador_solicitud_alta'::regrole;
...
BEGIN
 ...
 OR f IS NULL OR (SELECT count(*) FROM pg_proc WHERE pronamespace='vec_personal'::regnamespace
   AND proname='localizar_solicitud_alta_ejercicio_v1')<>1
 OR NOT EXISTS(SELECT 1 FROM pg_proc p WHERE p.oid=f
   ...
   AND p.proallargtypes=ARRAY[25,25,25,3802,25,25,25,25,25]::oid[]
   AND p.proargmodes=ARRAY['i','i','i','t','t','t','t','t','t']::"char"[]
   AND p.proargnames=ARRAY['p_organizacion_ref','p_expediente_ref','p_solicitud_ref','solicitud_json','resultado_ref','recibo_ref','relacion_ref','ocupacion_ref','material_sha256']
   AND p.proconfig=ARRAY['search_path=pg_catalog','row_security=on','statement_timeout=5s','lock_timeout=2s']
   AND octet_length(p.prosrc)=22619
   AND encode(sha256(convert_to(p.prosrc,'UTF8')),'hex')='5bce8013a7624231de3ed9bec884267c14a10452988f268b40cc84037ed6e1fa')
 ...
 OR (SELECT count(*) FROM pg_namespace n CROSS JOIN LATERAL aclexplode(n.nspacl) a
   WHERE n.oid='vec_personal'::regnamespace AND a.grantee=r)<>1
 OR NOT EXISTS(SELECT 1 FROM pg_namespace n CROSS JOIN LATERAL aclexplode(n.nspacl) a
   WHERE n.oid='vec_personal'::regnamespace AND n.nspowner=o
 ...
 OR EXISTS(SELECT 1 FROM pg_shdepend WHERE refclassid='pg_authid'::regclass AND refobjid=r
   AND (deptype<>'a' OR objsubid<>0 OR NOT (
     ...
     (classid='pg_namespace'::regclass AND objid='vec_personal'::regnamespace) OR
 ...
 OR EXISTS(SELECT 1 FROM pg_proc p WHERE p.oid<>f
   AND position('localizar_solicitud_alta_ejercicio_v1' IN p.prosrc)>0)
 THEN RAISE EXCEPTION 'lector historico tiene consumidores' USING ERRCODE='55000'; END IF;
END $guardas$;
DROP FUNCTION vec_personal.localizar_solicitud_alta_ejercicio_v1(text,text,text) RESTRICT;
REVOKE USAGE ON SCHEMA vec_personal FROM vec_personal_localizador_solicitud_alta RESTRICT;
-- Sin borrar historia, alterar codecs ni ampliar roles ejecutores.
-- Tras retirar LOGIN/membresias de ensayo, usar roles_localizador_solicitud_alta_down.sql.
COMMIT;
