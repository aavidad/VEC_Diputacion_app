...
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog AS $$
DECLARE m jsonb; i jsonb; c jsonb; d jsonb; v jsonb; amb text; atr text; h text; hasta text; accion text; finalidad text; audiencia text; hc text; campos_esperados jsonb;
...
BEGIN
 ...
 campos_esperados:=CASE m->>'operacion'
   ...
   ELSE NULL END;
 IF campos_esperados IS NULL OR d->'campos_permitidos' IS DISTINCT FROM campos_esperados OR d->'obligaciones' IS DISTINCT FROM '[]'::jsonb THEN RETURN false; END IF;
