...
DO $$ BEGIN
 ...
 END IF;
END $$;
-- Las cuatro tablas permanecen bloqueadas hasta COMMIT/ROLLBACK; así no existe
-- una ventana entre la preimagen y los DROP aunque FORCE RLS esté activo.
LOCK TABLE vec_dietas.borrador_comision,vec_dietas.recibo_borrador_comision,vec_dietas.historia_borrador_comision,vec_dietas.auditoria_borrador_comision IN ACCESS EXCLUSIVE MODE;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM vec_dietas.borrador_comision) OR EXISTS(SELECT 1 FROM vec_dietas.recibo_borrador_comision) OR EXISTS(SELECT 1 FROM vec_dietas.historia_borrador_comision) OR EXISTS(SELECT 1 FROM vec_dietas.auditoria_borrador_comision) THEN RAISE EXCEPTION 'Dietas 000001 DOWN protege historia/recibos/auditoría' USING ERRCODE='55000'; END IF;
END $$;
...
DROP TABLE vec_dietas.auditoria_borrador_comision,vec_dietas.historia_borrador_comision,vec_dietas.recibo_borrador_comision,vec_dietas.borrador_comision RESTRICT;
