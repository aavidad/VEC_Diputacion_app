    ...
    IF to_regclass('vec_autorizacion.decision_autorizacion') IS NULL
       ...
       OR to_regprocedure(
        ...
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'estado incompatible para instalar la frontera de convocatorias';
    END IF;
END
$prevalidacion$;

CREATE FUNCTION vec_autorizacion.revalidar_decision_bolsa_convocatorias_v1(
    p_prueba jsonb,
...
BEGIN
    IF p_prueba IS NULL OR jsonb_typeof(p_prueba) <> 'object'
       ...
       OR p_clase_recurso <> 'version_convocatoria_gobernada'
       OR vec_autorizacion.texto_positivo_valido(
           p_recurso_ref, 512
       ) IS NOT TRUE
    ...
    campos_exigidos := CASE p_accion
        WHEN 'bolsa.convocatoria.version.consultar' THEN
    ...
    IF jsonb_typeof(decision_canonica) <> 'object'
       ...
       OR decision_canonica ->> 'esquema' IS DISTINCT FROM
          'vec.autorizacion.decision.reforzada.v1.autenticacion-actor'
       OR recurso_canonico IS DISTINCT FROM
     ...
     WHERE c ->> 'accion' = p_accion
       ...
       AND EXISTS (
           SELECT 1
            ...
            WHERE finalidad = 'consulta_interna_convocatorias'
       );
...
COMMENT ON FUNCTION
    ...
    'Revalida decision exacta de consulta interna de convocatoria, RBAC/ABAC, garantia alta, superficie interna, sesion y ContextoActor. No acredita procedencia COSE.';
COMMIT;
