...
CREATE FUNCTION pg_temp.guardar(p_persona text, p_conv text, p_conv_version int, p_esperada int, p_clave text, p_material text,
  ...
  p_actor text DEFAULT NULL, p_extra jsonb DEFAULT '{}')
RETURNS TABLE(reutilizada boolean, solicitud_ref text, version integer, puntuacion_micropuntos bigint) LANGUAGE sql AS $$
 SELECT * FROM vec_seleccion.guardar_borrador_propio_v1(p_persona, p_conv, p_conv_version, p_esperada, p_clave, pg_temp.huella(p_material),
  ...
  convert_to('cifrado-sintetico-' || p_material, 'UTF8'), pg_temp.huella('doc' || p_persona), '***5678*', p_requisitos,
...
CREATE FUNCTION pg_temp.contenido(p_patron text DEFAULT '{anio}/SOL-{numero}') RETURNS jsonb LANGUAGE sql AS $$
 SELECT jsonb_build_object('turnos', '[{"clave":"libre"}]'::jsonb,
  'requisitos', '[{"clave":"nacionalidad","obligatorio":true,"impide_presentar":true},{"clave":"titulacion","obligatorio":true,"impide_presentar":false}]'::jsonb,
  'baremo', '{"maximo":"20"}'::jsonb, 'numeracion', jsonb_build_object('patron', p_patron, 'ancho', 6), 'fecha_referencia', '2026-10-30')
$$;
...
CREATE TEMP TABLE recibo AS SELECT * FROM pg_temp.presentar(:'A', (pg_temp.borrador(:'A', :'abierta')->>'solicitud_ref'), 4, 'clave-pres-A4', 'p5');
...
SELECT pg_temp.cierto((SELECT p.reutilizada AND p.numero_justificante = r.numero_justificante AND p.presentada_en = r.presentada_en AND p.recibo_ref = r.recibo_ref
  FROM recibo r, pg_temp.presentar(:'A', r.solicitud_ref, 4, 'clave-pres-A4', 'p5') p), 'repetición: mismo recibo, número e instante');
SELECT pg_temp.espera(format($$SELECT * FROM pg_temp.presentar(%L, %L, 4, 'clave-pres-A4', 'otro')$$, :'A', (SELECT solicitud_ref FROM recibo)), 'VSL02');
SELECT pg_temp.espera(format($$SELECT * FROM pg_temp.presentar(%L, %L, 4, 'clave-pres-A5', 'p5')$$, :'A', (SELECT solicitud_ref FROM recibo)), 'VSL04');
SELECT pg_temp.espera(format($$SELECT * FROM pg_temp.guardar(%L, %L, 2, 4, 'clave-borrador-8', 'm8')$$, :'A', :'abierta'), 'VSL04');
...
SELECT pg_temp.cierto((SELECT d->>'numero_justificante' = (SELECT numero_justificante FROM recibo) AND jsonb_array_length(d->'historia') = 5
