...
-- Fuente candidata: T13/2. No instala ni habilita el append genérico.
BEGIN;
...
SELECT pg_advisory_xact_lock(hashtextextended(
    'vec_bolsa_registro_accesos:migracion:000002', 0));
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_namespace
       ...
       OR to_regprocedure('vec_bolsa_registro_accesos.registrar_cambio_configuracion_correo_admin_v1(jsonb,text,text,bigint)') IS NOT NULL
       ...
       OR NOT EXISTS (SELECT 1 FROM pg_proc
                       WHERE oid=to_regprocedure('vec_autorizacion_atestada_v3.registrar_y_consumir_configuracion_correo_admin_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)')
       ...
       OR NOT EXISTS (SELECT 1 FROM pg_roles WHERE oid=admin
        ...
        RAISE EXCEPTION 'T13/2: dependencias incompatibles' USING ERRCODE='55000';
    END IF;
    FOREACH rol IN ARRAY ARRAY[
        ...
        IF has_function_privilege(rol,
           ...
           OR has_table_privilege(rol,
            ...
            RAISE EXCEPTION 'T13/2: acceso administrativo directo incompatible'
                USING ERRCODE='55000';
        END IF;
    END LOOP;
END $dependencias$;
...
-- 2. guardar_configuracion_correo_v1 debe invocarlo en su propia transacción
--    serializable, DESPUÉS de AD3-33 (consumo_nuevo=true), del cotejo exacto
--    huella_efecto_sha256 con SHA256(p_negocio) y del CAS de configuración.
-- 3. p_decision_ref y p_consumo_ref proceden exclusivamente del record devuelto
--    por AD3-33; p_entrada es p_negocio.auditoria y p_version la versión del CAS.
...
-- 4. Un error T13 aborta también estado, consumo V3 y outbox ADMIN. Este wrapper
--    no abre otra transacción, no reconsume V3 y no lee tablas de otro módulo.
CREATE FUNCTION vec_bolsa_registro_accesos.registrar_cambio_configuracion_correo_admin_v1(
...
BEGIN
    IF NOT pg_has_role(session_user,'vec_administracion_ejecutor','MEMBER')
       ...
       OR current_setting('transaction_read_only') <> 'off' THEN
        RAISE EXCEPTION 'T13/2: cambio administrativo no autorizado'
            USING ERRCODE='42501';
    END IF;
    IF p_entrada IS NULL OR octet_length(p_entrada::text)>16384
       OR vec_bolsa_registro_accesos.objeto_tipos_exactos_v1(p_entrada,'{
           ...
           "object_version":"number","result":"string",
           "correlation_ref":"string","occurred_at":"string"
       }'::jsonb) IS NOT TRUE THEN
        RAISE EXCEPTION 'T13/2: auditoria administrativa incompleta'
            USING ERRCODE='22023';
    END IF;
    IF p_decision_ref IS NULL OR p_decision_ref !~ '^[^*?[:space:][:cntrl:]]{1,160}$'
       ...
       OR p_entrada->>'action' <> 'administracion.configuracion_correo.actualizar'
       OR p_entrada->>'module_id' <> 'vec.module.administracion'
       OR p_entrada->>'subject_ref' <> 'configuracion:smtp:diputacion'
       OR p_entrada->>'object_version' IS DISTINCT FROM p_version::text
       OR p_entrada->>'result' <> 'accepted'
       ...
       OR p_entrada->>'occurred_at' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]{1,9})?Z$' THEN
        RAISE EXCEPTION 'T13/2: auditoria administrativa invalida'
            USING ERRCODE='22023';
    END IF;
    -- Conversión explícita del AuditEntry preparado y atestado a la entrada
    ...
    entrada_t13 := jsonb_build_object(
        ...
        'rule_ref','','reason','','result','permitido','before_hash','','after_hash','',
...
END $funcion$;
REVOKE ALL ON FUNCTION vec_bolsa_registro_accesos.registrar_cambio_configuracion_correo_admin_v1(jsonb,text,text,bigint) FROM PUBLIC;
GRANT USAGE ON SCHEMA vec_bolsa_registro_accesos TO vec_administracion_propietario;
GRANT EXECUTE ON FUNCTION vec_bolsa_registro_accesos.registrar_cambio_configuracion_correo_admin_v1(jsonb,text,text,bigint) TO vec_administracion_propietario;

DO $acl_final$
DECLARE
    f oid := 'vec_bolsa_registro_accesos.registrar_cambio_configuracion_correo_admin_v1(jsonb,text,text,bigint)'::regprocedure;
    propietario oid := 'vec_bolsa_accesos_propietario'::regrole;
    admin oid := 'vec_administracion_propietario'::regrole;
BEGIN
    IF NOT COALESCE((SELECT count(*)=2 AND count(DISTINCT a.grantee)=2
        ...
        RAISE EXCEPTION 'T13/2: ACL final incompatible' USING ERRCODE='55000';
    END IF;
END $acl_final$;
COMMIT;
