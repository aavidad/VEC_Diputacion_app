...
BEGIN
    IF EXISTS (SELECT 1 FROM vec_bolsa_registro_accesos.registro_acceso
                WHERE module_id='vec.module.administracion'
                  AND action='administracion.configuracion_correo.actualizar') THEN
        RAISE EXCEPTION 'T13/2 DOWN rechazado: existe historia administrativa'
            USING ERRCODE='55000';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM pg_proc
                WHERE pronamespace=to_regnamespace('vec_administracion')
                  AND strpos(prosrc,'registrar_cambio_configuracion_correo_admin_v1')>0) THEN
        RAISE EXCEPTION 'T13/2 DOWN rechazado: ADMIN conserva la integracion'
            USING ERRCODE='55000';
    END IF;
END $historia$;
DROP FUNCTION vec_bolsa_registro_accesos.registrar_cambio_configuracion_correo_admin_v1(jsonb,text,text,bigint) RESTRICT;
