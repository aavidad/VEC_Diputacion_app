...
-- Sólo para clúster aislado con T13/3 y roles ADMIN instalados, como DBA.
...
BEGIN
    FOREACH rol IN ARRAY ARRAY[
        ...
        IF has_function_privilege(rol,
            'vec_bolsa_registro_accesos.registrar_consulta_configuracion_correo_admin_v1(jsonb,text,text,bigint)','EXECUTE') THEN
...
CREATE SCHEMA vec_prueba_t13_consulta_admin AUTHORIZATION vec_administracion_propietario;
CREATE FUNCTION vec_prueba_t13_consulta_admin.puente(a jsonb,d text,c text,v bigint)
RETURNS jsonb LANGUAGE sql VOLATILE SECURITY DEFINER SET search_path=pg_catalog
AS $puente$
    SELECT vec_bolsa_registro_accesos.registrar_consulta_configuracion_correo_admin_v1(a,d,c,v)
$puente$;
ALTER FUNCTION vec_prueba_t13_consulta_admin.puente(jsonb,text,text,bigint)
    OWNER TO vec_administracion_propietario;
REVOKE ALL ON SCHEMA vec_prueba_t13_consulta_admin FROM PUBLIC;
REVOKE ALL ON FUNCTION vec_prueba_t13_consulta_admin.puente(jsonb,text,text,bigint) FROM PUBLIC;
GRANT USAGE ON SCHEMA vec_prueba_t13_consulta_admin TO vec_administracion_ejecutor;
GRANT EXECUTE ON FUNCTION vec_prueba_t13_consulta_admin.puente(jsonb,text,text,bigint)
...
DECLARE
    entrada jsonb := jsonb_build_object(
        ...
        'action','administracion.configuracion_correo.consultar',
        'module_id','vec.module.administracion',
        'subject_ref','configuracion:smtp:diputacion',
        'result','permitido','correlation_ref','correlacion_'||repeat('b',32),
        'occurred_at',to_char(transaction_timestamp() AT TIME ZONE 'UTC',
                              'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'));
    primera jsonb; repetida jsonb; invalida jsonb; clave text;
BEGIN
    BEGIN
        PERFORM vec_bolsa_registro_accesos.registrar_consulta_configuracion_correo_admin_v1(
            entrada,'decision_sintetica','consumo_sintetico',0);
        RAISE EXCEPTION 'ejecutor llamó T13 directamente';
    EXCEPTION WHEN insufficient_privilege THEN NULL;
    END;
    primera := vec_prueba_t13_consulta_admin.puente(entrada,'decision_sintetica','consumo_sintetico',0);
    repetida := vec_prueba_t13_consulta_admin.puente(entrada,'decision_sintetica','consumo_sintetico',0);
    IF primera IS DISTINCT FROM repetida
       ...
       OR primera->>'object_version' IS DISTINCT FROM '0'
    ...
    END IF;
    repetida := vec_prueba_t13_consulta_admin.puente(
        entrada||jsonb_build_object('correlation_ref','correlacion_'||repeat('c',32)),
        'decision_presente_sintetica','consumo_presente_sintetico',7);
    IF repetida->>'object_version' IS DISTINCT FROM '7' THEN
        RAISE EXCEPTION 'T13 no conservó version observada positiva';
    END IF;
    BEGIN
        PERFORM vec_prueba_t13_consulta_admin.puente(entrada,'decision_sintetica','consumo_sintetico',-1);
        RAISE EXCEPTION 'version observada negativa aceptada';
    EXCEPTION WHEN invalid_parameter_value THEN NULL;
    END;
    FOR clave IN SELECT jsonb_object_keys(entrada) LOOP
        BEGIN
            PERFORM vec_prueba_t13_consulta_admin.puente(entrada-clave,'decision_sintetica','consumo_sintetico',0);
            RAISE EXCEPTION 'campo obligatorio ausente aceptado: %',clave;
        EXCEPTION WHEN invalid_parameter_value THEN NULL;
        END;
        BEGIN
            PERFORM vec_prueba_t13_consulta_admin.puente(jsonb_set(entrada,ARRAY[clave],'null'::jsonb),
                                               'decision_sintetica','consumo_sintetico',0);
    ...
    FOR invalida IN SELECT v FROM (VALUES
        ...
        BEGIN
            PERFORM vec_prueba_t13_consulta_admin.puente(invalida,'decision_sintetica','consumo_sintetico',0);
            RAISE EXCEPTION 'auditoria invalida aceptada';
        EXCEPTION WHEN invalid_parameter_value THEN NULL;
        END;
    END LOOP;
    BEGIN
        PERFORM vec_prueba_t13_consulta_admin.puente(entrada,NULL,'consumo_sintetico',0);
        RAISE EXCEPTION 'decision ausente aceptada';
    EXCEPTION WHEN invalid_parameter_value THEN NULL;
    END;
    BEGIN
        PERFORM vec_prueba_t13_consulta_admin.puente(entrada,'decision_sintetica',NULL,0);
        RAISE EXCEPTION 'consumo ausente aceptado';
    EXCEPTION WHEN invalid_parameter_value THEN NULL;
    END;
    BEGIN
        PERFORM vec_prueba_t13_consulta_admin.puente(entrada,'decision_sintetica','consumo_sintetico',NULL);
        RAISE EXCEPTION 'version ausente aceptada';
    EXCEPTION WHEN invalid_parameter_value THEN NULL;
    END;
    BEGIN
        PERFORM vec_prueba_t13_consulta_admin.puente(entrada,'otra_decision','consumo_sintetico',0);
...
\if :rollback_intacto
\else
    \echo 'T13/3: ROLLBACK dejó efectos'
    SELECT 1/0;
\endif
-- Pendiente E2E real: emitir material V3 nominal de consulta, ejecutar ADMIN2
-- con configuración ausente (versión0) y presente (versiónreal); comprobar
-- campos permitidos, consumo único y rollback del consumo ante fallo T13.
-- Este archivo sólo comprueba mecánica T13; no simula esa autoridad.
