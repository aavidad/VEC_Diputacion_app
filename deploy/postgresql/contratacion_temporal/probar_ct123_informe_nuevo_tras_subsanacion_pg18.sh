...
# idempotencia (dobles explícitos de las fachadas AD3), reinicio y DOWN
...
[[ $(escalar "$funciones") == 0 && $(escalar "$restriccion") == 'CHECK ((version_expediente = (4)::numeric))' ]] || { echo 'FALLO: ROLLBACK dejó rastro' >&2; exit 1; }; ok 'ROLLBACK sin rastro'
run <"$m.up.sql"; [[ $(escalar "$funciones") == 4 ]] || { echo 'FALLO: UP incompleto' >&2; exit 1; }; ok 'UP'
falla_con "$m.up.sql" 'CT123 ya instalada'; ok 'doble UP rechazado'
run <"$m.down.sql"; [[ $(escalar "$funciones") == 0 && $(escalar "$restriccion") == 'CHECK ((version_expediente = (4)::numeric))' ]] || { echo 'FALLO: DOWN incompleto' >&2; exit 1; }; ok 'DOWN sin historia'
...
[[ $acl == f ]] || { echo 'FALLO: ACL de CT123' >&2; exit 1; }; ok 'ACL con roles reales'
...
[[ $tras == '9/fiscalizacion/informe:ct123:nuevo/confirmada' ]] || { echo "FALLO: estado tras reinicio: $tras" >&2; exit 1; }; ok 'historia conservada tras reinicio'
