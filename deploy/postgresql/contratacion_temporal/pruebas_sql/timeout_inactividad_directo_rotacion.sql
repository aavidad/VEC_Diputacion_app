...
SELECT count(*) AS replay_confirmado
FROM vec_contratacion_temporal.preparar_alta_v2(
    pg_catalog.jsonb_build_object(
        ...
        'reserva_ref_candidata', 'reserva:idle-no-debe-ganar',
        'referencias_candidatas',
        pg_catalog.jsonb_build_object(
            'expediente_ref', 'expediente:idle-no-debe-ganar',
            'numero_visible', '2026/IDLE-NO-GANA',
            'recibo_ref', 'recibo:idle-no-debe-ganar'
        )
    )
);

\! sleep 2
SELECT 1 AS conexion_que_debio_terminar;
COMMIT;
