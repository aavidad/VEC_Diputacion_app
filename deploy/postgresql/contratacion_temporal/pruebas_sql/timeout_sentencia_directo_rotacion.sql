...
FROM vec_contratacion_temporal.preparar_alta_v2(
    pg_catalog.jsonb_build_object(
        ...
        'reserva_ref_candidata', 'reserva:timeout-no-debe-ganar',
        'referencias_candidatas',
        pg_catalog.jsonb_build_object(
            'expediente_ref', 'expediente:timeout-no-debe-ganar',
            'numero_visible', '2026/TIME-NO-GANA',
            'recibo_ref', 'recibo:timeout-no-debe-ganar'
        )
    )
);
