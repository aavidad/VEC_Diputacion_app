-- CT-LITE-O6-REM-02: auxiliares privados del canon Go cerrado y fencing v2.
...
CREATE FUNCTION vec_contratacion_temporal.nuevo_token_fencing_seleccion_llamamiento_o6_v2()
RETURNS text LANGUAGE sql VOLATILE SECURITY INVOKER
SET search_path = pg_catalog
AS $funcion$
    SELECT 'reserva:seleccion-llamamiento:v2:' || pg_catalog.encode(
        pg_catalog.sha256(
...
BEGIN
    IF pg_catalog.jsonb_typeof(p_solicitud) IS DISTINCT FROM 'object'
       OR NOT (p_solicitud ?& ARRAY[
           'clave_idempotencia','huella_semantica','organizacion_ref','expediente_ref',
           'version_expediente','correlacion_ref','autoridad_solicitante',
           'autorizacion_consulta','accion_consulta','recurso_consulta','accion_orden',
           'finalidad','necesidad','bolsa','politica','maximo_posiciones','cantidad_disponible'
       ])
       OR p_solicitud - ARRAY[
           'clave_idempotencia','huella_semantica','organizacion_ref','expediente_ref',
           'version_expediente','correlacion_ref','autoridad_solicitante',
           'autorizacion_consulta','accion_consulta','recurso_consulta','accion_orden',
           'finalidad','necesidad','bolsa','politica','maximo_posiciones','cantidad_disponible'
       ] <> '{}'::jsonb
       OR p_solicitud->>'clave_idempotencia' !~
          '^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
       OR p_solicitud->>'huella_semantica' !~ '^[0-9a-f]{64}$'
       OR p_solicitud->>'huella_semantica' = pg_catalog.repeat('0', 64)
       OR EXISTS (
           SELECT 1 FROM (VALUES
               (p_solicitud->'organizacion_ref'), (p_solicitud->'expediente_ref'),
               (p_solicitud->'correlacion_ref'), (p_solicitud->'autoridad_solicitante')
           ) referencias(valor)
    ...
    FOR v_referencia IN
        SELECT valor FROM (VALUES
            (p_solicitud->'autorizacion_consulta'), (p_solicitud->'accion_consulta'),
            (p_solicitud->'recurso_consulta'), (p_solicitud->'accion_orden'),
            (p_solicitud->'finalidad'),
    ...
    LOOP
        IF pg_catalog.jsonb_typeof(v_referencia.valor) IS DISTINCT FROM 'object'
           ...
           OR NOT v_referencia.valor ?& ARRAY['referencia','version','huella_sha256']
           OR v_referencia.valor - ARRAY['referencia','version','huella_sha256'] <> '{}'::jsonb
           OR v_referencia.valor->>'referencia' !~ '^[A-Za-z0-9][A-Za-z0-9._:/#-]{2,159}$'
           OR v_referencia.valor->>'version' !~ '^[1-9][0-9]*$'
           OR (v_referencia.valor->>'version')::numeric > 9007199254740991
    ...
    v_canon := v_canon ||
        vec_contratacion_temporal.campo_canonico_seleccion_llamamiento_o6_v1(
            'autoridad_solicitante', p_solicitud->>'autoridad_solicitante');

    FOR v_referencia IN
        SELECT prefijo, valor FROM (VALUES
            (1, 'autorizacion_consulta', p_solicitud->'autorizacion_consulta'),
            (2, 'accion_consulta', p_solicitud->'accion_consulta'),
            (3, 'recurso_consulta', p_solicitud->'recurso_consulta'),
            (4, 'accion_orden', p_solicitud->'accion_orden'),
            (5, 'recurso', p_solicitud->'bolsa'),
            (6, 'finalidad', p_solicitud->'finalidad'),
            (7, 'necesidad', p_solicitud->'necesidad'),
            (8, 'bolsa', p_solicitud->'bolsa'),
            (9, 'politica', p_solicitud->'politica')
    ...
    END LOOP;
    IF p_solicitud->'recurso_consulta' IS DISTINCT FROM p_solicitud->'necesidad'
       OR (p_solicitud->>'version_expediente')::numeric > 9007199254740991
       OR (p_solicitud->>'maximo_posiciones')::numeric > 250000
       OR (p_solicitud->>'cantidad_disponible')::numeric >
...
AS $funcion$
    SELECT '{"clave_idempotencia":' || (p_solicitud->'clave_idempotencia')::text ||
        ...
        ',"autoridad_solicitante":' || (p_solicitud->'autoridad_solicitante')::text ||
        ',"autorizacion_consulta":' || vec_contratacion_temporal.referencia_json_seleccion_llamamiento_o6_v1(p_solicitud->'autorizacion_consulta') ||
        ',"accion_consulta":' || vec_contratacion_temporal.referencia_json_seleccion_llamamiento_o6_v1(p_solicitud->'accion_consulta') ||
        ',"recurso_consulta":' || vec_contratacion_temporal.referencia_json_seleccion_llamamiento_o6_v1(p_solicitud->'recurso_consulta') ||
