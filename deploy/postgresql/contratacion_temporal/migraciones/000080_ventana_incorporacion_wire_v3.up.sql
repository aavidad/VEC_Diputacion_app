...
-- CT80 UP: decisión V3 fija6 / capacidad RFC3339Nano, nunca recanoniza autoridad.
...
DECLARE
 ...
 origen text:=$origen$ FOREACH s IN ARRAY ARRAY[c->>'emitida_en',c->>'expira_en',d->>'emitida_en',d->>'valida_hasta',c->>'configuracion_expira_en',c->>'raiz_valida_hasta'] LOOP
 ...
 END LOOP;
$origen$;
 destino text:=$destino$ FOREACH s IN ARRAY ARRAY[c->>'emitida_en',c->>'expira_en',c->>'configuracion_expira_en',c->>'raiz_valida_hasta'] LOOP
  IF vec_contratacion_temporal.instante_incorporacion_go_v2(s,false) IS NOT TRUE THEN
   RAISE EXCEPTION 'CT75: ventana inválida' USING ERRCODE='42501'; END IF;
 END LOOP;
...
BEGIN
 ...
 IF octet_length(convert_to(p.prosrc,'UTF8'))<>931
 OR encode(sha256(convert_to(p.prosrc,'UTF8')),'hex') IS DISTINCT FROM '1fbddde59729e8cf05f469b7b25e322ea79e7eceae34c3bedb47387b9cd2c21f'
 ...
 OR (SELECT octet_length(convert_to(prosrc,'UTF8')) FROM pg_proc WHERE oid=f)<>1135
 OR (SELECT encode(sha256(convert_to(prosrc,'UTF8')),'hex') FROM pg_proc WHERE oid=f) IS DISTINCT FROM '9474153d1023ca3617e551398d3862191b91ae56aa818dc9fbf090cda7aba059'
 ...
 EXECUTE format('COMMENT ON FUNCTION %s IS %L',f::regprocedure,'CT80:ventana-wire-v3:fija6-nano');
 IF obj_description(f,'pg_proc') IS DISTINCT FROM 'CT80:ventana-wire-v3:fija6-nano' THEN
