    ...
    IF pg_catalog.to_regprocedure(
       ...
       OR pg_catalog.to_regprocedure(
        ...
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'compatibilidad O2-07 de análisis no instalada';
    END IF;
END
$prevalidacion$;

DO $restaurar_salida_reserva$
DECLARE
    v_definicion text;
...
BEGIN
    ...
    IF pg_catalog.length(v_definicion) - pg_catalog.length(
           pg_catalog.replace(v_definicion, v_nueva, '')
       ) <> pg_catalog.length(v_nueva)
       OR pg_catalog.strpos(v_definicion, v_anterior) <> 0 THEN
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'salida compatible de reserva no es reversible';
    END IF;
    EXECUTE pg_catalog.replace(v_definicion, v_nueva, v_anterior);
END
$restaurar_salida_reserva$;

DO $restaurar_normalizador$
DECLARE
    v_definicion text;
...
BEGIN
    ...
    IF pg_catalog.length(v_definicion) - pg_catalog.length(
           pg_catalog.replace(v_definicion, v_bloque, '')
       ) <> pg_catalog.length(v_bloque) THEN
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'normalizador compatible de análisis no es reversible';
    END IF;
    EXECUTE pg_catalog.replace(v_definicion, v_bloque, '');
END
$restaurar_normalizador$;
...
BEGIN
    ...
    IF v_normalizada #>> '{solicitud,rc,fecha}' <> ''
       ...
       OR v_normalizada #>> '{solicitud,periodo,inicio}' <> '2027-01-01'
       OR v_normalizada #>> '{solicitud,periodo,fin}' <> '2027-03-31' THEN
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'retirada de compatibilidad de análisis no acreditada';
    END IF;
END
$postvalidacion$;
