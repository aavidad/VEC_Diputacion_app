...
-- No retirar una resolución del sucesor ni dejarla sin lectura nominal.
LOCK TABLE vec_contratacion_temporal.resolucion_manual_respuesta_rrhh IN ACCESS EXCLUSIVE MODE;
DO $resolucion_sucesor_reversa$
DECLARE
...
BEGIN
    IF EXISTS (
        ...
        WHERE (c.material_json->'solicitud') ? 'TipoAntecedente'
    ) THEN
        RAISE EXCEPTION 'se conserva historia de resolución del sucesor' USING ERRCODE='55000';
    END IF;
    FOR v_objeto IN SELECT * FROM (VALUES
        ('consultar_justificante_respuesta_recibida_rrhh_v1',true,'64ab2899de68d5eefff5fc24dc71a81b0a60d61ccd16290aa3aa1f0baff438bd','637ade035c0d181b38681a26c51a8b49269e0fdbb35c083e9f8b47b17dc80b0b'),
        ('registrar_resolucion_manual_respuesta_rrhh_v1',false,'3e27f8a9a1dfcec701db5721e7cc679955bbd6bdd921e3cc402e2110ce750274','786607bb25d814189290295b704c682e085b67f93bd0af78e0a9dfc8144efd13')
    ...
    LOOP
        ...
        FOR v_fragmento IN SELECT nuevo AS anterior,anterior AS nuevo FROM (VALUES
