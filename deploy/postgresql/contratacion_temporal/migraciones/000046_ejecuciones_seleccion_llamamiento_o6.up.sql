...
BEGIN
    ...
    SELECT ejecucion.situacion, ejecucion.solicitud_json::text, ''::text,
           ...
           pg_catalog.coalesce(ejecucion.artefacto_canonico, '')
...
BEGIN
    ...
    IF v_ejecucion.huella_semantica IS DISTINCT FROM p_huella
       OR v_ejecucion.solicitud_json IS DISTINCT FROM p_solicitud
       OR v_ejecucion.reserva_ref IS DISTINCT FROM p_reserva THEN
    ...
    IF v_ejecucion.situacion = 'confirmada'
       ...
       AND v_ejecucion.artefacto_canonico = p_artefacto THEN
        RETURN true;
    END IF;
    IF v_ejecucion.situacion <> 'propietaria'
