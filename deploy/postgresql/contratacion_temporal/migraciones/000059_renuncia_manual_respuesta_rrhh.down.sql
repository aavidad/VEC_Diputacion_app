...
-- Con solo aceptaciones se recupera exactamente la función de CT58.
-- Ninguna fila, recibo ni consumo se elimina. Sin CASCADE.
DO $cambio$
...
BEGIN
    IF EXISTS (SELECT 1 FROM vec_contratacion_temporal.resolucion_manual_respuesta_rrhh
        WHERE solicitud_json->>'Respuesta'='renuncia'
           OR comando_siguiente_ref IS NOT NULL OR comando_siguiente_json IS NOT NULL) THEN
        RAISE EXCEPTION 'reversión denegada: se conserva renuncia, intención e historia' USING ERRCODE='55000';
    END IF;
...
ALTER TABLE vec_contratacion_temporal.resolucion_manual_respuesta_rrhh
    ...
    DROP CONSTRAINT resolucion_manual_comando_siguiente_unico,
    DROP COLUMN comando_siguiente_json,
    DROP COLUMN comando_siguiente_ref;
COMMIT;
