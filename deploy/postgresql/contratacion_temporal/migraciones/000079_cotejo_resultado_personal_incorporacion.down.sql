...
DECLARE
 ...
 origen text:=$origen$'resultado',(resultado_personal - 'motivo_rechazo'),'material_canonico'$origen$;
 destino text:=$destino$'resultado',resultado_personal,'material_canonico'$destino$;
BEGIN
 ...
 -- No vuelve al cotejo anterior una vez que existe historia CT.
 -- No borra filas, no altera consumos ni limpia artefactos para forzar la retirada.
 ...
 OR EXISTS (SELECT 1 FROM vec_contratacion_temporal.incorporacion_outbox_v2) THEN
  RAISE EXCEPTION 'CT79: historia CT conservada' USING ERRCODE='55000'; END IF;
