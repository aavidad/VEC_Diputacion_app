...
DO $propuesta_sucesor_reversa$
DECLARE
    v_antes record; v_despues record; v_fragmento record; v_definicion text;
BEGIN
    IF EXISTS (
        SELECT 1 FROM vec_contratacion_temporal.propuesta_formalizacion p
        JOIN vec_contratacion_temporal.resolucion_manual_respuesta_rrhh r USING(resolucion_ref)
        ...
        RAISE EXCEPTION 'se conserva propuesta del sucesor y su recuperación' USING ERRCODE='55000';
    END IF;
    ...
    IF NOT FOUND OR encode(sha256(convert_to(v_antes.cuerpo,'UTF8')),'hex')
       IS DISTINCT FROM 'f3e681486de901b464497af49737e72487329f5014322857c8845f5f782135bf' THEN
    ...
    IF encode(sha256(convert_to(v_despues.cuerpo,'UTF8')),'hex')
       IS DISTINCT FROM 'db71857357fccb4e1be745b4fe7f7fb03b325837dc54b48741cc475c2ac143cb'
