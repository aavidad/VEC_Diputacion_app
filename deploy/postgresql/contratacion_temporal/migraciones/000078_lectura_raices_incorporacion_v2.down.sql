...
DO $guardas$
DECLARE f oid:=to_regprocedure('vec_contratacion_temporal.leer_raices_incorporacion_ejercicio_v2(text,text,text)');
 r oid:='vec_contratacion_temporal_lector_raices_historicas'::regrole;
...
BEGIN
 ...
 OR f IS NULL OR (SELECT count(*) FROM pg_proc WHERE pronamespace='vec_contratacion_temporal'::regnamespace
   AND proname='leer_raices_incorporacion_ejercicio_v2')<>1
 OR NOT EXISTS(SELECT 1 FROM pg_proc p WHERE p.oid=f
   ...
   AND p.proallargtypes=ARRAY[25,25,25,25,25,25,25,25,25,25,25,3802,17,25,3802,17]::oid[]
   AND p.proargmodes=ARRAY['i','i','i','t','t','t','t','t','t','t','t','t','t','t','t','t']::"char"[]
   AND p.proargnames=ARRAY['p_organizacion_ref','p_expediente_ref','p_relacion_ref',
    'seguimiento_ref','organizacion_ref','expediente_ref','relacion_ref','version_expediente_observada',
    'definicion_ref','definicion_version','definicion_sha256','publicacion_json','raiz_canonica','raiz_sha256',
    'estado_inicial_json','estado_inicial_canonico']
   AND p.proconfig=ARRAY['search_path=pg_catalog','row_security=on','statement_timeout=5s','lock_timeout=2s']
   AND octet_length(p.prosrc)=19563
   AND encode(sha256(convert_to(p.prosrc,'UTF8')),'hex')='27bdd1f5f03621744f3f8721b6eaa87b199d2f92f9a0e4ff42cbfdc14a7d5c60')
 ...
 OR EXISTS(SELECT 1 FROM pg_proc p WHERE p.oid<>f
   AND position('leer_raices_incorporacion_ejercicio_v2' IN p.prosrc)>0)
 THEN RAISE EXCEPTION 'lector historico tiene consumidores' USING ERRCODE='55000'; END IF;
END $guardas$;
DROP FUNCTION vec_contratacion_temporal.leer_raices_incorporacion_ejercicio_v2(text,text,text) RESTRICT;
REVOKE USAGE ON SCHEMA vec_contratacion_temporal FROM vec_contratacion_temporal_lector_raices_historicas RESTRICT;
-- Sin borrar historia ni cambiar codecs, escritor CT75/76 o roles ejecutores.
-- Tras retirar LOGIN/membresias de ensayo, usar roles_raices_historicas_down.sql.
COMMIT;
