...
DECLARE
 ...
 origen text:=$origen$lectura:=vec_personal.acreditar_alta_ejercicio_v1(preparacion->>'OrganizacionRef',s->>'solicitud_ref',s->>'expediente_ref',(s->>'version_expediente')::bigint,
   ...
   lector_capacidad,lector_decision,lector_motivo,lector_contexto,lector_persona_version,lector_perfil_version,lector_payload,lector_sobre,lector_evidencia,lector_raiz);$origen$;
 destino text:=$destino$lectura:=vec_personal.acreditar_alta_ejercicio_v2(preparacion->>'OrganizacionRef',s->>'solicitud_ref',s->>'expediente_ref',(s->>'version_expediente')::bigint,
   resultado_personal->>'resultado_ref',resultado_personal->>'recibo_ref',resultado_personal->>'relacion_ref',resultado_personal->>'ocupacion_ref',original_personal->>'MaterialSHA256',
   preparacion->>'UnidadRef',lector_capacidad,lector_decision,lector_motivo,lector_contexto,lector_persona_version,lector_perfil_version,lector_payload,lector_sobre,lector_evidencia,lector_raiz);$destino$;
BEGIN
 ...
 IF octet_length(convert_to(p.prosrc,'UTF8'))<>23351
 OR encode(sha256(convert_to(p.prosrc,'UTF8')),'hex') IS DISTINCT FROM 'c54a9f6fca96cd8762533839e98d7ead3d011d9497793a4fcb7123f4edc8a961'
 ...
 OR obj_description(f,'pg_proc') IS DISTINCT FROM 'CT75:registro-incorporacion-v2'
 ...
 OR (SELECT octet_length(convert_to(prosrc,'UTF8')) FROM pg_proc WHERE oid=f)<>23377
 OR (SELECT encode(sha256(convert_to(prosrc,'UTF8')),'hex') FROM pg_proc WHERE oid=f) IS DISTINCT FROM '6b376e406f16c60b73d19c9854d7ab142fd452c2c74f2e8b88b64c327cbaef25'
 ...
 EXECUTE format('COMMENT ON FUNCTION %s IS %L',f::regprocedure,'CT76:lector-incorporacion-v2:sobre-CT75');
 IF obj_description(f,'pg_proc') IS DISTINCT FROM 'CT76:lector-incorporacion-v2:sobre-CT75' THEN
