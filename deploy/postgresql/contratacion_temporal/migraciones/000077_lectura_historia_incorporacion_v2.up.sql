...
DO $pre$
DECLARE r oid:='vec_contratacion_temporal_lector_historia_incorporacion'::regrole;
...
BEGIN
 ...
 OR EXISTS(SELECT 1 FROM pg_proc WHERE pronamespace='vec_contratacion_temporal'::regnamespace
  AND proname='leer_historia_incorporacion_original_v2')
 ...
 THEN RAISE EXCEPTION 'CT77: precondiciones incompatibles' USING ERRCODE='55000'; END IF;
 FOREACH n IN ARRAY ARRAY['incorporacion_registro_v2','incorporacion_auditoria_v2','incorporacion_outbox_v2',
   'seguimiento_raiz_v2','seguimiento_definicion_v2','seguimiento_estado_v2'] LOOP
  ...
  THEN RAISE EXCEPTION 'CT77: fuente propietaria incompatible' USING ERRCODE='55000'; END IF;
 END LOOP;
 FOREACH n IN ARRAY ARRAY[
  'material_incorporacion_ejercicio_canonico_v2(jsonb)',
  ...
  THEN RAISE EXCEPTION 'CT77: codec propietario requerido' USING ERRCODE='55000'; END IF;
 END LOOP;
END $pre$;

CREATE FUNCTION vec_contratacion_temporal.leer_historia_incorporacion_original_v2(
 p_recibo_ref text, p_material_sha256 text, p_intencion_sha256 text
) RETURNS jsonb
...
BEGIN
    ...
    SELECT oid, rolsuper, rolinherit, rolcreaterole, rolcreatedb, rolcanlogin,
           rolreplication, rolbypassrls, rolconfig
      INTO grupo FROM pg_catalog.pg_roles WHERE rolname = 'vec_contratacion_temporal_lector_historia_incorporacion';
    ...
    funciones := ARRAY[pg_catalog.to_regprocedure('vec_contratacion_temporal.leer_historia_incorporacion_original_v2(text,text,text)')];
    ...
    IF login_oid IS NULL OR runtime_oid IS NULL OR esquema_oid IS NULL OR base_oid IS NULL
       ...
       OR EXISTS (
           SELECT 1 FROM pg_catalog.pg_roles ct77_catalogo_r
            WHERE ct77_catalogo_r.oid<>login_oid
              AND pg_catalog.pg_has_role(login_oid,ct77_catalogo_r.oid,'MEMBER')
              AND ct77_catalogo_r.oid<>runtime_oid
       )
       OR EXISTS (
           SELECT 1 FROM pg_catalog.pg_roles ct77_catalogo_r
            WHERE ct77_catalogo_r.oid<>runtime_oid
              AND pg_catalog.pg_has_role(runtime_oid,ct77_catalogo_r.oid,'MEMBER')
       )
       OR NOT EXISTS (
       ...
       OR EXISTS (
           SELECT 1 FROM pg_catalog.pg_db_role_setting ct77_catalogo_s
            WHERE ct77_catalogo_s.setrole IN (login_oid,runtime_oid)
       )
       OR EXISTS (
           SELECT 1 FROM pg_catalog.pg_default_acl d
           LEFT JOIN LATERAL pg_catalog.aclexplode(
             coalesce(d.defaclacl,'{}'::aclitem[])
           ) ct77_catalogo_a ON true
            WHERE d.defaclrole IN (login_oid,runtime_oid)
               OR ct77_catalogo_a.grantee IN (login_oid,runtime_oid)
               OR ct77_catalogo_a.grantor IN (login_oid,runtime_oid)
       )
       OR EXISTS (
           SELECT 1 FROM pg_catalog.pg_policy ct77_catalogo_p
            WHERE login_oid=ANY(ct77_catalogo_p.polroles) OR runtime_oid=ANY(ct77_catalogo_p.polroles)
       )
       OR EXISTS (
       ...
       OR NOT COALESCE((
           SELECT count(*)=1 AND bool_and(ct77_catalogo_a.privilege_type='CONNECT' AND NOT ct77_catalogo_a.is_grantable AND ct77_catalogo_a.grantor=b.datdba)
             FROM pg_catalog.pg_database b
             CROSS JOIN LATERAL pg_catalog.aclexplode(
               coalesce(b.datacl,pg_catalog.acldefault('d',b.datdba))
             ) ct77_catalogo_a
            WHERE b.oid=base_oid AND ct77_catalogo_a.grantee=runtime_oid
       ),false)
       OR NOT COALESCE((
           SELECT count(*)=1 AND bool_and(ct77_catalogo_a.privilege_type='USAGE' AND NOT ct77_catalogo_a.is_grantable AND ct77_catalogo_a.grantor=n.nspowner)
             FROM pg_catalog.pg_namespace n
             CROSS JOIN LATERAL pg_catalog.aclexplode(
               coalesce(n.nspacl,pg_catalog.acldefault('n',n.nspowner))
             ) ct77_catalogo_a
            WHERE n.oid=esquema_oid AND ct77_catalogo_a.grantee=runtime_oid
       ),false)
       OR NOT COALESCE((
           SELECT count(*)=1 AND count(DISTINCT ct77_catalogo_p.oid)=1
                  AND bool_and(ct77_catalogo_a.privilege_type='EXECUTE' AND NOT ct77_catalogo_a.is_grantable AND ct77_catalogo_a.grantor=ct77_catalogo_p.proowner)
             FROM pg_catalog.pg_proc ct77_catalogo_p
             CROSS JOIN LATERAL pg_catalog.aclexplode(
               coalesce(ct77_catalogo_p.proacl,pg_catalog.acldefault('f',ct77_catalogo_p.proowner))
            ) ct77_catalogo_a
            WHERE ct77_catalogo_p.oid=ANY(funciones) AND ct77_catalogo_a.grantee=runtime_oid
       ),false)
       ...
       AND NOT EXISTS (
         SELECT 1 FROM pg_catalog.pg_proc ct77_catalogo_p
         JOIN pg_catalog.pg_namespace n ON n.oid=ct77_catalogo_p.pronamespace
          WHERE n.nspname <> 'information_schema' AND n.nspname !~ '^pg_'
            AND pg_catalog.has_schema_privilege(login_oid,n.oid,'USAGE')
            AND ct77_catalogo_p.oid <> ALL(funciones)
            AND pg_catalog.has_function_privilege(login_oid,ct77_catalogo_p.oid,'EXECUTE')
       )
       AND NOT EXISTS (
       ...
       AND NOT EXISTS (
         SELECT 1 FROM pg_catalog.pg_foreign_data_wrapper ct77_catalogo_f
          WHERE pg_catalog.has_foreign_data_wrapper_privilege(login_oid,ct77_catalogo_f.oid,'USAGE')
       )
       AND NOT EXISTS (
         SELECT 1 FROM pg_catalog.pg_foreign_server ct77_catalogo_s
          WHERE pg_catalog.has_server_privilege(login_oid,ct77_catalogo_s.oid,'USAGE')
       )
       AND NOT EXISTS (
       ...
       AND NOT EXISTS (
         SELECT 1 FROM pg_catalog.pg_parameter_acl ct77_catalogo_a
          WHERE pg_catalog.has_parameter_privilege(login_oid,ct77_catalogo_a.parname,'SET')
             OR pg_catalog.has_parameter_privilege(login_oid,ct77_catalogo_a.parname,'ALTER SYSTEM')
       )) IS NOT TRUE THEN
        RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'LOGIN lector historico no acreditado';
    END IF;
 ...
 PERFORM pg_advisory_xact_lock_shared(hashtextextended('vec_contratacion_temporal:000077:lectura_historia:v2',0));
 IF p_recibo_ref IS NULL OR octet_length(p_recibo_ref)<>68
 OR p_recibo_ref !~ '^ref:[0-9a-f]{64}$' OR p_recibo_ref='ref:'||repeat('0',64)
 OR p_material_sha256 IS NULL OR octet_length(p_material_sha256)<>64
 OR p_material_sha256 !~ '^[0-9a-f]{64}$' OR p_material_sha256=repeat('0',64)
 OR p_intencion_sha256 IS NULL OR octet_length(p_intencion_sha256)<>64
 OR p_intencion_sha256 !~ '^[0-9a-f]{64}$' OR p_intencion_sha256=repeat('0',64)
 THEN RAISE EXCEPTION 'CT77: selector invalido' USING ERRCODE='22023'; END IF;

 -- Solo filas originales por PK/FK. Ningun puntero actual ni bloqueo de fila RO.
 SELECT q.* INTO STRICT r FROM vec_contratacion_temporal.incorporacion_registro_v2 q
  WHERE q.recibo_ref=p_recibo_ref AND q.material_sha256=p_material_sha256 AND q.intencion_sha256=p_intencion_sha256;
 ...
 THEN RAISE EXCEPTION 'CT77: registro incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'CT77: historia excede limite operativo' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'CT77: material o fuente cruzados' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'CT77: estados cruzados' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'CT77: canon historico incompatible' USING ERRCODE='55000'; END IF;
 ...
 THEN RAISE EXCEPTION 'CT77: evidencia historica incompatible' USING ERRCODE='55000'; END IF;
 FOREACH k IN ARRAY ARRAY['HuellaExportacionCT','ConsumoCTSHA256','ConsumoLecturaSHA256'] LOOP
  ...
  THEN RAISE EXCEPTION 'CT77: consumo historico incompatible' USING ERRCODE='55000'; END IF;
 END LOOP;
 FOREACH k IN ARRAY ARRAY['DecisionCTRef','DecisionLecturaRef','AuditoriaPersonalLecturaRef'] LOOP
  ...
  THEN RAISE EXCEPTION 'CT77: referencia historica incompatible' USING ERRCODE='55000'; END IF;
 END LOOP;
 ...
 THEN RAISE EXCEPTION 'CT77: registro durable incoherente' USING ERRCODE='55000'; END IF;
 resultado:=jsonb_build_object('Esquema','vec.contratacion-temporal.incorporacion.ejercicio.historia-original.v2',
  'Evidencia',r.evidencia_orden_json,'Recibo',r.recibo_json,'Publicacion',pub,'Anterior',a.estado_json,'Posterior',p.estado_json);
 IF octet_length(resultado::text)>33554432 OR clock_timestamp()<inicio THEN
  RAISE EXCEPTION 'CT77: retorno no disponible' USING ERRCODE='55000'; END IF;
 RETURN resultado;
EXCEPTION WHEN OTHERS THEN
 codigo:=SQLSTATE;
 IF codigo NOT IN('42501','25000','22023','40001','40P01','55P03') THEN codigo:='55000'; END IF;
 RAISE EXCEPTION 'CT77: historia original no disponible' USING ERRCODE=codigo;
END $historia$;
REVOKE ALL ON FUNCTION vec_contratacion_temporal.leer_historia_incorporacion_original_v2(text,text,text) FROM PUBLIC;
-- Crear objetos solo con defaults owner-only; no limpiar permisos ajenos silenciosamente.
DO $acl$
DECLARE f oid:='vec_contratacion_temporal.leer_historia_incorporacion_original_v2(text,text,text)'::regprocedure;
BEGIN
 ...
 THEN RAISE EXCEPTION 'CT77: defaults incompatibles' USING ERRCODE='55000'; END IF;
END $acl$;
GRANT USAGE ON SCHEMA vec_contratacion_temporal TO vec_contratacion_temporal_lector_historia_incorporacion;
GRANT EXECUTE ON FUNCTION vec_contratacion_temporal.leer_historia_incorporacion_original_v2(text,text,text)
 TO vec_contratacion_temporal_lector_historia_incorporacion;
COMMIT;
