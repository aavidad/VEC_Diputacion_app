...
-- CT-000102 DOWN: revierte observaciones en el detalle de RRHH.
    ...
    IF p_entrada.cobertura_presente THEN
        ...
        ELSE
            IF v_cobertura.procedimiento_ref !~
               ...
               OR v_cobertura.bolsa_ref !~
                  '^$|^[A-Za-z0-9][A-Za-z0-9._:/#-]{2,159}$'
               OR v_total_comprobaciones < 1
               OR v_total_comprobaciones > 32
               OR pg_catalog.array_ndims(
                   v_cobertura.comprobaciones
               ) <> 1
            ...
            FOR v_indice IN 1..v_total_comprobaciones LOOP
                v_comprobacion :=
                    v_cobertura.comprobaciones[v_indice];
...
ALTER TYPE vec_contratacion_temporal.analisis_operativo_rrhh_v1
    DROP ATTRIBUTE IF EXISTS observaciones;
