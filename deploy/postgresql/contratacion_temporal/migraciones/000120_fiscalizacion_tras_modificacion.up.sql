...
-- CT120: fiscalización de un expediente modificado tras el nombramiento
-- (petición RRHH p. 1, punto 3; duda 12). CT116 devuelve el expediente a
-- fiscalización con el análisis y el coste nuevos, pero CT52 (inicial, v5) y
-- CT93 (tras subsanación) no admiten ese antecedente. CT120 añade el tercer
-- origen sin editar ninguno de ellos: mismo material HMAC, misma reserva,
-- mismo recibo y mismo consumidor AD3-10 que CT93, con estas diferencias:
--   * antecedente: fase fiscalización en curso, sin fiscalización vigente,
--     con informe y asignación, cuya última actuación es la modificación de
--     CT116 y cuyo registro durable (modificacion_nombramiento_v1) coincide
--     con la versión vigente;
--   * favorable o favorable con observaciones: vuelve al nombramiento en
--     curso; desfavorable: vuelve a la unidad gestora (subsanación), igual
--     que CT52 y CT93;
--   * el contexto autorizado incluye el recibo de la modificación.
-- Añade también las fachadas v2 de preparación y confirmación que eligen,
-- por la forma del expediente, entre CT93 y CT120 para versiones desde la 7.
BEGIN;
...
DO $prevalidacion$
BEGIN
    IF pg_catalog.to_regprocedure('vec_contratacion_temporal.preparar_fiscalizacion_v2(jsonb)') IS NOT NULL
       OR pg_catalog.to_regprocedure('vec_contratacion_temporal.preparar_fiscalizacion_tras_modificacion_ct120(jsonb)') IS NOT NULL THEN
        RAISE EXCEPTION 'CT120 ya instalada: no se reaplica' USING ERRCODE='55000';
    END IF;
    IF current_user<>'vec_contratacion_temporal_propietario'
       OR pg_catalog.to_regprocedure('vec_contratacion_temporal.preparar_fiscalizacion_tras_subsanacion_v1(jsonb)') IS NULL
       OR pg_catalog.to_regprocedure('vec_contratacion_temporal.confirmar_fiscalizacion_tras_subsanacion_v1(jsonb,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NULL
       OR pg_catalog.to_regclass('vec_contratacion_temporal.modificacion_nombramiento_v1') IS NULL
       OR pg_catalog.to_regprocedure('vec_contratacion_temporal.recibo_fiscalizacion_v1(text)') IS NULL
       OR NOT pg_catalog.has_function_privilege(current_user,
          'vec_autorizacion_atestada_v3.registrar_y_consumir_fiscalizacion_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)','EXECUTE')
       OR NOT EXISTS (SELECT 1 FROM pg_catalog.pg_constraint
            WHERE conrelid='vec_contratacion_temporal.reserva_fiscalizacion'::regclass
              AND conname='reserva_fiscalizacion_version_expediente_check' AND convalidated
              AND pg_catalog.pg_get_constraintdef(oid) LIKE '%version_expediente >= (7)::numeric%')
       OR NOT EXISTS (SELECT 1 FROM pg_catalog.pg_constraint
            WHERE conrelid='vec_contratacion_temporal.retorno_fiscalizacion_unidad'::regclass
              AND conname='retorno_fiscalizacion_unidad_version_expediente_check' AND convalidated
              AND pg_catalog.pg_get_constraintdef(oid) LIKE '%version_expediente >= (8)::numeric%') THEN
        RAISE EXCEPTION 'CT120 requiere CT52, CT93 y CT116 intactas' USING ERRCODE='55000';
    END IF;
END
$prevalidacion$;

-- Forma del antecedente: la modificación de CT116 es la última actuación y el
-- expediente espera su fiscalización.
CREATE FUNCTION vec_contratacion_temporal.es_antecedente_fiscalizacion_modificacion_ct120(p_expediente jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path=pg_catalog AS $funcion$
    SELECT coalesce(pg_catalog.jsonb_typeof(p_expediente)='object'
       AND p_expediente->>'fase_actual'='fiscalizacion'
       AND p_expediente->>'estado_actual'='en_curso'
       AND NOT (p_expediente ? 'fiscalizacion')
       AND p_expediente ? 'asignacion' AND p_expediente ? 'informe_juridico'
       AND pg_catalog.jsonb_typeof(p_expediente->'actuaciones')='array'
       AND (p_expediente->'actuaciones')->-1->>'accion_clave'='contratacion_temporal.expediente.modificar_tras_nombramiento'
       AND (p_expediente->'actuaciones')->-1->>'fase_destino'='fiscalizacion'
       AND (p_expediente->'actuaciones')->-1->>'estado_destino'='en_curso'
       AND coalesce(p_expediente->>'version','') ~ '^[1-9][0-9]{0,15}$'
       AND ((p_expediente->'actuaciones')->-1->>'version_expediente')=(p_expediente->>'version'), false)
$funcion$;
...
BEGIN
    ...
    SELECT m.recibo_ref INTO v_recibo
     ...
     WHERE m.organizacion_ref=p_expediente->>'organizacion_ref'
       AND m.expediente_ref=p_expediente->>'referencia'
       AND m.estado='confirmada' AND m.fase_retorno='fiscalizacion'
...
CREATE FUNCTION vec_contratacion_temporal.preparar_fiscalizacion_tras_modificacion_ct120(
    p_operacion jsonb
)
RETURNS TABLE (
...
BEGIN
    ...
    ELSE
        ...
        IF v_actual.version <> (p_operacion ->> 'version_expediente')::numeric
           OR v_actual.agregado_json ->> 'organizacion_ref' <>
              p_operacion ->> 'organizacion_ref'
           OR NOT vec_contratacion_temporal.es_antecedente_fiscalizacion_modificacion_ct120(v_actual.agregado_json) THEN
    ...
    IF resultado <> 'idempotencia_reutilizada' THEN
        PERFORM vec_contratacion_temporal.antecedente_fiscalizacion_modificacion_ct120(v_reserva.expediente_anterior_json);
    END IF;
...
CREATE FUNCTION vec_contratacion_temporal.confirmar_fiscalizacion_tras_modificacion_ct120(
    p_operacion jsonb,
...
BEGIN
    ...
    v_antecedente := vec_contratacion_temporal.antecedente_fiscalizacion_modificacion_ct120(v_actual.agregado_json);
    ...
    ELSE
        -- Favorable (con o sin observaciones): el expediente modificado
        -- vuelve al nombramiento en curso con el análisis nuevo.
        v_fase_destino := 'nombramiento';
    ...
    v_contexto_canonico := pg_catalog.convert_to(
        ...
        '","informe_juridico_ref":"' || v_informe_ref ||
        '","modificacion_recibo_ref":"' || (v_antecedente->>'modificacion_recibo_ref') ||
...
-- Fachadas v2: una sola entrada para las fiscalizaciones desde la versión 7.
-- Eligen CT120 si el expediente (el de la reserva, en una repetición) es una
-- modificación pendiente de fiscalizar y CT93 en otro caso; cada función
-- elegida conserva todas sus comprobaciones de sesión, material y ACL.
CREATE FUNCTION vec_contratacion_temporal.preparar_fiscalizacion_v2(p_operacion jsonb)
RETURNS TABLE (
    resultado text, expediente_json text, reserva_ref text,
    fiscalizacion_ref text, recibo_ref text, evento_ref text,
    retorno_ref text, ambito_hmac text, huella_peticion_hmac text,
...
AS $funcion$
DECLARE v_anterior jsonb; v_par jsonb;
BEGIN
    IF pg_catalog.jsonb_typeof(p_operacion) IS DISTINCT FROM 'object'
       OR pg_catalog.jsonb_typeof(p_operacion #> '{sellos_hmac,retenidos}') IS DISTINCT FROM 'array' THEN
        RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'preparación de fiscalización no autorizada';
    END IF;
    FOR v_par IN SELECT value FROM pg_catalog.jsonb_array_elements(
        pg_catalog.jsonb_build_array(p_operacion #> '{sellos_hmac,activo}')||(p_operacion #> '{sellos_hmac,retenidos}')) LOOP
        SELECT r.expediente_anterior_json INTO v_anterior
          FROM vec_contratacion_temporal.reserva_fiscalizacion r WHERE r.ambito_hmac = v_par ->> 'ambito_hmac';
        EXIT WHEN FOUND;
    END LOOP;
    IF v_anterior IS NULL THEN
        SELECT v.agregado_json INTO v_anterior
         ...
         WHERE a.expediente_ref = p_operacion ->> 'expediente_ref';
    END IF;
    IF vec_contratacion_temporal.es_antecedente_fiscalizacion_modificacion_ct120(v_anterior) THEN
        RETURN QUERY SELECT * FROM vec_contratacion_temporal.preparar_fiscalizacion_tras_modificacion_ct120(p_operacion);
    ELSE
        RETURN QUERY SELECT * FROM vec_contratacion_temporal.preparar_fiscalizacion_tras_subsanacion_v1(p_operacion);
    END IF;
END
$funcion$;

CREATE FUNCTION vec_contratacion_temporal.confirmar_fiscalizacion_v2(
    p_operacion jsonb,
    p_capacidad bytea, p_decision bytea, p_motivo bytea, p_contexto bytea,
    p_persona_version numeric, p_perfil_version numeric,
    p_payload bytea, p_sobre bytea, p_evidencia bytea, p_raiz bytea
...
AS $funcion$
BEGIN
    IF vec_contratacion_temporal.es_antecedente_fiscalizacion_modificacion_ct120(p_operacion -> 'expediente_anterior') THEN
        RETURN QUERY SELECT * FROM vec_contratacion_temporal.confirmar_fiscalizacion_tras_modificacion_ct120(
            p_operacion, p_capacidad, p_decision, p_motivo, p_contexto, p_persona_version, p_perfil_version,
            p_payload, p_sobre, p_evidencia, p_raiz);
    ELSE
        RETURN QUERY SELECT * FROM vec_contratacion_temporal.confirmar_fiscalizacion_tras_subsanacion_v1(
            p_operacion, p_capacidad, p_decision, p_motivo, p_contexto, p_persona_version, p_perfil_version,
            p_payload, p_sobre, p_evidencia, p_raiz);
    END IF;
END
...
DO $acl$
DECLARE f regprocedure; v record;
    fachadas regprocedure[]:=ARRAY[
      'vec_contratacion_temporal.preparar_fiscalizacion_v2(jsonb)'::regprocedure,
      'vec_contratacion_temporal.confirmar_fiscalizacion_v2(jsonb,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure,
      'vec_contratacion_temporal.preparar_fiscalizacion_tras_modificacion_ct120(jsonb)'::regprocedure,
      'vec_contratacion_temporal.confirmar_fiscalizacion_tras_modificacion_ct120(jsonb,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)'::regprocedure];
    auxiliares regprocedure[]:=ARRAY[
      'vec_contratacion_temporal.es_antecedente_fiscalizacion_modificacion_ct120(jsonb)'::regprocedure,
      'vec_contratacion_temporal.antecedente_fiscalizacion_modificacion_ct120(jsonb)'::regprocedure];
BEGIN
    FOREACH f IN ARRAY fachadas||auxiliares LOOP
        FOR v IN SELECT DISTINCT x.grantee FROM pg_proc p CROSS JOIN LATERAL aclexplode(coalesce(p.proacl,acldefault('f',p.proowner))) x
            ...
            EXECUTE format('REVOKE ALL ON FUNCTION %s FROM %s',f::text,
                CASE WHEN v.grantee=0 THEN 'PUBLIC' ELSE quote_ident(pg_get_userbyid(v.grantee)) END);
        END LOOP;
    END LOOP;
    ...
    IF EXISTS (SELECT 1 FROM pg_proc p CROSS JOIN LATERAL aclexplode(coalesce(p.proacl,acldefault('f',p.proowner))) x
       ...
       OR EXISTS (SELECT 1 FROM unnest(fachadas) x WHERE NOT has_function_privilege('vec_contratacion_temporal_ejecutor',x,'EXECUTE')) THEN
        RAISE EXCEPTION 'CT120: ACL efectiva incompatible' USING ERRCODE='42501';
    END IF;
END
$acl$;
COMMENT ON FUNCTION vec_contratacion_temporal.preparar_fiscalizacion_v2(jsonb) IS
    'CT120: preparación de fiscalización desde v7; elige CT120 (expediente modificado) o CT93 (tras subsanación).';
COMMIT;
