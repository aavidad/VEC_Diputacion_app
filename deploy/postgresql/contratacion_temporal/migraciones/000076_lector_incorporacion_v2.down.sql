...
-- CT76: única sustitución del call Personal; CT75, material e historia intactos.
...
DECLARE
 ...
 origen text:=$origen$lectura:=vec_personal.acreditar_alta_ejercicio_v2(preparacion->>'OrganizacionRef',s->>'solicitud_ref',s->>'expediente_ref',(s->>'version_expediente')::bigint,
   ...
   preparacion->>'UnidadRef',lector_capacidad,lector_decision,lector_motivo,lector_contexto,lector_persona_version,lector_perfil_version,lector_payload,lector_sobre,lector_evidencia,lector_raiz);$origen$;
 destino text:=$destino$lectura:=vec_personal.acreditar_alta_ejercicio_v1(preparacion->>'OrganizacionRef',s->>'solicitud_ref',s->>'expediente_ref',(s->>'version_expediente')::bigint,
   resultado_personal->>'resultado_ref',resultado_personal->>'recibo_ref',resultado_personal->>'relacion_ref',resultado_personal->>'ocupacion_ref',original_personal->>'MaterialSHA256',
   lector_capacidad,lector_decision,lector_motivo,lector_contexto,lector_persona_version,lector_perfil_version,lector_payload,lector_sobre,lector_evidencia,lector_raiz);$destino$;
BEGIN
 ...
 OR f IS NULL THEN RAISE EXCEPTION 'CT76: propietario o firma incompatibles' USING ERRCODE='55000'; END IF;
 ...
 OR (SELECT count(*) FROM pg_proc WHERE pronamespace=p.pronamespace AND proname=p.proname)<>1 THEN
  RAISE EXCEPTION 'CT76: definición previa alterada' USING ERRCODE='55000'; END IF;
 ...
 IS DISTINCT FROM (SELECT array_agg(a::text ORDER BY a::text) FROM unnest(ARRAY[
  ...
  RAISE EXCEPTION 'CT76: ACL previa alterada' USING ERRCODE='42501'; END IF;
 ...
 OR NOT has_function_privilege(current_user,fachada,'EXECUTE') THEN
  RAISE EXCEPTION 'CT76: fachada propietaria V2 requerida' USING ERRCODE='55000'; END IF;
 ...
 IS DISTINCT FROM (SELECT array_agg(a::text ORDER BY a::text) FROM unnest(ARRAY[
  ...
  RAISE EXCEPTION 'CT76: firma, metadata o ACL Personal V2 incompatibles' USING ERRCODE='55000'; END IF;
 ...
 IF (SELECT count(*) FROM pg_class t WHERE t.relnamespace='vec_contratacion_temporal'::regnamespace
  ...
  RAISE EXCEPTION 'CT76: inventario, ACL o RLS de historia alterados' USING ERRCODE='55000'; END IF;
 ...
 OR EXISTS (SELECT 1 FROM pg_proc z WHERE z.oid<>f AND strpos(z.prosrc,'registrar_incorporacion_ejercicio_v2')>0) THEN
  RAISE EXCEPTION 'CT76: dependencia llamadora conservada' USING ERRCODE='55000'; END IF;
 -- Sin reinterpretar consumos/historia V2 como V1: retirar sólo antes de historia CT.
 ...
 OR EXISTS (SELECT 1 FROM vec_contratacion_temporal.incorporacion_outbox_v2) THEN
  RAISE EXCEPTION 'CT76: historia CT conservada' USING ERRCODE='55000'; END IF;
 -- Debe seguir disponible la fachada V1 que CT75 volverá a invocar.
 IF NOT EXISTS (SELECT 1 FROM pg_proc z WHERE z.oid=to_regprocedure('vec_personal.acreditar_alta_ejercicio_v1(text,text,text,bigint,text,text,text,text,text,bytea,bytea,bytea,bytea,bigint,bigint,bytea,bytea,bytea,bytea)')
  AND z.proowner=personal_owner AND z.prosecdef AND z.prokind='f' AND z.provolatile='v' AND z.prorettype='jsonb'::regtype
  AND NOT z.proretset AND z.proconfig @> ARRAY['search_path=pg_catalog','row_security=on']
  AND has_function_privilege(current_user,z.oid,'EXECUTE')) THEN
  RAISE EXCEPTION 'CT76: restauración V1 sin dependencia' USING ERRCODE='55000'; END IF;
 ...
 OR strpos(p.prosrc,destino)>0 THEN RAISE EXCEPTION 'CT76: sustitución no unívoca' USING ERRCODE='55000'; END IF;
 ...
 OR (SELECT octet_length(convert_to(prosrc,'UTF8')) FROM pg_proc WHERE oid=f)<>23351
 OR (SELECT encode(sha256(convert_to(prosrc,'UTF8')),'hex') FROM pg_proc WHERE oid=f) IS DISTINCT FROM 'c54a9f6fca96cd8762533839e98d7ead3d011d9497793a4fcb7123f4edc8a961'
 ...
 OR (SELECT jsonb_agg(to_jsonb(d) ORDER BY d.classid,d.objid,d.objsubid,d.refclassid,d.refobjid,d.refobjsubid,d.deptype)
  ...
  RAISE EXCEPTION 'CT76: postimagen no exacta' USING ERRCODE='55000'; END IF;
 ...
 IF obj_description(f,'pg_proc') IS DISTINCT FROM 'CT75:registro-incorporacion-v2' THEN
  RAISE EXCEPTION 'CT76: comentario versionado incompatible' USING ERRCODE='55000'; END IF;
END $ct76$;
COMMIT;
