...
SELECT pg_advisory_xact_lock(hashtextextended('vec_contratacion_temporal:migracion:000098',0));

-- Revisiones de pruebas: conserva siempre reserva, recibo e intención originales.
CREATE TABLE vec_contratacion_temporal.revision_reserva_operacion_analisis (
    ...
    artefacto_huella_sha256 text NOT NULL CHECK (artefacto_huella_sha256 ~ '^[a-f0-9]{64}$'),
    huella_semantica_hmac text NOT NULL CHECK (huella_semantica_hmac ~
        '^hmac-sha256:vec[.]contratacion-temporal[.]analisis[.]huella-semantica/v[1-9][0-9]{0,8}:[a-f0-9]{64}$'),
    registrada_en timestamptz(6) NOT NULL DEFAULT clock_timestamp(),
    PRIMARY KEY (ambito_raiz_hmac,revision),
    CONSTRAINT revision_reserva_analisis_artefacto_unico UNIQUE (ambito_raiz_hmac,artefacto_huella_sha256,huella_semantica_hmac)
);
CREATE TRIGGER revision_reserva_operacion_analisis_inmutable
BEFORE UPDATE OR DELETE ON vec_contratacion_temporal.revision_reserva_operacion_analisis
FOR EACH ROW EXECUTE FUNCTION vec_contratacion_temporal.rechazar_mutacion_historia_v1();
ALTER TABLE vec_contratacion_temporal.revision_reserva_operacion_analisis ENABLE ROW LEVEL SECURITY;
CREATE POLICY revision_reserva_operacion_analisis_propietario
...
ALTER TABLE vec_contratacion_temporal.revision_reserva_operacion_analisis FORCE ROW LEVEL SECURITY;
REVOKE ALL ON vec_contratacion_temporal.revision_reserva_operacion_analisis
...
BEGIN
    ...
    IF v_fila.resultado = 'idempotencia_reutilizada' THEN
        ...
        WHERE a.alias_ambito_hmac=p_operacion #>> '{sellos_hmac,activo,ambito_hmac}'
        FOR UPDATE OF r;
        ...
        IF v_reserva.operacion=p_operacion ->> 'operacion'
           ...
           AND NOT EXISTS (
               ...
               WHERE NOT EXISTS (
                   ...
                   WHERE i.ambito_raiz_hmac=v_reserva.ambito_raiz_hmac
...
-- Parche cerrado del guardado interno: la cadena v3/v2 de pruebas,
-- autorización fresca, consumo, historial y outbox permanece íntegra.
DO $parche$
...
BEGIN
    ...
    IF encode(sha256(convert_to(v_cuerpo,'UTF8')),'hex') <> '637335bf28baeabc1110e5b082164b4f2aa747695b549b7ee3f98c2f1de19658'
       OR position($ancla$    IF r.reserva_ref <> o ->> 'reserva_ref'$ancla$ IN v_def)=0 THEN
        RAISE EXCEPTION 'guardado previo de análisis incompatible' USING ERRCODE='55000';
    END IF;
    EXECUTE replace(v_def,$ancla$    IF r.reserva_ref <> o ->> 'reserva_ref'$ancla$,$nuevo$    -- Sólo cambia la copia local usada para contrastar esta confirmación.
    -- La fila original es inmutable; cada renovación queda registrada aparte.
    IF EXISTS (
        SELECT 1 FROM vec_contratacion_temporal.revision_reserva_operacion_analisis rv
        WHERE rv.ambito_raiz_hmac=r.ambito_raiz_hmac
          AND rv.artefacto_huella_sha256=o ->> 'artefacto_huella_sha256'
          AND rv.huella_semantica_hmac=o ->> 'huella_semantica_hmac'
    ) AND EXISTS (
        SELECT 1 FROM vec_contratacion_temporal.identidad_reserva_operacion_analisis i
        WHERE i.ambito_raiz_hmac=r.ambito_raiz_hmac
          AND i.ambito_consulta_hmac=o ->> 'ambito_consulta_hmac'
          AND i.huella_consulta_hmac=o ->> 'huella_consulta_hmac'
    ) THEN
        r.artefacto_huella_sha256 := o ->> 'artefacto_huella_sha256';
        r.huella_semantica_raiz_hmac := o ->> 'huella_semantica_hmac';
    END IF;
    IF r.reserva_ref <> o ->> 'reserva_ref'$nuevo$);
    SELECT p.prosrc INTO STRICT v_cuerpo FROM pg_proc p
    WHERE p.oid=to_regprocedure('vec_contratacion_temporal.confirmar_operacion_analisis_v1(jsonb)');
    IF encode(sha256(convert_to(v_cuerpo,'UTF8')),'hex') <> 'bb9de1e4e213cfdeca8aebdea7437a7d0e22eacbe976d6e324829e550614a52f' THEN
        RAISE EXCEPTION 'guardado resultante de análisis incompatible' USING ERRCODE='55000';
    END IF;
END
$parche$;
COMMIT;
