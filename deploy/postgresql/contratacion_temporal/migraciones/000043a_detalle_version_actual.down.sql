...
-- CT-000043A: restaura byte a byte la función CT-000043 anterior.
BEGIN;
...
BEGIN
    IF pg_catalog.current_setting('server_version_num') <> '180004'
       ...
       OR (
        ...
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'estado incompatible para revertir detalle RRHH';
    END IF;
    ...
    IF pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
       ...
       OR pg_catalog.strpos(
        ...
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'definición corregida de detalle RRHH incompatible';
    END IF;

    v_definicion := pg_catalog.pg_get_functiondef(v_firma);
    v_definicion := pg_catalog.replace(
        v_definicion, v_fragmento_corregido, v_fragmento_anterior
    );
    ...
    IF pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
       ...
       THEN
        RAISE EXCEPTION USING ERRCODE = '55000',
            MESSAGE = 'reversión de detalle RRHH incompleta';
    END IF;
END
$reversion$;
COMMIT;
