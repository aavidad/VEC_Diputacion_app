...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname='vec_contratacion_temporal_lector_historia_incorporacion')
...
END $pre$;
CREATE ROLE vec_contratacion_temporal_lector_historia_incorporacion NOLOGIN NOINHERIT NOSUPERUSER
 NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
DO $conexion$
BEGIN
 EXECUTE format('GRANT CONNECT ON DATABASE %I TO vec_contratacion_temporal_lector_historia_incorporacion',current_database());
END $conexion$;
COMMIT;
