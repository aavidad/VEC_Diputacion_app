-- Dobles de ensayo de los consumidores AD3 v3 para probar Bolsa 000029:
...
DO $f$ DECLARE n text; BEGIN
 ...
 END LOOP;
END $f$;
-- B4 lee además la huella del efecto: el doble devuelve la firma completa.
CREATE OR REPLACE FUNCTION vec_autorizacion_atestada_v3.registrar_y_consumir_datos_contacto_participacion_v3_atestada(p_capacidad bytea,p_decision bytea,p_motivo bytea,p_contexto bytea,p_persona_version numeric,p_perfil_version numeric,p_payload bytea,p_sobre bytea,p_evidencia bytea,p_raiz bytea)
 RETURNS TABLE(decision_ref text,efecto_ref text,huella_efecto_sha256 text,consumo_huella_sha256 text,auditoria_ref text,consumida_en timestamptz,consumo_nuevo boolean)
 LANGUAGE sql VOLATILE SECURITY DEFINER SET search_path=pg_catalog AS $f$
 SELECT 'decision:doble', convert_from(p_decision,'UTF8')::jsonb->>'recurso_ref', convert_from(p_decision,'UTF8')::jsonb->>'contexto_recurso_huella_sha256', repeat('c',64), 'auditoria:doble', now(), convert_from(p_capacidad,'UTF8')<>'repetida'
$f$;
...
-- Emisión B7 (publicar y resolver ofertas, 000028) y fachada AD3-84 del
-- portal del candidato: firma completa; la capacidad «repetida» simula una
-- concesión ya consumida.
DROP FUNCTION IF EXISTS vec_autorizacion_atestada_v3.registrar_y_consumir_emision_llamamiento_v3_atestada(bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea);
DO $f$ DECLARE n text; BEGIN
 FOREACH n IN ARRAY ARRAY['registrar_y_consumir_emision_llamamiento_v3_atestada','registrar_y_consumir_portal_candidato_bolsa_v3_atestada'] LOOP
  EXECUTE format($s$CREATE FUNCTION vec_autorizacion_atestada_v3.%I(p_capacidad bytea,p_decision bytea,p_motivo bytea,p_contexto bytea,p_persona_version numeric,p_perfil_version numeric,p_payload bytea,p_sobre bytea,p_evidencia bytea,p_raiz bytea)
   ...
   SELECT 'decision:'||md5(p_capacidad||p_decision), convert_from(p_capacidad,'UTF8')::jsonb->>'efecto_ref', repeat('a',64), repeat('b',64), 'auditoria:doble', clock_timestamp(),
