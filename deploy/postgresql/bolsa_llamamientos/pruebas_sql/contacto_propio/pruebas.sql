...
BEGIN
 ...
 j := vec_bolsa_llamamientos.leer_contacto_candidato_v1(a, clock_timestamp());
 ...
 IF jsonb_array_length(vec_bolsa_llamamientos.leer_contacto_candidato_v1(b, clock_timestamp())) <> 0 THEN RAISE EXCEPTION 'B sin contacto aparece'; END IF;
 ...
 IF vec_bolsa_llamamientos.leer_confirmacion_contacto_participacion_v1('part:cp:1', 1) IS NULL
    OR vec_bolsa_llamamientos.leer_contacto_candidato_v1(a, clock_timestamp())->0->>'confirmada_en' IS NULL THEN RAISE EXCEPTION 'confirmación no visible'; END IF;
