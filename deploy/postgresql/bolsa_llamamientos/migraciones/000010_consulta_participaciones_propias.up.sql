...
SELECT pg_advisory_xact_lock(hashtextextended('vec_bolsa_llamamientos:migracion:000010', 0));
...
BEGIN
 IF current_user <> 'vec_bolsa_llamamientos_propietario'
    ...
    OR to_regprocedure('vec_bolsa_llamamientos.consultar_mi_bolsa_v1(text,timestamptz,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea)') IS NOT NULL THEN
   RAISE EXCEPTION 'estado incompatible para consulta Mi bolsa' USING ERRCODE='55000';
 END IF;
END $precondicion$;

REVOKE EXECUTE ON FUNCTION vec_bolsa_llamamientos.listar_participaciones_candidato_v1(text) FROM vec_bolsa_llamamientos_ejecutor;

CREATE FUNCTION vec_bolsa_llamamientos.consultar_mi_bolsa_v1(
...
END $f$;
ALTER FUNCTION vec_bolsa_llamamientos.consultar_mi_bolsa_v1(text,timestamptz,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) OWNER TO vec_bolsa_llamamientos_propietario;
...
GRANT EXECUTE ON FUNCTION vec_bolsa_llamamientos.consultar_mi_bolsa_v1(text,timestamptz,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) TO vec_bolsa_llamamientos_ejecutor;
COMMIT;
