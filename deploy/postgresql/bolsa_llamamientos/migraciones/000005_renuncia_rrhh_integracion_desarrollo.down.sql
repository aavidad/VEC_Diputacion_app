...
BEGIN
    IF EXISTS (SELECT 1 FROM vec_bolsa_llamamientos.integracion_desarrollo WHERE tipo='renuncia_rrhh'
        OR convert_from(registro_canonico,'UTF8')::jsonb->>'tipo'='renuncia_rrhh')
       OR EXISTS (SELECT 1 FROM vec_bolsa_llamamientos.outbox_integracion_desarrollo
        WHERE convert_from(evento_canonico,'UTF8')::jsonb->>'tipo'='bolsa.llamamiento.renuncia_rrhh.registrada') THEN
        RAISE EXCEPTION 'reversión denegada: historia de renuncia RRHH' USING ERRCODE='55000';
    END IF;
    ...
    FOR v_cambio IN SELECT * FROM (VALUES
      ...
      ('integracion_desarrollo_apertura_check',$antes$tipo <> 'aceptacion_rrhh'::text$antes$,$despues$tipo <> ALL (ARRAY['aceptacion_rrhh'::text, 'renuncia_rrhh'::text])$despues$)
        ...
        EXECUTE format('ALTER TABLE vec_bolsa_llamamientos.integracion_desarrollo DROP CONSTRAINT %I, ADD CONSTRAINT %I %s',
            v_cambio.nombre,v_cambio.nombre,replace(v_def,v_cambio.nuevo,v_cambio.anterior));
    END LOOP;
