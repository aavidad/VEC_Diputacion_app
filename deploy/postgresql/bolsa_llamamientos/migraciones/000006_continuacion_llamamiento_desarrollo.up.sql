...
-- Mismo almacén y único algoritmo; permite sucesores derivados, no nuevas
-- órdenes ni fuentes fabricadas para la misma necesidad.
DO $estructura$
DECLARE v_nombre name;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
        ...
        RAISE EXCEPTION 'falta permiso de continuación Bolsa' USING ERRCODE='55000';
    END IF;
    SELECT conname INTO STRICT v_nombre FROM pg_constraint
     WHERE conrelid='vec_bolsa_llamamientos.integracion_desarrollo'::regclass
       AND contype='u' AND convalidated
       AND pg_get_constraintdef(oid,true)='UNIQUE (tipo, necesidad_ref, version_necesidad)';
    EXECUTE format('ALTER TABLE vec_bolsa_llamamientos.integracion_desarrollo DROP CONSTRAINT %I',v_nombre);
END
$estructura$;
ALTER TABLE vec_bolsa_llamamientos.integracion_desarrollo
    ADD COLUMN terminal_anterior_ref text REFERENCES vec_bolsa_llamamientos.integracion_desarrollo(operacion_ref),
    ADD COLUMN continuacion_intencion_ref text,
    ADD CONSTRAINT integracion_continuacion_terminal_unico UNIQUE (terminal_anterior_ref),
    ADD CONSTRAINT integracion_continuacion_intencion_unica UNIQUE (continuacion_intencion_ref),
    ADD CONSTRAINT integracion_continuacion_ligada CHECK ((
        (tipo='propuesta'
         AND (convert_from(registro_canonico,'UTF8')::jsonb->'propuesta'->'continuacion') IS NOT NULL
         AND terminal_anterior_ref IS NOT NULL AND continuacion_intencion_ref IS NOT NULL
         AND terminal_anterior_ref=convert_from(registro_canonico,'UTF8')::jsonb#>>'{propuesta,continuacion,terminal_operacion_ref}'
         AND continuacion_intencion_ref=convert_from(registro_canonico,'UTF8')::jsonb#>>'{propuesta,continuacion,intencion_ref}'
         AND terminal_anterior_ref<>operacion_ref)
        OR ((tipo<>'propuesta' OR (convert_from(registro_canonico,'UTF8')::jsonb->'propuesta'->'continuacion') IS NULL)
         AND terminal_anterior_ref IS NULL AND continuacion_intencion_ref IS NULL)
    ) IS TRUE);
CREATE UNIQUE INDEX integracion_orden_necesidad_unica ON vec_bolsa_llamamientos.integracion_desarrollo
    (necesidad_ref,version_necesidad) WHERE tipo='orden';
CREATE UNIQUE INDEX integracion_primera_propuesta_unica ON vec_bolsa_llamamientos.integracion_desarrollo
    (necesidad_ref,version_necesidad) WHERE tipo='propuesta' AND terminal_anterior_ref IS NULL;
DO $funcion$
