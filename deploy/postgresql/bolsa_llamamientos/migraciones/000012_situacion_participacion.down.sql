...
DO $f$ BEGIN IF EXISTS (SELECT 1 FROM vec_bolsa_llamamientos.situacion_participacion) OR EXISTS (SELECT 1 FROM vec_bolsa_llamamientos.bitacora_intento_borrador_llamamiento WHERE accion='cambiar_situacion' OR ruta_clase='situacion') THEN RAISE EXCEPTION USING ERRCODE='55000', MESSAGE='hay historia B2; no se deshace'; END IF; END $f$;
...
DROP FUNCTION vec_bolsa_llamamientos.recuperar_situacion_participacion_v1(text,text) RESTRICT;
DROP FUNCTION vec_bolsa_llamamientos.registrar_situacion_participacion_v1(text,text,text,timestamptz,timestamptz,text,text,text,text,timestamptz,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea) RESTRICT;
...
ALTER TABLE vec_bolsa_llamamientos.bitacora_intento_borrador_llamamiento ADD CONSTRAINT bitacora_intento_borrador_llamamiento_accion_check CHECK (accion IN ('crear','consultar'));
...
ALTER TABLE vec_bolsa_llamamientos.bitacora_intento_borrador_llamamiento ADD CONSTRAINT bitacora_intento_borrador_llamamiento_ruta_clase_check CHECK (ruta_clase IN ('coleccion','detalle'));
COMMIT;
