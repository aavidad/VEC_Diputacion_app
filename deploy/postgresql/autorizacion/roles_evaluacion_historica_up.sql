...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname='vec_autorizacion_evaluacion_historica_lector')
...
END $pre$;
CREATE ROLE vec_autorizacion_evaluacion_historica_lector NOLOGIN NOINHERIT NOSUPERUSER
 NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
DO $conexion$
BEGIN
 EXECUTE format('GRANT CONNECT ON DATABASE %I TO vec_autorizacion_evaluacion_historica_lector',current_database());
END $conexion$;
COMMIT;
