-- Historia y puntero opacos del vinculo corporativo RRHH C2.2-B (correccion 000004a).
-- Documento autonomo: no publica, selecciona ni concede acceso de negocio.
-- Mismo contenido que 000004, que no se reescribe porque tiene historia. Solo
-- cambia la guarda del manifiesto simbolico del predecesor: 000004 exige una
-- unica huella (bddc5574...) que ninguna base actual reproduce. 000004a admite
-- una lista cerrada, cada huella con su origen medido:
--   bddc5574... huella original de 000004 (predecesor 000001..000003 al redactarla);
--   613d1837... base nueva con 000001..000003 y el rol selector (PostgreSQL 18.4);
--   6fa63aa7... base con historia 000001, 000002 y 000005 (instalada antes que la
--               4) a la que se anade 000003: la principal de desarrollo.
-- Se instala en lugar de 000004, nunca ademas: sus guardas de ausencia rechazan
-- una base que ya tenga el vinculo corporativo.
BEGIN;
...
BEGIN
    ...
    SELECT pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
      ...
      FROM objetos;
    IF observado IS NULL OR observado NOT IN (
         ...
         '6fa63aa726d0f27d9b22315bdcc2514c5a7a0cdc696a973f90082e9a2f209fd6') THEN
      RAISE EXCEPTION USING ERRCODE='55000',
