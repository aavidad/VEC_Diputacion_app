...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname='vec_contexto_actor_v1_lector_historico')
 OR NOT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname='vec_contexto_actor_v1'
...
END $pre$;
CREATE ROLE vec_contexto_actor_v1_lector_historico NOLOGIN NOINHERIT NOSUPERUSER
 NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
DO $conexion$
BEGIN
 EXECUTE format('GRANT CONNECT ON DATABASE %I TO vec_contexto_actor_v1_lector_historico',current_database());
END $conexion$;
COMMIT;
