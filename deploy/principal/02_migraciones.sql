...
BEGIN
 SELECT pg_get_constraintdef(oid,true) INTO STRICT accion FROM pg_constraint WHERE conrelid='vec_bolsa_llamamientos.bitacora_intento_borrador_llamamiento'::regclass AND conname='bitacora_intento_borrador_llamamiento_accion_check' AND contype='c';
	 ...
	 IF strpos(accion,'''consultar_datos_contacto''')<>0 OR strpos(accion,'''emitir_llamamiento''')<>0 OR strpos(accion,'''recuperar_llamamiento''')<>0 OR strpos(ruta,'''emisiones''')<>0 OR strpos(resultado,'''correcto''')<>0 OR strpos(accion,'CHECK (accion = ANY (ARRAY[')<>1 OR right(accion,3)<>']))' OR strpos(ruta,'CHECK (ruta_clase = ANY (ARRAY[')<>1 OR right(ruta,3)<>']))' OR strpos(resultado,'CHECK (resultado = ANY (ARRAY[')<>1 OR right(resultado,3)<>']))' THEN RAISE EXCEPTION 'bitácora de frontera incompatible con B7' USING ERRCODE='55000'; END IF;
 ALTER TABLE vec_bolsa_llamamientos.bitacora_intento_borrador_llamamiento DROP CONSTRAINT bitacora_intento_borrador_llamamiento_accion_check;
 ...
 ALTER TABLE vec_bolsa_llamamientos.bitacora_intento_borrador_llamamiento DROP CONSTRAINT bitacora_intento_borrador_llamamiento_ruta_clase_check;
	 ...
	 EXECUTE 'ALTER TABLE vec_bolsa_llamamientos.bitacora_intento_borrador_llamamiento ADD CONSTRAINT bitacora_intento_borrador_llamamiento_resultado_check '||left(resultado,length(resultado)-3)||', ''correcto''::text]))';
END $bitacora$;
...
CREATE TABLE vec_bolsa_llamamientos.llamamiento_emitido(
 ...
 configuracion jsonb NOT NULL,
	 ...
	 huella_finalizacion bytea NOT NULL CHECK(octet_length(huella_finalizacion)=32),
...
DECLARE previo record; consumo record; d jsonb; v_huella text; v_total int; v_validas int; v_ordenadas int; v_contactos jsonb; BEGIN
 ...
 IF FOUND THEN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object('participacion_ref',c.participacion_ref,'resultado',c.resultado,'recibo_ref',c.recibo_ref) ORDER BY x.ordinality),'[]'::jsonb) INTO v_contactos FROM jsonb_array_elements_text(previo.participaciones) WITH ORDINALITY x(ref,ordinality) JOIN vec_bolsa_llamamientos.contacto_participacion c ON c.participacion_ref=x.ref AND c.llamamiento_ref=previo.llamamiento_ref AND c.canal='correo' AND c.clave_idempotencia=previo.clave_idempotencia||':correo:'||x.ordinality AND c.recibo_ref='recibo:contacto:'||encode(sha256(convert_to(previo.bolsa_ref||chr(31)||previo.clave_idempotencia||chr(31)||x.ref,'UTF8')),'hex');
 ...
 INSERT INTO vec_bolsa_llamamientos.llamamiento_emitido(llamamiento_ref,recibo_ref,bolsa_ref,actor_ref,clave_idempotencia,participaciones,configuracion,huella_comando_sha256,huella_finalizacion,estado,emitido_en,decision_ref) VALUES(p_llamamiento,p_recibo,p_bolsa,p_actor,p_clave,p_participaciones,p_configuracion,v_huella,p_huella_finalizacion,'emision_reservada',p_emitido,consumo.decision_ref);
...
END $f$;
CREATE FUNCTION vec_bolsa_llamamientos.registrar_contactos_llamamiento_v1(p_bolsa text,p_clave text,p_actor text,p_token_finalizacion bytea,p_contactos jsonb) RETURNS jsonb LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog SET lock_timeout='2s' AS $f$
DECLARE previo record; v_contactos jsonb; BEGIN
 IF current_user<>'vec_bolsa_llamamientos_propietario' OR p_actor !~ '^per_[A-Za-z0-9_-]{22,128}$' OR octet_length(p_token_finalizacion)<>32 OR jsonb_typeof(p_contactos)<>'array' THEN RAISE EXCEPTION 'resultado B7 inválido' USING ERRCODE='22023'; END IF;
 PERFORM pg_advisory_xact_lock(hashtextextended('vec_bolsa_llamamientos:emision:'||p_bolsa||':'||p_clave,0));
 SELECT * INTO previo FROM vec_bolsa_llamamientos.llamamiento_emitido WHERE bolsa_ref=p_bolsa AND clave_idempotencia=p_clave;
 IF NOT FOUND OR previo.actor_ref<>p_actor OR previo.huella_finalizacion<>sha256(p_token_finalizacion) OR jsonb_array_length(p_contactos)<>jsonb_array_length(previo.participaciones) OR NOT(SELECT bool_and(jsonb_typeof(c.value)='object' AND (SELECT count(*)=3 FROM jsonb_object_keys(c.value)) AND c.value->>'participacion_ref'=previo.participaciones->>(c.ordinality-1)::int AND c.value->>'resultado' IN('enviado','no_enviado') AND c.value->>'recibo_ref'='recibo:contacto:'||encode(sha256(convert_to(previo.bolsa_ref||chr(31)||previo.clave_idempotencia||chr(31)||(c.value->>'participacion_ref'),'UTF8')),'hex')) FROM jsonb_array_elements(p_contactos) WITH ORDINALITY c(value,ordinality)) THEN RAISE EXCEPTION 'resultado B7 no autorizado' USING ERRCODE='42501'; END IF;
 SELECT coalesce(jsonb_agg(jsonb_build_object('participacion_ref',c.participacion_ref,'resultado',c.resultado,'recibo_ref',c.recibo_ref) ORDER BY x.ordinality),'[]'::jsonb) INTO v_contactos FROM jsonb_array_elements_text(previo.participaciones) WITH ORDINALITY x(ref,ordinality) JOIN vec_bolsa_llamamientos.contacto_participacion c ON c.participacion_ref=x.ref AND c.llamamiento_ref=previo.llamamiento_ref AND c.canal='correo' AND c.clave_idempotencia=previo.clave_idempotencia||':correo:'||x.ordinality AND c.recibo_ref='recibo:contacto:'||encode(sha256(convert_to(previo.bolsa_ref||chr(31)||previo.clave_idempotencia||chr(31)||x.ref,'UTF8')),'hex');
...
CREATE FUNCTION vec_bolsa_llamamientos.recuperar_llamamiento_emitido_v1(p_bolsa text,p_clave text) RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog AS $f$
 SELECT jsonb_build_object('llamamiento_ref',l.llamamiento_ref,'recibo_ref',l.recibo_ref,'bolsa_ref',l.bolsa_ref,'estado',CASE WHEN (SELECT count(*) FROM jsonb_array_elements_text(l.participaciones) WITH ORDINALITY x(ref,ordinality) JOIN vec_bolsa_llamamientos.contacto_participacion c ON c.participacion_ref=x.ref AND c.llamamiento_ref=l.llamamiento_ref AND c.canal='correo' AND c.clave_idempotencia=l.clave_idempotencia||':correo:'||x.ordinality AND c.recibo_ref='recibo:contacto:'||encode(sha256(convert_to(l.bolsa_ref||chr(31)||l.clave_idempotencia||chr(31)||x.ref,'UTF8')),'hex'))=jsonb_array_length(l.participaciones) THEN 'emitido_pendiente_respuesta' ELSE 'emision_reservada_resultado_pendiente' END,'participaciones',l.participaciones,'configuracion',l.configuracion,'emitido_en',l.emitido_en,'contactos',coalesce((SELECT jsonb_agg(jsonb_build_object('participacion_ref',c.participacion_ref,'resultado',c.resultado,'recibo_ref',c.recibo_ref) ORDER BY x.ordinality) FROM jsonb_array_elements_text(l.participaciones) WITH ORDINALITY x(ref,ordinality) JOIN vec_bolsa_llamamientos.contacto_participacion c ON c.participacion_ref=x.ref AND c.llamamiento_ref=l.llamamiento_ref AND c.canal='correo' AND c.clave_idempotencia=l.clave_idempotencia||':correo:'||x.ordinality AND c.recibo_ref='recibo:contacto:'||encode(sha256(convert_to(l.bolsa_ref||chr(31)||l.clave_idempotencia||chr(31)||x.ref,'UTF8')),'hex')),'[]'::jsonb)) FROM vec_bolsa_llamamientos.llamamiento_emitido l WHERE l.bolsa_ref=p_bolsa AND l.clave_idempotencia=p_clave
$f$;
CREATE FUNCTION vec_bolsa_llamamientos.contar_llamamientos_en_curso_v1(p_bolsa text) RETURNS bigint LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog AS $f$ SELECT count(*) FROM vec_bolsa_llamamientos.llamamiento_emitido l WHERE l.bolsa_ref=p_bolsa AND (SELECT count(*) FROM jsonb_array_elements_text(l.participaciones) WITH ORDINALITY x(ref,ordinality) JOIN vec_bolsa_llamamientos.contacto_participacion c ON c.participacion_ref=x.ref AND c.llamamiento_ref=l.llamamiento_ref AND c.canal='correo' AND c.clave_idempotencia=l.clave_idempotencia||':correo:'||x.ordinality AND c.recibo_ref='recibo:contacto:'||encode(sha256(convert_to(l.bolsa_ref||chr(31)||l.clave_idempotencia||chr(31)||x.ref,'UTF8')),'hex'))=jsonb_array_length(l.participaciones) $f$;
REVOKE ALL ON FUNCTION vec_bolsa_llamamientos.reservar_llamamiento_v1(text,text,text,text,text,jsonb,jsonb,timestamptz,bytea,bytea,bytea,bytea,bytea,numeric,numeric,bytea,bytea,bytea,bytea),vec_bolsa_llamamientos.registrar_contactos_llamamiento_v1(text,text,text,bytea,jsonb),vec_bolsa_llamamientos.recuperar_llamamiento_emitido_v1(text,text),vec_bolsa_llamamientos.contar_llamamientos_en_curso_v1(text) FROM PUBLIC;
