...
const actorF4 = "administracion:f4:reactivacion-dietas-r1d"
const actoF4 = "acto:f4:reactivacion-dietas-r1d:20260924"
const actorP6 = "administracion:p6:retirada-dietas-r1d"
const actoP6 = "acto:p6:revocacion-dietas-r1d:20260923"
const actorRetiradaF4 = "administracion:f4:retirada-dietas-r1d"
const actoRetiradaF4 = "acto:f4:retirada-dietas-r1d:20260924"
...
func construir(p preimagen, ahora time.Time) (plan, error) {
	...
	if !ahora.After(r.RevocadaEn) || !ahora.Before(r.VigenteHasta) {
		return plan{}, errors.New("vigencia F4 agotada o reloj incompatible")
	}
	b := r
	b.Version = 3
...
func construirRetirada(p preimagenRetirada, ahora time.Time) (planRetirada, error) {
	...
	if a.Validar() != nil || a.Referencia() != p.Activa.Ref ||
		...
		return planRetirada{}, errors.New("preimagen de retirada F4 incompatible")
	}
	h, err := a.HuellaSHA256()
	if err != nil || h != p.Activa.Huella {
		return planRetirada{}, errors.New("huella de retirada F4 incompatible")
	}
	ahora = ahora.UTC().Truncate(time.Microsecond)
	if !ahora.After(a.EmitidaEn) {
		return planRetirada{}, errors.New("reloj de retirada F4 incompatible")
	}
	b := a
	b.Version = 4
...
func main() {
	if len(os.Args) == 2 && os.Args[1] == "--retirar" {
		...
		fmt.Printf("\\set f4_retirada_plan_b64 %s\n", base64.StdEncoding.EncodeToString(contenido))
		return
	}
	if len(os.Args) != 1 {
		fail(errors.New("modo de plan F4 desconocido"))
	}
	var p preimagen
	d := json.NewDecoder(os.Stdin)
	d.DisallowUnknownFields()
