...
SELECT pg_advisory_xact_lock(hashtextextended('f4:retirada-dietas-r1d:20260924',0));
...
CREATE TEMP TABLE f4_retirada_plan (dato jsonb NOT NULL) ON COMMIT DROP;
INSERT INTO f4_retirada_plan VALUES
 (convert_from(decode(:'f4_retirada_plan_b64','base64'),'UTF8')::jsonb);
GRANT SELECT ON f4_retirada_plan TO vec_autorizacion_propietario;
...
BEGIN
 IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname=session_user AND rolsuper)
   ...
   RAISE EXCEPTION 'F4 retirada requiere DBA en socket local de postgres' USING ERRCODE='42501';
 END IF;
 SELECT dato INTO STRICT v FROM f4_retirada_plan;
 IF v->>'actor'<>'administracion:f4:retirada-dietas-r1d'
    OR v->>'acto'<>'acto:f4:retirada-dietas-r1d:20260924'
   ...
   RAISE EXCEPTION 'plan Go F4 retirada invalido' USING ERRCODE='55000';
 END IF;
 SELECT count(*) INTO n FROM pg_roles
 WHERE rolname ~ '^vec_dietas_r1d_.*_desarrollo$';
 IF n<>8 THEN
   RAISE EXCEPTION 'inventario F4 retirada no contiene exactamente ocho LOGIN nominales' USING ERRCODE='55000';
 END IF;
 ...
 IF EXISTS (SELECT 1 FROM pg_stat_activity
   ...
   RAISE EXCEPTION 'F4 retirada requiere cero sesiones Dietas antes de la transaccion' USING ERRCODE='55000';
 END IF;
 IF EXISTS (
   ...
   RAISE EXCEPTION 'atributos o membresias F4 retirada incompatibles' USING ERRCODE='55000';
 END IF;
END $pre$;
...
BEGIN
 ...
 IF puntero.asignacion_ref IS DISTINCT FROM v->>'anterior_ref'
    OR puntero.actualizada_por<>'administracion:f4:reactivacion-dietas-r1d'
    OR puntero.acto_ref<>'acto:f4:reactivacion-dietas-r1d:20260924'
   ...
   RAISE EXCEPTION 'preimagen F4 retirada cambio o documento alterado' USING ERRCODE='55000';
 END IF;
 ...
 IF filas<>1 THEN RAISE EXCEPTION 'puntero F4 retirada no avanzo' USING ERRCODE='55000'; END IF;
END $revocar$;
RESET ROLE;
...
BEGIN
 ...
 IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname ~ '^vec_dietas_r1d_.*_desarrollo$'
   ...
   RAISE EXCEPTION 'postcondicion F4 retirada fallida' USING ERRCODE='55000';
 END IF;
END $post$;

:f4_retirada_finalizar;
