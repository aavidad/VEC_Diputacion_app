...
Recibo conservado de refiscalización:
`recibo:ct-fiscalizacion:fef58ba0c0ff0d8fdb1f451b3e0b14530b4a01c52bbd2f507809434a5782c3e7`.
Expediente sintético `expediente:ct:9511d16dce57e0ebe1baa849a749f3eed2a29ff7722dc1499132d1d536d66253`.
No repetir el registro con otra clave. La subsanación v7 y el informe original v5
permanecen en la historia. La selección posterior v8 está preparada en pantalla;
no se afirma ejecutada por este corte.
...
Recibo de subsanación conservado:
`recibo:ct-subsanacion-reparo:452803a13a00b6278f8a97a0fddd4ecafd936e410f254c2ecc804267b16c1468`.
Fecha: `2026-09-13T00:29:43.777283Z`.
Expediente: `expediente:ct:9511d16dce57e0ebe1baa849a749f3eed2a29ff7722dc1499132d1d536d66253`.
La petición original y los respaldos permanecen en la evidencia privada.
...
Abra el expediente desde la bandeja y despliegue «Historial de actuaciones»,
debajo de la tramitación. La secuencia muestra únicamente los hitos recuperados;
no rellena huecos. En nombramiento, el resumen de aceptación está conectado al
formulario CT65 habitual, pero falta comprobarlo con la aceptación recuperada.
No repita altas ni propuestas para recrear antecedentes ya conservados.
...
instaló una sola vez y recibió dos `GO`; pasaron el ensayo PostgreSQL del
catálogo en `ROLLBACK`, el catálogo instalado y las comprobaciones focales Go
de HTTP, adaptador PostgreSQL y composición. En la aplicación, abra el detalle
v9 y elija uno de los seis borradores de la propuesta original. La prueba
descargó dos representaciones del informe definitivo: PDF de 29280 bytes, SHA256
...
Las cuatro consultas de cuadro, detalle y PDF respondieron `POST 200`, y
incorporación y preparación de cierre dieron `GET 200`; el DOCX respondió `200`.
Son un PDF y un DOCX representativos, no doce descargas. La comparación de solo
lectura conservó sin cambios la historia CT en 12 tablas, su estado y las cinco
tablas de Personal, sin escrituras de negocio. No hubo
errores JavaScript ni desbordamiento a 1440/390 px; persistieron dos `404` de
Bolsa heredados y ajenos. El parser admite `menu: null` sin habilitar acciones
y usa las siete claves locales canónicas desplegadas. No reaplique CT91.
...
Bandeja y detalle muestran los nombres del catálogo de centros y categorías ya cargado; conservan las referencias originales si falta una etiqueta. Comprobación real: «Centro solicitante» y «Categoría C2», sin errores JavaScript ni desbordamiento a 1440/390 px. El 404 histórico dejó entonces pendiente la descarga desde v9; el corte D posterior la cerró desde la propuesta original v7 con un PDF y un DOCX representativos.

El detalle muestra la jornada como porcentaje (10000 → 100 %). La pantalla anterior se comprobó a 1440/390 px sin desbordamiento ni errores JavaScript y presentaba dos columnas en móvil. El CSS vigente usa una columna por debajo de 620 px y mantiene botones de navegación de al menos 44 px; el QA actual detectó desbordamiento horizontal a 390 px y no valida todavía el cambio.
...
recuperación real; Go global y vet terminaron en `PASS`.

Contacto13 está aplicado solo en fuente y revisado. La API interna Go, aún sin
exposición HTTP, permite alta, cambio y consulta con emisor V3 real y cifra con subclave KMS, AES-GCM y AAD por
persona y versión; el buffer es efímero. El módulo de usuarios solo declara
permisos, no los concede. No instale `roles_up.sql`, no infiera una dirección
heredada y no intente SMTP. Faltan preparador HMAC central y su composición,
almacén AD3-35/T13-6, gobierno, alta web, vínculo Bolsa y replay durable. Go
global y vet pasaron; no hubo JavaScript nuevo y los manifiestos 11/111/3/1
...
## Apéndice de recorrido de Módulos — 20 de septiembre de 2026
- Abra `/portal-empleado/?presentacion=rrhh&perfil=funcionario#personal` para consultar la RPT pública.
- Verifique búsqueda, paginación, fuente y aviso de no vigencia; no deduzca ocupación ni competencias.
- La estructura organizativa muestra 66 referencias sin ocupantes, permisos ni cadena de mando efectiva.
- Abra `#dietas`, ordene origen, paradas y destino y calcule con el OSRM interno.
- Cada tramo debe mostrar texto, patrón, geometría vial, kilómetros y leyenda; no se completan discontinuidades.
- El mapa usa teselas internas y el cálculo sigue rotulado como orientativo y no liquidable.
