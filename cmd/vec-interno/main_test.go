...
func TestEjecutarRechazaReglasEjemploAntesDeComponer(t *testing.T) {
	casos := map[string]struct {
		...
		"motivos fuera de desarrollo": {false, config.EnvCTAnalisisMotivosSourcePath, config.ErrConfiguracionReglasEjemploFueraDesarrollo},
		"bolsa con doble llave":       {true, config.EnvBolsaReglasSourcePath, config.ErrConfiguracionReglasEjemploSinComposicion},
		"ct con doble llave":          {true, config.EnvCTReglasSourcePath, config.ErrConfiguracionReglasEjemploSinComposicion},
	...
	for nombre, caso := range casos {
		t.Run(nombre, func(t *testing.T) {
			...
			// El rechazo llega antes de cargar la configuración interna y de
			// componer la aplicación: no abre ficheros, TLS ni escucha.
