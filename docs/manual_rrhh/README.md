...
En el detalle, abra «Historial de actuaciones» para consultar la secuencia
registrada. En móvil, el contenido se dispone en una columna y el rail conserva
su desplazamiento horizontal dentro del panel. El resumen de aceptación está
conectado al formulario de propuesta habitual; su recorrido completo sigue
pendiente de comprobación con una aceptación recuperada.
...
Bandeja y detalle muestran los nombres del catálogo de centros y categorías ya cargado; conservan las referencias originales si falta una etiqueta. Comprobación real: «Centro solicitante» y «Categoría C2», sin errores JavaScript ni desbordamiento a 1440/390 px. El 404 del lector histórico y la descarga pendiente desde v9 pertenecen a un corte anterior; el corte D posterior cerró la recuperación desde la propuesta original v7 con descargas representativas PDF y DOCX.

El detalle muestra la jornada como porcentaje (10000 → 100 %). La pantalla anterior se comprobó a 1440/390 px sin desbordamiento ni errores JavaScript y usaba dos columnas en móvil. El CSS vigente cambia a una columna por debajo de 620 px y conserva botones de navegación de al menos 44 px; el QA actual detectó desbordamiento horizontal a 390 px, por lo que el ajuste no está validado.
...
cierre devolvió `400`. Tras corregir la allowlist HTTP, se importaron exactamente
...
mismo clon87 reinició con salud `200`, sin tocar principal ni material web, y la
anotación se recuperó con el mismo recibo, fecha y v9. Tras reiniciar también
PostgreSQL y la app del clon87, **Recuperar anotación** devolvió `GET 200` con el
mismo recibo/v9 y el replay del cierre devolvió `POST 200` con el mismo recibo
y seguimiento 2. No duplicó registro, preparación ni outbox; solo añadió la
auditoría prevista de recuperación y conservó la original. Personal permaneció
exacto. Este cierre no firma, cesa, da eficacia, envía ni confirma GINPIX externo.
...
La corrección UI5 del estado mostrado al refrescar un `GET` tiene `GO`, está
aplicada en integración y pasó dentro de 395 pruebas web y manifiestos
11/111/3/1. Aún no está en runtime; este recorrido usó la web `037b…` y el
binario `bc65…`.
