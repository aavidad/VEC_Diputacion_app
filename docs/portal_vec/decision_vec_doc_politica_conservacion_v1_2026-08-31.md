...
`internal/vec/ports` incorpora un contrato neutral para solicitar y obtener la
única política aprobada que coincide con todos estos vínculos:
...
al coordinador a elegir la primera. `ResolverPoliticaConservacionDocumental`
exige cardinalidad exactamente uno y vuelve a comprobar cada ligadura.

El reloj neutral ya existente en `ports` se inyecta y se consulta después de
la resolución. La política solo es aplicable cuando está aprobada y el
instante UTC de microsegundo se encuentra dentro de su vigencia semiabierta.
Un contexto cancelado antes o durante la frontera lenta deniega sin producir
resultado.
...
La función de resolución devuelve el valor cero y el único error público
`ErrPoliticaConservacionDocumentalNoResuelta` ante cualquiera de estas
condiciones:
...
- `capacidad_documental.md`: Documentos es la autoridad transversal y los
  módulos consumidores no generan reglas propias;
...
La familia `TestPoliticaConservacionDocumental` cubre:
...
```text
gofmt
GOPROXY=off go test ./internal/vec/ports -run '^TestPoliticaConservacionDocumental' -count=1
GOPROXY=off go test -race ./internal/vec/ports -run '^TestPoliticaConservacionDocumental' -count=1
GOPROXY=off go vet ./internal/vec/ports
git diff --check
```

Resultado local sobre el árbol de trabajo de la base exacta:

- prueba focal normal: verde;
- prueba focal con detector de carreras: verde;
- `go vet` focal: verde;
- formato y `git diff --check`: verdes;
- tamaños: producción 358 líneas, pruebas 473 y decisión 144 antes de añadir
  este resultado; todos permanecen por debajo del máximo de 800.
