...
| Camino crítico | F0/C4b-2/G2-O: O3a-P0 → O3a → O3b → O3c → O4a → O4b → O4c → O5a → O5b → O5c → O5d → O6a → O6b → O6c → C4b-3 → C4c → C4d → C2.4 selección/recibo → C2.5 fachada/reconciliación → PDP → composición raíz → TLS/mTLS → web definitiva → E2E |
...
| Último corte remoto completamente verde | `c1ca5aa` en `integracion/ct-o4-04e-20260726`; CI `31287803830`, cinco de cinco puertas verdes. |
| Último commit técnico verificado | `d86aea8`–`4b39265` integra O2b tras doble GO: runner 800, G1 692, G2 798, G3 431, G4 404, G5 507, binario reproducible, AST, 31 mutantes muertos, H0 PostgreSQL 18.4 y puerta global verde; `c1ca5aa` publica su cierre. O3a-P0 tiene contrato y acta con doble GO, pendientes de publicación+CI antes del código. |
...
| Bloqueo externo actual | Ninguno; O3a-P0 mantiene una puerta interna obligatoria de publicación+CI antes del código y producción sigue sujeta a las conformidades formales |
