...
| `web/static/presentacion/index.html`, `presentacion.css`, `selector-perfiles.js`, `selector-perfiles.css` y sus pruebas | Lanzador y selector de cuatro puntos de vista exclusivos de la muestra. | **Eliminar al cierre**. Los dos archivos del selector y sus dos importaciones condicionales pueden retirarse como una sola pieza. | Portadas pública e interna definitivas; la identidad y las capacidades se resuelven en servidor, no por un selector DEMO. | RRHH acepta accesos y navegación definitivos, incluidos retorno por logotipo y separación público/interno. | No existe `app/web/static/presentacion/`; el script de contenido ya lo comprueba. |
