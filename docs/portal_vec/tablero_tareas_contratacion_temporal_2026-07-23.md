...
| O2-05 | Función SQL de confirmación atómica. | Consumo único de autorización + reserva + expediente + actuación + auditoría + outbox en un `COMMIT`. | 🚧 | Diseño `1f16164`; VEC-AD-3 integrado en `8461aee`; candidato SQL local con agregado portable, replay exhaustivo, emisor Go público, cancelación/resultado indeterminado, ACL y rollback probado; pendientes dos revisiones independientes |
