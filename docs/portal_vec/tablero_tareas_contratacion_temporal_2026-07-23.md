...
| O2-08 | API interna y neutral al cliente del alta. | Lista positiva, límites, sin `Cookie`/`Set-Cookie`, almacenamiento web ni cabeceras libres de autoridad; credencial breve ligada al cliente; mismo contrato para web, escritorio, CLI y MCP; contrato OpenAPI. | 🚧 | Candidato O2-08B `3e2885c`: sobre común `{clave_idempotencia, solicitud}`, contexto autoritativo separado y puertas del productor verdes. Pendiente revisión independiente; no registra ruta ni compone O2-07. |
...
| O3-02 | Caso de uso de registrar/rectificar análisis con CAS. | Versiones concurrentes, rectificación motivada y segregación de funciones. | 🚧 Candidato `a1c0739` en corrección tras NO-GO independiente: persiste orquestación multipuerto en `ports` y el replay temprano no coteja actor, perfil, idempotencia, datos ni motivo. No integrado. |
...
| O4-02 | Consultas minimizadas a Bolsa, SAE y convocatorias. | No accede a tablas ajenas; timeouts y procedencia registrada. | 🚧 Candidato `a0c7ecf` en corrección tras NO-GO independiente: revalidación temporal posterior a la presentación, replay durable tras rotación de clave y orquestación residual fuera de `ports`. No integrado. |
...
| Tarea | Entregable aislado | Verificación de cierre | Estado |
| --- | --- | --- | --- |
| O8-01 | Estados de seguimiento, prórroga, incidencia y cese. | Transiciones versionadas, motivos y calendario hábil. | 🚧 Candidato `d2d2b75` en corrección tras NO-GO independiente: reapertura conserva un cese incompatible, el detector admite ciclos silenciosos con documentos opcionales y el canon no limita periodos desacoplados. No integrado. |
