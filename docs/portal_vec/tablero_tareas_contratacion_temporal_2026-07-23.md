...
| O2-06 | Adaptador Go de confirmación y reconciliación de resultado indeterminado. | Éxito, replay, concurrencia, timeout antes/después de `COMMIT`, reinicio y recibo adulterado. | 🚧 | Corrección local candidata: resolver durable, fachada SQL V2, `SafeToRetry`, reinicio, conflicto de huella, concurrencia, ambigüedad, ACL y `down` protegido superan PostgreSQL 18. Falta revisión independiente y puertas globales sobre el commit final; el `NO-GO` previo aún no se da por levantado. |
