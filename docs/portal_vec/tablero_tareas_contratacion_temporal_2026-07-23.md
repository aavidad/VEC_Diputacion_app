...
| O4-05 | API, pantalla comparativa y E2E. | La vía elegida muestra fuentes y justificación sin exponer PII indebida. | 🚧 Tres de cinco hitos. Los contratos HTTP de cobertura, registro modular, cliente seguro, recuperación, ciclo de vida, proyecciones, autorización, recibos, acceso durable, cursores y composición visual están cerrados aisladamente. CT-000039 a CT-000046 cierran registrador, contrato, cánones, recibo, vocabulario de estados, prueba durable, motor privado, fachadas nominales y adaptador Go; CT-000047A integra los manejadores protegidos y sus dos endurecimientos finales con P0=P1=P2=0. CT-000047B ha cerrado de forma aislada la retención exacta, recursos D0/D1/D2, raíz A2.1, fachada A2.2, motivos M0, preparaciones A4.1/A4.2, guardián A4.3, consumidores A5 y toda la cadena M1/M2, incluido PostgreSQL 18.4 real. CT-000047C1 cierra la cápsula de identidad ligada una sola vez al canal y petición, con revalidación viva y doble GO. CT-000047C2.1a añade el rol selector RRHH mínimo, aislado y sin acceso funcional: el código final está en `e8c950c`, el cierre documental en `808522d` y su CI `30527303065` terminó verde. C2.1b conserva el borrador `e8883df` sin runner ni `GO`; su dependencia ACL está probada en una rama candidata pendiente de revisión. Ninguno se contabiliza. Producción conserva `NO-GO`: faltan la fachada de Identidad C2.1b, selección/registro corporativos, PDP, composición raíz, TLS/mTLS viva y E2E completo. La trazabilidad detallada se conserva en los relevos y revisiones. |
