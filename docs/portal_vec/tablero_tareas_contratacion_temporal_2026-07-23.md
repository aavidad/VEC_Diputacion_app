...
| O4-05 | API, pantalla comparativa y E2E. | La vía elegida muestra fuentes y justificación sin exponer PII indebida. | 🚧 `1a764cf` con `GO`: contrato HTTP cerrado. Segundo corte local con `GO` mecánico: registro atómico de cuatro rutas y autoridad previa obligatoria, sin descubrimiento genérico; faltan C5, dependencias productivas, cliente, recuperación y E2E. |
