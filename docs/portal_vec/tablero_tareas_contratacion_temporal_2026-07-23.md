...
| O4-05 | API, pantalla comparativa y E2E. | La vía elegida muestra fuentes y justificación sin exponer PII indebida. | 🚧 Contrato HTTP `1a764cf`, registro modular inicial `7a866b3`, cliente web inicial `023b890` y recuperación propia `87a1c37`–`93674c7`, con revisiones independientes verdes. Adaptador PostgreSQL, consulta web, HTTP y quinta ruta modular cerrados aisladamente en `e36310a`, `7d4e1ec`, `42920ae` y `5965223`; `6b33474` acredita además adaptador Go→PostgreSQL 18.4 con LOGIN real. `69b6a14` cierra el ciclo de vida, `d6d1305` las proyecciones protegidas y `4714088` la composición visual/catálogos gobernados. `d307b42` y `d3d6a04` acreditan técnicamente el pool y la recuperación atómica; `2a9ddb1` aporta el resumen tipado VEC-AD-3. `a0d39c1` añade consumidores nominales de cuadro/detalle y `2820759` el rol y registro durable de accesos. `3cb17ca` añade la publicación global estable y `2f014c4` la infraestructura de cursor con encadenamiento probatorio, privilegio criptográfico mínimo y doble GO independiente en PostgreSQL 18.4. `9655d62` liga sesión, perfil y huella estable de filtros; `183e563` revalida el consumo tras la lectura y `0dc2edc` acredita la identidad corporativa. CT `000039` cierra el registrador v2 minimizado, snapshot *as-of*, carreras con revocación, privacidad probada y safe-down `RESTRICT`. CT `000040` añade contrato tipado, canon de alcance, igualdad sesión-control y reversión exhaustiva, con GO independiente y cuatro reproducciones PostgreSQL 18.4. Los commits `0109f48`–`0f7d979` añaden los cánones Go y el Recibo de lectura RRHH V2, corrigen el replay temporal y exigen el discriminador SQL exacto, con `GO` independiente. Los commits `7ae4122`–`676191b` cierran el vocabulario PostgreSQL de seis estados con reversión estructural exacta y `GO` independiente. `a1a09b9` cierra CT `000043`: prueba durable, Recibo RRHH V2, causalidad VEC, concurrencia, retirada segura y doble `GO` en PostgreSQL 18.4. `35cba6a` añade CT `000043A`: conserva la consulta con versión observada cero y registra la versión positiva materializada, con doble `GO` y barreras `23/7` intactas. `2f7e9e2`–`b442b02` cierran CT `000044`: motor privado de cuadro/detalle, consumo VEC prelectura, identidad viva, control causal por familia, cursores, cuatro carreras, paquete `UP/DOWN`, barreras `24/8` y doble `GO` PostgreSQL 18.4; los runners tampoco exponen capacidades en `argv`. Producción conserva `NO-GO` hasta cerrar fachadas `000045`, adaptador Go, composición raíz, matriz TLS viva y E2E HTTP completo. Corte oficial: 3/5 hitos hasta superar esas puertas. |
