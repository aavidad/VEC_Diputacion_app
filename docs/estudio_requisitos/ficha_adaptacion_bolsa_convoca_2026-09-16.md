...
| B11 | Portal personal del candidato: su posición, estado, contratos, último llamamiento, disponibilidad; pausar / reactivar. | R p.2; M §7 | 15 acciones declaradas en `area-personal/cliente-http.js`, 0 compuestas. | Acceso con DNIe o certificado (decisión 4), ficha propia y las operaciones de B8 desde el portal. |
...
| B15 | Evaluación personal de oportunidades y acceso directo a la solicitud. | Decisión del operador, 20/09/2026 | El área personal enumera convocatorias DEMO y la web pública muestra su detalle y requisitos, pero los requisitos son texto libre y no hay cotejo con el expediente de méritos ni enlace personalizado real. | Estructurar y versionar requisitos de acceso; contrastarlos con datos declarados, previstos o acreditados de la persona y mostrar `cumple`, `cumplimiento_previsto`, `no_cumple` o `pendiente` con explicación. Cada regla fija el hito en que debe cumplirse según las bases. Si se cumple o las bases admiten obtenerlo antes de un hito futuro, permitir abrir el detalle público por `identificador_publico` e iniciar la solicitud precompletada, revalidando después; si falta un dato, dirigir a completarlo. No confundir acceso con puntuación ni evaluación automática con admisión definitiva. |
