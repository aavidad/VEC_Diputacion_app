...
4. **P2-b (cursor ligado al canal TLS)**: el vínculo usa el material exportado de la conexión TLS (RFC 5705), por lo que la continuación falla a través del proxy de captura (una conexión por petición) pero funciona con el navegador directo por mTLS y HTTP/2 (una sola conexión). Cambiarlo a un vínculo por certificado + sesión de servidor exige la doble revisión prevista.
