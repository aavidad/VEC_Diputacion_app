...
export function crearVistasOperaciones(u) {
  ...
  function renderizarComunicaciones(datos) {
    ...
    return `
      ...
      <section class="panel panel-separado"><div class="cabecera-panel"><h3>Componer comunicación</h3><span class="estado-chip info">Sin envío</span></div><form class="cuerpo-panel formulario-gobernado" data-comando="preparar-comunicacion"><fieldset><legend>Mensaje</legend><div class="rejilla-formulario">${campo("Expediente", '<select name="expediente"><option>DEMO-LLA-045</option><option>DEMO-ALE-001</option></select>')}${campo("Plantilla", '<select name="plantilla"><option>Aviso de llamamiento v3</option><option>Resolución de alegación v2</option></select>')}${campo("Canales", '<select name="canales"><option>Correo + aviso interno</option><option>Notificación fehaciente</option></select>')}${campo("Plazo de comparecencia", '<input name="plazo" value="10 días hábiles">')}${campo("Asunto", '<input name="asunto" value="Comunicación DEMO del expediente">')}${campo("Contenido", '<textarea name="contenido">Contenido sintético generado desde una plantilla versionada.</textarea>')}</div></fieldset>${botonOperacion("Preparar sin enviar", "preparar-comunicacion", "DEMO-COM-NUEVA", "boton-primario")}</form></section>`;
  }
