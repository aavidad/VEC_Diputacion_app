...
test("una caché caliente descarga el grafo renovado hasta Personal i18n", async () => {
  ...
  assert.deepEqual(versiones(html, "/portal-empleado/portal.js"), [versionNueva]);
  const portal = await cargar("portal.js", versionNueva);
  assert.deepEqual(versiones(portal, "./portal-modulos-coordinador.js"), [versionNueva]);
  const coordinador = await cargar("portal-modulos-coordinador.js", versionNueva);
  ...
  assert.deepEqual(descargas, new Set([
    `/portal-empleado/portal.js?v=${versionNueva}`,
    `/portal-empleado/portal-modulos-coordinador.js?v=${versionNueva}`,
