...
export function crearPresentadorPanelInterno(dependencias) {
  ...
  function renderizarNuevoLlamamiento(bolsa, candidatos, flujo) {
    ...
    if (paso === 2) {
      ...
      const limiteAlcanzado = visibles.length > 100;
      ...
      contenido = `<form class="panel" data-bolsa-form="b7-paso2"><div class="cabecera-panel"><h3>2. Seleccionar candidatos</h3><span class="estado-chip info">${numero(visibles.length)} ocupan turno</span></div><div class="cuerpo-panel"><fieldset><legend>Estados a incluir</legend>${["disponible", "disponible_desde"].map((estado) => `<label><input type="checkbox" name="estado" value="${estado}" ${estados.has(estado) ? "checked" : ""}> ${escaparHTML(etiquetaClave(estado))}</label>`).join(" ")}</fieldset><label><input type="checkbox" checked disabled> Respetar orden de prelación (obligatorio)</label><p><small>Las personas trabajando o en pausa no ocupan turno.</small></p><button type="button" class="boton-secundario" data-bolsa-accion="b7-seleccionar-todas">Seleccionar todas las que cumplen el filtro</button><p role="status"><strong>${numero(seleccionadas)} seleccionadas.</strong> ${limiteAlcanzado ? "Se envían las 100 primeras; el resto, en otro envío." : ""}</p></div><div class="tabla-contenedor"><table class="tabla-datos"><caption>Candidatos por el orden vigente de B6</caption><thead><tr><th>Selección</th><th>Nº orden</th><th>Candidato</th><th>Estado</th></tr></thead><tbody>${filas || '<tr><td colspan="4">No hay candidatos con turno vigente en esos estados.</td></tr>'}</tbody></table></div><div class="cuerpo-panel"><span>Mostrando ${visibles.length ? inicio + 1 : 0} a ${Math.min(inicio + 6, visibles.length)} de ${numero(visibles.length)}</span> <button type="button" class="boton-secundario" data-bolsa-accion="b7-pagina" data-pagina="${pagina - 1}" ${pagina === 0 ? "disabled" : ""}>Anterior</button> <button type="button" class="boton-secundario" data-bolsa-accion="b7-pagina" data-pagina="${pagina + 1}" ${inicio + 6 >= visibles.length ? "disabled" : ""}>Siguiente</button> <button class="boton-primario" type="submit">Configurar llamamiento</button></div></form>`;
    }
