...
export function crearPresentadorPanelInterno(dependencias) {
  ...
  function renderizarNuevoLlamamiento(bolsa, candidatos, flujo, fuente = {}) {
    ...
    if (paso === 2) {
      ...
      contenido = `<form class="panel" data-bolsa-form="b7-paso2" aria-busy="${flujo.consultando ? "true" : "false"}"><div class="cabecera-panel"><h3>2. Seleccionar candidatos</h3><span class="estado-chip info">${numero(visibles.length)} ocupan turno en esta página</span></div><div class="cuerpo-panel"><fieldset><legend>Estados a incluir</legend>${["disponible", "disponible_desde"].map((estado) => `<label><input type="checkbox" name="estado" value="${estado}" ${estados.has(estado) ? "checked" : ""}> ${escaparHTML(etiquetaClave(estado))}</label>`).join(" ")}</fieldset><label><input type="checkbox" checked disabled> Respetar orden de prelación (obligatorio)</label><p><small>Las personas trabajando o en pausa no ocupan turno.</small></p><button type="button" class="boton-secundario" data-bolsa-accion="b7-seleccionar-todas" ${flujo.consultando ? "disabled" : ""}>Seleccionar todas las que cumplen el filtro</button><details><summary aria-label="Ayuda sobre el límite de selección">?</summary><p>Se consulta la bolsa completa con los estados elegidos aquí y se seleccionan por orden B6 hasta 100 candidaturas por envío. Para incluir candidaturas posteriores, avance por las páginas de la bolsa y selecciónelas en otro envío.</p></details><p role="status" tabindex="-1" data-b7-seleccion-status><strong>${numero(seleccionadas)} seleccionadas</strong>${flujo.consultando ? ". Consultando todas las páginas…" : flujo.totalElegibles !== null && flujo.totalElegibles !== undefined ? ` de ${numero(flujo.totalElegibles)} candidaturas que cumplen el filtro.` : "."}</p>${flujo.error ? `<p role="alert" tabindex="-1" data-b7-seleccion-error class="mensaje-error">${escaparHTML(flujo.error)}</p>` : ""}</div><div class="tabla-contenedor"><table class="tabla-datos"><caption>Candidatos por el orden vigente de B6</caption><thead><tr><th>Selección</th><th>Nº orden</th><th>Candidato</th><th>Estado</th></tr></thead><tbody>${filas || '<tr><td colspan="4">No hay candidatos con turno vigente en esos estados.</td></tr>'}</tbody></table></div><div class="cuerpo-panel"><span>Mostrando ${visibles.length ? inicio + 1 : 0} a ${Math.min(inicio + 6, visibles.length)} de ${numero(visibles.length)} en esta página</span> <button type="button" class="boton-secundario" data-bolsa-accion="b7-pagina" data-pagina="${pagina - 1}" ${pagina === 0 ? "disabled" : ""}>Anterior</button> <button type="button" class="boton-secundario" data-bolsa-accion="b7-pagina" data-pagina="${pagina + 1}" ${inicio + 6 >= visibles.length ? "disabled" : ""}>Siguiente</button> <button type="button" class="boton-secundario" data-bolsa-accion="b7-fuente-anterior" ${!flujo.cursoresPagina || flujo.cursoresPagina.length < 2 || flujo.consultando ? "disabled" : ""}>Página anterior de la bolsa</button> <button type="button" class="boton-secundario" data-bolsa-accion="b7-fuente-siguiente" ${!fuente.hay_mas || flujo.consultando ? "disabled" : ""}>Más candidaturas de la bolsa</button> <button type="button" class="boton-secundario" data-bolsa-accion="b7-limpiar-seleccion" ${!seleccionadas || flujo.consultando ? "disabled" : ""}>Borrar selección</button> <button class="boton-primario" type="submit" ${flujo.consultando ? "disabled" : ""}>Configurar llamamiento</button></div></form>`;
    }
    if(paso===3) contenido=`<form class="panel" data-bolsa-form="b7-paso3"><div class="cabecera-panel"><h3>3. Configurar llamamiento</h3></div><div class="cuerpo-panel rejilla-formulario"><p class="nota-seguridad campo-ancho" role="note"><strong>El asunto y el texto son comunes a todas las personas seleccionadas.</strong> La personalización por persona está pendiente de desarrollo.</p><label>Referencia de necesidad<input name="referencia" required minlength="2" maxlength="160" value="${escaparHTML(flujo.configuracion?.referencia||"")}"></label><label>Categoría<input name="categoria" required value="${escaparHTML(flujo.configuracion?.categoria||bolsa.categoria)}"></label><label>Centro<input name="centro" required minlength="2" maxlength="200" value="${escaparHTML(flujo.configuracion?.centro||"")}"></label><label>Modalidad<select name="modalidad" required>${["Sustitución", "Vacante", "Programa temporal", "Acumulación de tareas"].map((modalidad) => `<option ${flujo.configuracion?.modalidad === modalidad ? "selected" : ""}>${modalidad}</option>`).join("")}</select></label><label>Fecha prevista de inicio<input type="date" name="fecha_inicio" required value="${escaparHTML(flujo.configuracion?.fecha_inicio||"")}"></label><label>Canal<input value="Correo obligatorio" readonly></label><label>Plazo provisional<input name="plazo" required value="48 horas · pendiente de RRHH, dudas 1–3" readonly></label><label class="campo-ancho">Descripción<textarea name="descripcion" required minlength="2" maxlength="1000">${escaparHTML(flujo.configuracion?.descripcion||"")}</textarea></label><label>Plantilla<input name="plantilla_version" value="bolsa-llamamiento-v1" readonly></label><label>Asunto<input name="asunto" required value="${escaparHTML(flujo.configuracion?.asunto||`Llamamiento de bolsa: ${bolsa.categoria}`)}"></label><label class="campo-ancho">Texto del correo<textarea name="cuerpo" required maxlength="4000">${escaparHTML(flujo.cuerpoBorrador||"Se comunica una necesidad de cobertura. La referencia, categoría, centro, modalidad y fecha prevista figuran en este llamamiento. Responda por el cauce indicado por RRHH.")}</textarea></label><p class="nota-integracion campo-ancho"><strong>Pendiente de integración con correo corporativo.</strong> El recorrido usa el relay de desarrollo configurado; no acredita entrega en el buzón corporativo.</p>${flujo.error?`<p role="alert" class="mensaje-error campo-ancho">${escaparHTML(flujo.error)}</p>`:""}<button type="submit" class="boton-primario">Revisar llamamiento</button></div></form>`;
    if(paso===4){const c=flujo.configuracion||{};contenido=`<section class="panel"><div class="cabecera-panel"><h3>4. Revisar y enviar</h3><span class="estado-chip advertencia">Pendiente de confirmación</span></div><div class="cuerpo-panel"><dl class="resumen-expediente"><div class="fila-resumen"><dt>Necesidad</dt><dd>${escaparHTML(c.referencia)}</dd></div><div class="fila-resumen"><dt>Centro / modalidad</dt><dd>${escaparHTML(c.centro)} · ${escaparHTML(c.modalidad)}</dd></div><div class="fila-resumen"><dt>Candidatos</dt><dd>${numero(flujo.participaciones?.length||0)} seleccionados${flujo.totalElegibles > (flujo.participaciones?.length||0) ? ` de ${numero(flujo.totalElegibles)} elegibles` : ""}, en orden de prelación</dd></div><div class="fila-resumen"><dt>Canal</dt><dd>Correo SMTP; teléfono queda como intento manual B3</dd></div><div class="fila-resumen"><dt>Plazo</dt><dd>${escaparHTML(c.plazo)}</dd></div></dl><form data-bolsa-form="b7-paso4" data-cantidad="${numero(flujo.participaciones?.length||0)}"><label><input type="checkbox" name="confirmacion" required> Confirmo la emisión para exactamente ${numero(flujo.participaciones?.length||0)} candidatos y el envío al relay configurado.</label><button type="submit" class="boton-primario" ${flujo.enviando||flujo.recibo?"disabled":""}>${flujo.enviando?"Enviando…":flujo.recibo?"Emitido":"Emitir llamamiento"}</button></form>${flujo.error?`<p role="alert" tabindex="-1" data-b7-emision-error class="mensaje-error">${escaparHTML(flujo.error)}</p>`:""}${flujo.error_422?`<div><button type="button" class="boton-secundario" data-bolsa-accion="b7-revisar-configuracion">Revisar configuración</button> <button type="button" class="boton-secundario" data-bolsa-accion="b7-volver-seleccion">Actualizar selección</button></div>`:""}${flujo.recibo?`<section class="mensaje-exito" tabindex="-1" data-b7-recibo><strong>Llamamiento ${escaparHTML(flujo.llamamiento_ref)}</strong><br>Estado: emitido, pendiente de respuesta.<br>Recibo <code>${escaparHTML(flujo.recibo)}</code> · <button type="button" class="boton-secundario" data-bolsa-accion="ver-historico-b7">Abrir Histórico</button></section>`:""}</div></section>`}
    return `${encabezadoVista("Gestión interna de Bolsas","Nuevo llamamiento","Recorrido real B7 sobre bolsa constituida, candidatos B5, PostgreSQL y SMTP.",`<button type="button" class="boton-secundario" data-bolsa-accion="cancelar-b7" ${flujo.enviando?"disabled":""}>${flujo.enviando?"Envío en curso":"Cancelar"}</button>`)}${rail}<div class="distribucion-llamamiento"><div>${contenido}</div>${resumen}</div>`;
  }
