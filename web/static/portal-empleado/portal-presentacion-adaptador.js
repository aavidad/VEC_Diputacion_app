...
export function crearFuenteLecturaBolsasPresentacion({ datosIniciales } = {}) {
  ...
  const bolsasBase = datosIniciales.bolsas.map((bolsa, indice) => {
    ...
    const candidatos = [
      ...
      orden: estado_clave === "disponible" ? 1 : null,
      orden_acta: posicion + 1,
      razon_orden: estado_clave === "disponible" ? "orden_acta" : estado_clave === "trabajando" ? "trabajando" : estado_clave === "no_disponible" ? "pausa" : "sin_turno",
      nombre_visible,
    ...
    return {
      bolsa: {
		...
		tipo_lista: "rotatoria",
		llamamientos_en_curso: 0,
		politica_orden: { politica_ref: `politica:orden:presentacion:${indice + 1}`, version: 1, criterio: "puntuacion_desc_acta", tipo_lista: "rotatoria", reposicion: "misma_posicion", provisional: true, rotulo: "Provisional, pendiente de RRHH (dudas 13–14)", actor: "sistema:presentacion", vigente_desde: "2026-09-01T00:00:00Z" },
