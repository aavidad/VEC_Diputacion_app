...
export function crearVistasBaremacion(u) {
  ...
  function renderizarMeritos(datos, estado = {}) {
    ...
    return `<div class="baremacion-vista">
      ...
      <p class="nota-pendiente">${e(t("meritos_decision_bloqueada"))}</p>${recorrido()}
  ...
  function renderizarBaremacion(datos, estado = {}) {
    ...
    if (!contexto?.convocatoria_ref || !contexto?.solicitud_ref || !contexto?.version_reglas
    ...
    return `<div class="baremacion-vista">
      ...
      <p class="nota-pendiente">${e(t("baremacion_aviso"))}</p>${recorrido()}
      ...
      <section class="panel">${cabecera("baremacion_contexto", "ayuda_ranking", "baremacion_contexto_ayuda")}
        <div class="cuerpo-panel"><dl class="resumen-expediente">
          ...
          <div class="fila-resumen"><dt>${e(t("baremacion_entrada"))}</dt><dd>${e(contexto.solicitud_ref)} · ${e(contexto.fuente_calculo)}</dd></div>
  ...
  function renderizarAlegaciones(datos, estado = {}) {
    ...
    const filas = alegaciones.map((item) => [
      ...
      item.registro?.fecha && item.registro?.referencia
        ? `${e(fecha(item.registro.fecha))}<small>${e(item.registro.referencia)}</small>`
      ...
      e(item.evidencia), chip(item.estado),
    ...
    return `<div class="baremacion-vista">
      ...
      <p class="nota-pendiente">${e(t("alegaciones_aviso"))}</p>${recorrido()}
