...
export function crearVistasBaremacion(u) {
  ...
  function renderizarBaremacion(datos) {
    ...
    return `
      ...
      <section class="panel panel-separado"><div class="cabecera-panel"><div><h3>Configurar criterio</h3><p>Formulario operativo que se conectará al catálogo gobernado de reglas.</p></div>${fuentePresentacion()}</div><form class="cuerpo-panel formulario-gobernado" data-comando="guardar-reglas-baremo"><fieldset><legend>Puntuación por experiencia</legend><div class="rejilla-formulario">${campo("Unidad de tiempo", '<select name="unidad_tiempo"><option>Mes completo</option><option>Día</option><option>Año</option></select>')}${campo("Puntos por unidad", '<input type="number" name="puntos_unidad" value="0.10" min="0" step="0.01">')}${campo("Fracción de jornada", '<select name="fraccion_jornada"><option>Prorratear por porcentaje</option><option>No prorratear si las bases lo permiten</option></select>')}${campo("Tope del bloque", '<input type="number" name="tope_bloque" value="6.00" min="0" step="0.01">')}${campo("Ámbito de experiencia", '<select name="ambito_experiencia"><option>Administración convocante</option><option>Otra administración</option><option>Sector privado equivalente</option></select>')}${campo("Regla de redondeo", '<select name="redondeo"><option>Sin redondeo intermedio</option><option>Dos decimales al final</option></select>')}</div></fieldset><fieldset><legend>Desempates ordenados</legend><div class="rejilla-formulario">${campo("1.º criterio", '<select name="desempate_1"><option>Mayor puntuación en experiencia</option></select>')}${campo("2.º criterio", '<select name="desempate_2"><option>Mayor puntuación en formación</option></select>')}${campo("3.º criterio", '<select name="desempate_3"><option>Fecha y hora de registro</option></select>')}${campo("Último recurso", '<select name="ultimo_recurso"><option>Sorteo trazado</option></select>')}</div></fieldset>${botonOperacion("Guardar versión de reglas", "guardar-reglas-baremo", "DEMO-CRI-001", "boton-primario")}</form></section>
