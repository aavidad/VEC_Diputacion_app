...
export function crearVistasBaremacion(u) {
  ...
  function renderizarMeritos(datos, estado = {}) {
    ...
    return `
      ...
      ${avisoPresentacion("Los datos y decisiones son sintéticos; la motivación solo se edita en la demostración y no registra valoraciones administrativas ni produce efectos.")}
      ...
      <section class="panel"><div class="cabecera-panel"><div><h3>Cola de evidencias</h3><p>Referencias seudonimizadas para reducir exposición durante la revisión masiva.</p></div>${fuentePresentacion()}</div>
        ...
        <p class="resultado-filtro" role="status" data-total-filtro="meritos" data-total="${meritos.length}">${traducirBolsaInterna("numero_meritos", { numero: numero(meritos.length) })}</p>
