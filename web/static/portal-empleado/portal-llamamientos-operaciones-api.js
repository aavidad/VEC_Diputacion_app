...
export async function crearLlamamientoCandidato(participacionRef, payload, { fetchImpl = fetch } = {}) {
  ...
  try {
    const respuesta = await fetchImpl(`/api/vec/bolsa/candidatos/${segmentoRuta(participacionRef)}/llamamientos`, { method: "POST", credentials: "omit", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(envelopeAccion) });
    if (!respuesta.ok) {
...
export async function registrarResultadoLlamamiento(llamamientoRef, payload, { fetchImpl = fetch } = {}) {
  ...
  try {
    const respuesta = await fetchImpl(`/api/vec/bolsa/llamamientos/${segmentoRuta(llamamientoRef)}/resultado`, { method: "POST", credentials: "omit", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(envelopeAccion) });
    if (!respuesta.ok) {
