...
const versionDietasVista = "20260924-osm-base-v1";
const versionDietasEstilos = "20260924-dietas-ayuda-icono-v1";
...
test("una carga con caché caliente solicita CSS F2 y entrada JS con URL nueva", async () => {
  ...
  for (const [recurso, versionAntigua] of previo) {
    assert.equal(versionDe(html, `/portal-empleado/${recurso}`),
      recurso === "portal.js" ? versionDietasShell : recurso === "portal.css" ? versionSaltoMovil : recurso === "modulos/dietas/dietas.css" ? versionDietasEstilos : version);
...
test("la caché immutable previa no retiene el catálogo i18n ni los consumidores F2", async () => {
  const versionesPrevias = new Map([
    ...
    ["modulos/dietas/vista-recorridos.js", [version, "20260924-f2-dietas-consulta-v2", "20260924-dietas-recuperacion-v3"]],
