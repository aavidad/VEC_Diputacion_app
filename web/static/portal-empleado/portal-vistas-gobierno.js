...
export function crearVistasGobierno(u) {
  ...
  function renderizarEstadisticas(datos) {
    ...
    return `
      ...
      <section class="panel panel-separado"><div class="cabecera-panel"><h3>Definir exportación</h3><span class="estado-chip violeta">Finalidad obligatoria</span></div><form class="cuerpo-panel formulario-gobernado"><fieldset><legend>Contenido y formato</legend><div class="rejilla-formulario">${campo("Informe", '<select><option>Cobertura y tiempos</option><option>Actividad de llamamientos</option><option>Estado de convocatorias</option></select>')}${campo("Ámbito", '<select><option>Datos agregados</option><option>Expedientes autorizados</option></select>')}${campo("Formato", '<select><option>PDF/A</option><option>CSV</option><option>ODS</option><option>JSON</option></select>')}${campo("Finalidad", '<select><option>Seguimiento de gestión</option><option>Transparencia</option><option>Auditoría</option></select>')}</div></fieldset>${botonOperacion("Preparar exportación DEMO", "exportar-informe", "DEMO-INF-ESTADISTICAS", "boton-primario")}</form></section>`;
  }
  ...
  function renderizarConfiguracion(datos) {
    ...
    return `
      ...
      <div class="rejilla-dos-columnas panel-separado"><section class="panel"><div class="cabecera-panel"><h3>Propuesta de rol</h3><span class="estado-chip violeta">Sin activar</span></div><form class="cuerpo-panel formulario-gobernado"><fieldset><legend>Alcance mínimo</legend><div class="rejilla-formulario">${campo("Nombre", '<input value="Revisor DEMO de admisión">')}${campo("Unidad", '<select><option>Unidad DEMO de Selección</option></select>')}${campo("Recursos", '<select><option>Convocatorias asignadas</option><option>Expedientes asignados</option></select>')}${campo("Acciones", '<select><option>Consultar y proponer</option><option>Solo consultar</option></select>')}${campo("Vigencia desde", '<input type="date" value="2026-07-21">')}${campo("Revisión obligatoria", '<select><option>90 días</option><option>180 días</option></select>')}</div></fieldset>${botonOperacion("Guardar propuesta DEMO", "crear-rol", "DEMO-ROL-NUEVO", "boton-primario")}</form></section><aside class="nota-pendiente">${botonBloqueado("Activar rol en directorio", "La demo no dispone de identidad corporativa, Kerberos ni aprobación de la persona responsable de seguridad.")}</aside></div>`;
  }
