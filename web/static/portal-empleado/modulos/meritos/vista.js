...
function panel(titulo, subtitulo, cuerpo, extra = "") {
  return `<section class="panel meritos-panel"><header class="cabecera-panel"><div><h3>${escapar(titulo)}</h3><p>${escapar(subtitulo)}</p></div>${extra}</header><div class="cuerpo-panel">${cuerpo}</div></section>`;
}
...
function inventario(datos, filtro, t) {
  const visibles = datos.meritos.filter((item) => filtro === "todos" || item.estado === filtro);
  ...
  const filas = visibles.map((item) => `<tr><th scope="row">${texto(item.nombre, t("sin_dato"))}<small>${texto(item.tipo, t("sin_dato"))}</small></th><td>${texto(item.fuente, t("aviso_sin_fuente"))}</td><td>${texto(item.evidencia, t("aviso_sin_evidencia"))}</td><td>${texto(item.vigencia, t("aviso_sin_vigencia"))}</td><td>${pastilla(item.estado, ESTADOS_HECHO, t)}</td></tr>`);
  return panel(t("inventario"), t("inventario_ayuda"), `${controles}${visibles.length ? tabla(t("inventario"), ["nombre", "fuente", "evidencia", "vigencia", "estado"].map(t), filas) : `<p class="meritos-vacio">${escapar(t(datos.meritos.length ? "sin_resultados" : "vacio_texto"))}</p>`}${accion(t)}`);
}
...
export function renderizarMeritos(entrada = {}, { pestana = "inventario", filtro = "todos" } = {}) {
  ...
  const contenido = datos.estado === "disponible" ? { inventario: () => inventario(datos, filtroSeguro, t), requisitos: () => requisitos(datos, t), valoraciones: () => valoraciones(datos, t) }[actual]() : aviso;
...
export function montarVistaMeritos({ raiz, datos, anunciar = () => {}, registrarDesmontar } = {}) {
  ...
  let activa = true; let pestana = "inventario"; let filtro = "todos";
  const pintar = () => { if (activa && contenedor.parentElement === raiz) contenedor.innerHTML = renderizarMeritos(proyeccion, { pestana, filtro }); };
  pintar(); anunciar(t("anuncio_inicial"), "informacion");
  const alClick = (evento) => { const boton = evento.target?.closest?.("[data-meritos-vista]"); if (!boton || !contenedor.contains?.(boton) || !PESTANAS.includes(boton.dataset.meritosVista)) return; pestana = boton.dataset.meritosVista; filtro = "todos"; pintar(); contenedor.querySelector?.(`[data-meritos-vista="${pestana}"]`)?.focus?.(); anunciar(t("anuncio_seccion", { nombre: t(pestana) }), "informacion"); };
  const alCambio = (evento) => { if (!evento.target?.matches?.("[data-meritos-filtro]")) return; filtro = evento.target.value; pintar(); contenedor.querySelector?.("[data-meritos-filtro]")?.focus?.(); };
  ...
  const actualizar = (siguiente) => { if (!activa) return; proyeccion = normalizar(siguiente); filtro = "todos"; pintar(); anunciar(t("anuncio_actualizado"), "informacion"); };
