...
test("subsanación confirma solo el recibo de servidor", async () => {
  const eventos=new Map(); const raiz={innerHTML:"",ownerDocument:{body:{append(){}},createElement:()=>({click(){},remove(){}})},addEventListener:(n,f)=>eventos.set(n,f),removeEventListener(){},contains:()=>true,querySelector:()=>({focus(){}}),replaceChildren(){this.innerHTML="";}};
  const confirmaciones=[], orden=[]; let envios=0;
  const desmontar=montarFormularioSubsanacionReparos({raiz,contexto,traducir:(k)=>textos[k]??k,generarClaveIdempotencia:()=>"123e4567-e89b-42d3-a456-426614174000",confirmarOperacion:()=>true,alDenegacion:()=>{},alCambiarIntencion:(intencion)=>{orden.push(intencion?.incierta ?? null);return true;},alConfirmar:(reciboConfirmado, contextoOriginal)=>{orden.push("recibo");confirmaciones.push([reciboConfirmado, contextoOriginal]);},cliente:{registrarSubsanacionReparos:async()=>{envios+=1;return recibo;}}});
  eventos.get("submit")({target:{closest:()=>({elements:{namedItem:()=>({value:"Corrección del reparo."})}})},preventDefault(){}});
  assert.equal(envios,0); assert.match(raiz.innerHTML,/Descargar archivo de recuperación/u);
...
test("remonta el recibo confirmado con su contexto original sin habilitar otro envío", () => {
  ...
  const desmontar=montarFormularioSubsanacionReparos({raiz,contexto:{ expediente_ref: contexto.expediente_ref, version_esperada: 7 },reciboConfirmado:{ recibo, contexto },traducir:(k)=>textos[k]??k,alDenegacion:()=>{},alCambiarIntencion:()=>true,cliente:{registrarSubsanacionReparos:()=>{llamadas+=1;}}});
