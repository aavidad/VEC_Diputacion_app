...
function escenario(respuestas, confirmar = () => true) {
  const eventos = new Map(), peticiones = [], confirmaciones = [], anuncios = [];
  ...
  const raiz = {
    ...
    set innerHTML(valor) {
      ...
      html = valor;
      nodos.clear();
      for (const selector of ["[data-ct-subsanacion-form]", "[data-ct-subsanacion-recuperar]", "[data-ct-subsanacion-recibo]", "[data-ct-subsanacion-form] textarea"]) {
  ...
  return {
    raiz, documento, peticiones, confirmaciones, anuncios, desmontar,
    enviar: () => {
