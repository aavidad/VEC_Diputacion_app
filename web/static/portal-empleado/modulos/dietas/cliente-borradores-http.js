...
function validarSolicitud(entrada) {
  const campos = ["clave_idempotencia", "fecha_inicio", "fecha_fin", "motivo", "codigos_ruta", "relacion_ref"];
...
function validarComision(comision) {
  const campos = ["referencia", "estado", "fecha_inicio", "fecha_fin", "motivo", "codigos_ruta", "relacion_ref"];
  ...
  if (!Array.isArray(codigos) || codigos.length > 16 || !codigos.every((codigo) => typeof codigo === "string" && /^[A-Za-z0-9:_-]{1,64}$/u.test(codigo)) || new Set(codigos).size !== codigos.length || codigos.some((codigo, indice) => indice > 0 && codigos[indice - 1] >= codigo)) throw new TypeError("comisión de Dietas incompatible");
  return Object.freeze({ ...comision, codigos_ruta: Object.freeze([...codigos]) });
}
