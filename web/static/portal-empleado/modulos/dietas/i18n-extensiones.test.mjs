...
test("la consulta y su ayuda están traducidas y sus imports no usan la versión anterior", async () => {
  ...
  assert.match(borradores, /i18n\.js\?v=20260924-web-paradas-periodos-v1/u);
  assert.match(borradores, /i18n-borradores\.js\?v=20260924-web-paradas-periodos-v1/u);
  assert.match(recorridos, /vista-borradores-propios\.js\?v=20260924-web-paradas-periodos-v1/u);
...
test("D1 y D4 están en el catálogo común, con sus importaciones de versión renovadas", async () => {
  ...
  assert.match(recorridos, /i18n\.js\?v=20260924-web-paradas-periodos-v1/u);
  assert.match(recorridos, /vista-acceso-papeles\.js\?v=20260924-web-paradas-periodos-v1/u);
  assert.match(papeles, /i18n-d1\.js\?v=20260924-web-paradas-periodos-v1/u);
  assert.match(itinerario, /i18n\.js\?v=20260924-web-paradas-periodos-v1/u);
  assert.match(itinerario, /i18n-d4\.js\?v=20260924-web-paradas-periodos-v1/u);
  for (const fuente of [d1, d4])
    assert.match(fuente, /i18n\.js\?v=20260924-web-paradas-periodos-v1/u);
