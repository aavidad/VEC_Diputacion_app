...
export function renderizarRecorridosCronos({ mensajes = MENSAJES_CRONOS_ES } = {}) {
  ...
  const formulario = `<div class="cronos-alta" data-cronos-alta-panel hidden><form class="cronos-recorrido-formulario"><label>${t("presentacion_form_tipo")}<select><option>${t("presentacion_form_vacaciones")}</option><option>${t("presentacion_form_correccion")}</option></select></label><label>${t("presentacion_form_periodo")}<input type="text" value="05–09/10/2026"></label><label>${t("presentacion_form_observacion")}<textarea></textarea></label><button type="button" class="boton-primario" disabled aria-disabled="true">${t("presentacion_form_continuar")}</button></form></div>`;
