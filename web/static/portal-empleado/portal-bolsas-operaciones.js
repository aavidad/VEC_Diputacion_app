...
export function crearControladorOperacionesSituacion({ estado, renderizar, recargar, consultarReglas = consultarReglasSituacion }) {
  async function cargar(modalFicha) {
    ...
    modalFicha.operacionesB8 = res.ok
      ? { ...modalFicha.operacionesB8, carga: "listo", noDisponible: false, items: res.datos, causasBaja: causas, transiciones }
      : { ...modalFicha.operacionesB8, carga: "error", noDisponible: res.status === 404, error: res.mensaje, items: [], causasBaja: causas, transiciones };
    renderizar();
  }
