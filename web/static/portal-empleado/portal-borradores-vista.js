...
export function crearRenderizadorBorradores({
    ...
    if (estado.faseLista === FASE_CARGANDO || estado.faseLista === FASE_INICIAL) {
      return `
        <section class="panel borradores-cargando" role="status" aria-live="polite">
          <div class="cuerpo-panel"><strong>Comprobando acceso y cargando borradores…</strong><p>La credencial se solicita al proveedor en memoria y no se usan cookies ni almacenamiento local.</p></div>
        </section>`;
    }
  ...
  function renderizar() {
    const cabecera = `
      ...
      <section class="nota-seguridad" aria-label="Tratamiento del borrador">La credencial procede únicamente del proveedor inyectado y vive en memoria. No se usan cookies, almacenamiento local ni datos de presentación para esta bandeja.</section>`;
