...
func TestCotejoMemoriaReservaEsIdempotenteExpiraYConfirmaSinParcialidad(t *testing.T) {
	...
	if err != nil || primera.Token == "" || primera.Repetida {
	...
	if err != nil || segunda.Token == "" || segunda.Token == primera.Token {
		t.Fatalf("reserva tras expiracion = %+v, %v", segunda, err)
	}
	if _, existe := store.reservasCotejoToken[primera.Token]; existe {
	...
	if _, existe := store.reservasCotejoToken[segunda.Token]; !existe {
	...
	if err != nil || !repetida.Repetida || repetida.Token != "" || !reflect.DeepEqual(repetida.Codigo, codigo) {
