...
type registroMemoria struct {
	...
	sesiones         map[string]AltaSesionAtomica
...
func nuevoRegistroMemoria() *registroMemoria {
	return &registroMemoria{
		asercionesUsadas: make(map[string]struct{}),
		sesiones:         make(map[string]AltaSesionAtomica),
		revocadas:        make(map[string]struct{}),
		cuentasInactivas: make(map[string]struct{}),
	}
}

func (r *registroMemoria) ConsumirAsercionYRegistrar(_ context.Context, alta AltaSesionAtomica) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if _, usada := r.asercionesUsadas[alta.AsercionID]; usada {
		return errors.New("reproduccion")
	}
	...
	if _, existe := r.sesiones[alta.SesionID]; existe {
		return errors.New("sesion duplicada")
	}
	r.asercionesUsadas[alta.AsercionID] = struct{}{}
	r.sesiones[alta.SesionID] = alta
	return nil
}
...
func (r *registroMemoria) ComprobarSesionYCuentaActivas(_ context.Context, consulta ConsultaSesionActiva) error {
	...
	alta, existe := r.sesiones[consulta.SesionID]
	if !existe || alta.AsercionID != consulta.AsercionID || alta.SujetoID != consulta.SujetoID ||
		alta.CuentaID != consulta.CuentaID || alta.CuentaOrdinariaID != consulta.CuentaOrdinariaID ||
		alta.CuentaPrivilegiada != consulta.CuentaPrivilegiada || alta.Superficie != consulta.Superficie ||
		alta.ExpiraEn != consulta.ExpiraEn {
