...
const (
	longitudMinimaTokenContextoActorV1 = 24
	longitudMaximaTokenContextoActorV1 = 128
)

// GeneradorOperacionContextoActorV1 crea una referencia nueva antes de entrar
...
func (s SolicitudResolucionRegistroContextoActorV1) Validar() error {
	if !referenciaRegistroContextoActorV1Valida(s.OperacionRef, "oca_") ||
		s.Contexto.Validar() != nil || !instanteRegistroContextoActorV1Canonico(s.SolicitadoEn) {
		return ErrSolicitudRegistroContextoActorV1Invalida
	}
	return nil
}
...
func (c ConfirmacionRegistroContextoActorV1) ValidarPara(
	...
	if solicitud.Validar() != nil || c.OperacionRef != solicitud.OperacionRef ||
		!referenciaRegistroContextoActorV1Valida(c.RegistroContextoRef, "rca_") ||
		c.Contexto.Validar() != nil ||
		!instanteRegistroContextoActorV1Canonico(c.ResueltoEnAutoritativo) ||
		...
		return ErrConfirmacionRegistroContextoActorV1Invalida
	}
	representacion, err := c.Contexto.RepresentacionCanonicaVinculadaV1()
...
// confirmar en la misma transaccion. Antes del commit debe conservar los bytes de
// RepresentacionCanonicaVinculadaV1 y su huella SHA-256, ligados a la version
// resuelta. Una espera que alcance caducidad o exceda la ventana debe abortar.
//
...
func referenciaRegistroContextoActorV1Valida(valor, prefijo string) bool {
	if len(valor) < len(prefijo)+longitudMinimaTokenContextoActorV1 ||
		len(valor) > len(prefijo)+longitudMaximaTokenContextoActorV1 ||
...
func instanteRegistroContextoActorV1Canonico(instante time.Time) bool {
