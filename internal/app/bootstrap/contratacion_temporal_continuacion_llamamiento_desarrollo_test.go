...
func (r *registroContinuacionCoordinadorPrueba) LeerAntecedente(ctx context.Context, s ports.SolicitudContinuarLlamamiento) (ports.AntecedenteContinuacionLlamamiento, error) {
	...
	if err != nil {
		return ports.AntecedenteContinuacionLlamamiento{}, err
	}
...
func (r *registroContinuacionCoordinadorPrueba) Confirmar(ctx context.Context, s ports.SolicitudContinuarLlamamiento, b ports.ReciboBolsaContinuacion) (ports.ResultadoContinuacionLlamamiento, error) {
	...
	if err != nil {
		return ports.ResultadoContinuacionLlamamiento{}, err
	}
	...
	if r.fallarConfirmacion {
		return ports.ResultadoContinuacionLlamamiento{}, ports.ErrOperacionContinuacionNoDisponible
	}
	if r.confirmado.ReciboRef != "" {
		if r.confirmado.Solicitud != s || r.confirmado.ReciboBolsa != b {
			r.t.Fatal("replay cambió material")
		}
		copia := r.confirmado
...
func (l *lectorJustificanteContinuacionPrueba) ConsultarJustificanteRespuestaRecibida(ctx context.Context, s ports.SolicitudResolverLlamamiento) (ports.JustificanteRespuestaRecibida, error) {
	if _, err := l.proveedor.AutorizarConsultaJustificanteRespuestaRecibida(ctx, s); err != nil {
		return ports.JustificanteRespuestaRecibida{}, err
	}
...
func escenarioContinuacionCoordinadorPrueba(t *testing.T) escenarioContinuacionCoordinador {
	...
	if seleccion.OperacionRef != l.justificante.Seleccion.OperacionRef || seleccion.Propuesta != l.justificante.Seleccion.Propuesta {
	...
	if l.justificante.ValidarPara(l.solicitud) != nil {
		t.Fatal("justificante de renuncia inválido")
	}
	...
	if a.ValidarPara(s) != nil {
		t.Fatal("antecedente de continuación inválido")
	}
...
func TestContinuacionLlamamientoDesarrolloCorteCTYReplayAutorizado(t *testing.T) {
	...
	for intento := 0; intento < 3; intento++ {
		r, err := f.ejecutor.Continuar(f.ctx, f.solicitud)
		if intento == 0 {
			if !errors.Is(err, ports.ErrOperacionContinuacionNoDisponible) || r != (ports.ResultadoContinuacionLlamamiento{}) || f.registro.confirmado.ReciboRef != "" {
				t.Fatal("éxito parcial tras fallo CT", err)
			}
		} else {
			if err != nil || r.ValidarPara(f.solicitud) != nil || r.Estado != []string{"", "confirmado", "replay_confirmado"}[intento] {
				t.Fatal("recuperación falló", err)
			}
			if intento == 1 {
				primero = r
			} else if r.ReciboRef != primero.ReciboRef || r.ConfirmadaEn != primero.ConfirmadaEn || r.ReciboBolsa != primero.ReciboBolsa {
				t.Fatal("replay perdió recibo o fecha")
			}
		}
		if len(f.registro.bolsa.filas) != 4 || len(f.registro.recibidos) != intento+1 {
			t.Fatal("efectos omitidos o duplicados")
		}
		if f.registro.recibidos[intento] != f.registro.recibidos[0] {
			t.Fatal("reintento cambió operación o recibo Bolsa")
		}
	...
	if len(f.autoridad.etapas) != 12 {
		t.Fatal("falta autorización fresca", f.autoridad.etapas)
	}
	for i := 0; i < 3; i++ {
		if !reflect.DeepEqual(f.autoridad.etapas[4*i:4*i+4], secuencia) {
			t.Fatal("permisos omitidos o desordenados", f.autoridad.etapas)
		}
		if f.autoridad.huellas[4*i] == f.autoridad.huellas[4*i+3] {
			t.Fatal("lectura y confirmación comparten material")
		}
	}
	if f.lector.llamadas != 3 || f.registro.antecedente.Resolucion.IntencionSiguiente.Estado != ports.OutboxSiguienteCandidatoPendiente {
		t.Fatal("se omitió relectura o se alteró la renuncia original")
	}
