...
func nuevoMultiplexoresLectoresRRHHDesarrollo(sello *selloConsultasContratacionTemporalDesarrollo) *multiplexoresLectoresRRHHDesarrollo {
	return &multiplexoresLectoresRRHHDesarrollo{cuadros: make(map[string]httpinterno.ConsultorCuadroRRHH), detalles: make(map[string]httpinterno.ConsultorDetalleRRHH), soportes: make(map[string]*soporteAltaContratacionTemporalDesarrollo), sello: sello}
}

func (m *multiplexoresLectoresRRHHDesarrollo) registrar(principal string, soporte *soporteAltaContratacionTemporalDesarrollo, cuadro httpinterno.ConsultorCuadroRRHH, detalle httpinterno.ConsultorDetalleRRHH) bool {
	if m == nil || principal == "" || soporte == nil || soporte.sello != m.sello || cuadro == nil || detalle == nil || m.cuadros[principal] != nil {
		return false
	}
	m.cuadros[principal], m.detalles[principal], m.soportes[principal] = cuadro, detalle, soporte
	return true
}
