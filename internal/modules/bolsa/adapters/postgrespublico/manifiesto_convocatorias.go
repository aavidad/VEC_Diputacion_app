...
func leerEntradasManifiestoEnFlujo(
	...
	defer filas.Close()
	resultado := make([]canonicopublico.ConvocatoriaManifiestoPublicoV1, 0, 256)
	bytesLeidos := 0
	for filas.Next() {
		...
		resultado = append(resultado, canonicopublico.ConvocatoriaManifiestoPublicoV1{
...
func escanearConvocatoriaManifiesto(
	...
	bytesEscalares := len(convocatoria.Version) + len(estado) + len(datos.IdentificadorPublico) +
		...
		len(datos.CatalogoCategorias.CatalogoHuellaSHA256) + len(convocatoria.HuellaSHA256) +
