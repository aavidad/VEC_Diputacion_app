...
func TestIntegracionLlamamientosDesarrolloCanonAceptacionYLigaduras(t *testing.T) {
	...
	r.OperacionRef, r.Tipo = "operacion:aceptacion", "aceptacion_rrhh"
	r.Llamamiento.Version, r.EstadoLlamamiento = 2, domain.EstadoLlamamientoAceptado
	canon, err := r.Canonico()
	if err != nil || !bytes.Contains(canon, []byte(`"resolucion":`)) || r.Accion() != AccionAceptarLlamamientoRRHHDesarrollo {
		t.Fatalf("aceptación válida rechazada: %v", err)
	}
