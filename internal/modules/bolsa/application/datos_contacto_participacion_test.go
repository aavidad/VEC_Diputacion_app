...
// cifradorContactoPrueba invierte los bytes: suficiente para comprobar que el
// claro nunca llega al repositorio y que la lectura descifra lo guardado.
type cifradorContactoPrueba struct{ cifrados, descifrados int }

func invertir(b []byte) []byte {
	r := make([]byte, len(b))
	for i := range b {
		r[len(b)-1-i] = b[i]
	}
...
func (c *cifradorContactoPrueba) CifrarDatosContactoParticipacion(_ context.Context, ref string, version uint64, claro []byte) (puertosbolsa.SobreDatosContacto, error) {
	c.cifrados++
	return puertosbolsa.SobreDatosContacto{Version: version, ClaveRef: "clave:prueba", Nonce: []byte(ref), Cifrado: invertir(claro)}, nil
}

func (c *cifradorContactoPrueba) ConDatosContactoParticipacionDescifrados(_ context.Context, ref string, sobre puertosbolsa.SobreDatosContacto, usar func([]byte) error) error {
	c.descifrados++
	if !bytes.Equal(sobre.Nonce, []byte(ref)) {
		return errors.New("sobre de otra participación")
	}
	return usar(invertir(sobre.Cifrado))
}

type repositorioContactoPrueba struct {
	registros []puertosbolsa.RegistroDatosContactoParticipacion
	llamadas  int
	ultimo    puertosbolsa.ComandoRegistrarDatosContactoParticipacion
}

func (r *repositorioContactoPrueba) DatosContactoVigentes(_ context.Context, ref string) (puertosbolsa.RegistroDatosContactoParticipacion, error) {
...
func (r *repositorioContactoPrueba) BuscarRegistroDatosContacto(_ context.Context, ref, clave string) (puertosbolsa.RegistroDatosContactoParticipacion, error) {
	for _, registro := range r.registros {
		if registro.ParticipacionRef == ref && registro.ReciboRef == reciboContactoPrueba(ref, clave) {
			return registro, nil
		}
	}
	return puertosbolsa.RegistroDatosContactoParticipacion{}, puertosbolsa.ErrDatosContactoParticipacionNoEncontrados
...
func (r *repositorioContactoPrueba) RegistrarDatosContacto(_ context.Context, comando puertosbolsa.ComandoRegistrarDatosContactoParticipacion) (puertosbolsa.RegistroDatosContactoParticipacion, error) {
	r.llamadas++
	r.ultimo = comando
...
// reciboContactoPrueba reproduce el recibo determinista del servicio.
func reciboContactoPrueba(ref, clave string) string {
	h := sha256.Sum256([]byte(ref + "\x1f" + clave))
	return "recibo:datos-contacto:" + hex.EncodeToString(h[:])
}

type pertenenciaContactoPrueba struct{ pertenece bool }

func (p pertenenciaContactoPrueba) ParticipacionPerteneceABolsa(context.Context, string, string) (bool, error) {
	return p.pertenece, nil
}

func solicitudContactoPrueba(t *testing.T, ahora time.Time, clave string, datos dominiobolsa.DatosContactoParticipacion) puertosbolsa.SolicitudRegistrarDatosContactoParticipacion {
	t.Helper()
...
func datosContactoPrueba() dominiobolsa.DatosContactoParticipacion {
...
func servicioContactoPrueba(t *testing.T, ahora time.Time, repo *repositorioContactoPrueba, cifrador *cifradorContactoPrueba, contexto puertosbolsa.ResolutorContextoSituacionParticipacion, pertenece bool) (*ServicioDatosContactoParticipacion, *autorizadorBorradorPrueba) {
	t.Helper()
	autorizador := &autorizadorBorradorPrueba{t: t, instante: ahora}
	servicio, err := NuevoServicioDatosContactoParticipacion(contexto, autorizador, pertenenciaContactoPrueba{pertenece}, cifrador, repo, func() time.Time { return ahora })
	if err != nil {
		t.Fatal(err)
	}
...
func TestServicioContactoRegistraCifradoYConsultaDescifrado(t *testing.T) {
	...
	cifrador := &cifradorContactoPrueba{}
	servicio, autorizador := servicioContactoPrueba(t, ahora, repo, cifrador, contextoSituacionPrueba{}, true)
	registro, err := servicio.Registrar(context.Background(), solicitudContactoPrueba(t, ahora, "b4-alta-0001", datosContactoPrueba()))
...
func TestServicioContactoRepiteSinVersionNuevaYRechazaOtrosDatos(t *testing.T) {
	...
	repo := &repositorioContactoPrueba{}
	cifrador := &cifradorContactoPrueba{}
	servicio, _ := servicioContactoPrueba(t, ahora, repo, cifrador, contextoSituacionPrueba{}, true)
	primero, err := servicio.Registrar(context.Background(), solicitudContactoPrueba(t, ahora, "b4-alta-0001", datosContactoPrueba()))
	if err != nil {
		t.Fatal(err)
	}
	repetido, err := servicio.Registrar(context.Background(), solicitudContactoPrueba(t, ahora, "b4-alta-0001", datosContactoPrueba()))
	...
	otros := datosContactoPrueba()
	otros.Telefono1 = "611111111"
	if _, err = servicio.Registrar(context.Background(), solicitudContactoPrueba(t, ahora, "b4-alta-0001", otros)); !errors.Is(err, dominiobolsa.ErrDatosContactoParticipacionInvalidos) {
		t.Fatalf("misma clave con otros datos debe rechazarse: %v", err)
	}
	segundo, err := servicio.Registrar(context.Background(), solicitudContactoPrueba(t, ahora, "b4-alta-0002", otros))
...
func TestServicioContactoDeniegaSinAmbitoOParticipacionAjenaAntesDeCifrar(t *testing.T) {
	...
	repo := &repositorioContactoPrueba{}
	cifrador := &cifradorContactoPrueba{}
	servicio, autorizador := servicioContactoPrueba(t, ahora, repo, cifrador, contextoSituacionPrueba{err: dominiovec.ErrAutorizacionDenegada}, true)
	if _, err := servicio.Registrar(context.Background(), solicitudContactoPrueba(t, ahora, "b4-alta-0001", datosContactoPrueba())); !errors.Is(err, dominiovec.ErrAutorizacionDenegada) || autorizador.llamadas != 0 || cifrador.cifrados != 0 || repo.llamadas != 0 {
		t.Fatalf("sin ámbito: err=%v auth=%d cifrados=%d repo=%d", err, autorizador.llamadas, cifrador.cifrados, repo.llamadas)
	}
	servicio, autorizador = servicioContactoPrueba(t, ahora, repo, cifrador, contextoSituacionPrueba{}, false)
	if _, err := servicio.Registrar(context.Background(), solicitudContactoPrueba(t, ahora, "b4-alta-0001", datosContactoPrueba())); !errors.Is(err, dominiovec.ErrAutorizacionDenegada) || autorizador.llamadas != 0 || cifrador.cifrados != 0 {
	...
	invalidos := datosContactoPrueba()
	invalidos.Correo, invalidos.Telefono1, invalidos.Telefono2 = "", "", ""
	if _, err := servicio.Registrar(context.Background(), solicitudContactoPrueba(t, ahora, "b4-alta-0009", invalidos)); !errors.Is(err, dominiobolsa.ErrDatosContactoParticipacionInvalidos) {
