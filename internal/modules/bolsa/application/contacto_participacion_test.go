...
func (r *repositorioContactoPrueba) ListarContactosBolsa(context.Context, string, string, int) (puertosbolsa.PaginaContactosParticipacion, error) {
	r.lecturasBolsa++
	return puertosbolsa.PaginaContactosParticipacion{}, nil
}

func resultadoContextoContactoPrueba(t *testing.T) dominiovec.ResultadoContextoActorRegistradoV2 {
	t.Helper()
	resultado, _, err := pruebas.NuevoContextoRegistradoYVinculoV2(time.Date(2026, 9, 22, 8, 0, 0, 0, time.UTC), "per_0123456789abcdefghijkl", "prf_0123456789abcdefghijkl", dominiovec.AuthMethodCertificate, dominiovec.AuthAssuranceHigh)
	if err != nil {
		t.Fatal(err)
	}
	return resultado
...
func TestServicioContactoDeniegaLecturasFueraDeAmbitoAntesDelRepositorio(t *testing.T) {
	...
	resultado := resultadoContextoContactoPrueba(t)
	_, errParticipacion := servicio.ListarContactosParticipacion(context.Background(), puertosbolsa.ConsultaContactosParticipacion{ResultadoContexto: resultado, BolsaRef: "bolsa:ajena", ParticipacionRef: "participacion:01", Limite: 20})
	_, errBolsa := servicio.ListarContactosBolsa(context.Background(), puertosbolsa.ConsultaContactosBolsa{ResultadoContexto: resultado, BolsaRef: "bolsa:ajena", Limite: 20})
...
func TestServicioContactoLeeSoloTrasResolverElAmbito(t *testing.T) {
	repo := &repositorioContactoPrueba{}
	servicio, err := NuevoServicioContactoParticipacion(contextoContactoPrueba{}, &autorizadorBorradorPrueba{}, repo)
	if err != nil {
		t.Fatal(err)
	}
	resultado := resultadoContextoContactoPrueba(t)
	if _, err = servicio.ListarContactosParticipacion(context.Background(), puertosbolsa.ConsultaContactosParticipacion{ResultadoContexto: resultado, BolsaRef: "bolsa:permitida", ParticipacionRef: "participacion:01", Limite: 20}); err != nil || repo.lecturasParticipacion != 1 {
