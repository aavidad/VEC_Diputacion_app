...
type renderizadorInformeRRHHPrueba struct {
	contenido    []byte
	err          error
	llamadas     int
...
func (r *renderizadorInformeRRHHPrueba) RenderizarInforme(ctx context.Context, d ports.DetalleExpedienteRRHH) ([]byte, error) {
	r.llamadas++
...
func TestConsultaRRHHInformeDefinitivoDescargaTrasLectura(t *testing.T) {
	c := &consultorDetalleRRHHPrueba{detalle: detalleInformeRRHHPrueba()}
	r := &renderizadorInformeRRHHPrueba{contenido: []byte("%PDF-1.4\nfixture transporte\n%%EOF\n")}
	r.alRenderizar = func(_ context.Context, d ports.DetalleExpedienteRRHH) {
		if c.llamadas != 1 || d.Resumen.Version != 7 || d.Resumen.ExpedienteRef != c.solicitud.ExpedienteRef() {
			t.Fatal("render sin lectura previa o detalle distinto")
		}
	...
	h.ServeHTTP(w, peticionInformeRRHHPrueba())
...
func TestConsultaRRHHInformeDefinitivoJSONYConstructor(t *testing.T) {
	c := &consultorDetalleRRHHPrueba{detalle: detalleInformeRRHHPrueba()}
	r := &renderizadorInformeRRHHPrueba{}
	var nulo *renderizadorInformeRRHHPrueba
	for _, lista := range [][]ports.RenderizadorInformeDefinitivoRRHH{{nil}, {nulo}, {r, r}} {
	...
	for _, lista := range [][]ports.RenderizadorInformeDefinitivoRRHH{nil, {r}} {
...
func TestConsultaRRHHInformeDefinitivoRechazosAntesDeRender(t *testing.T) {
	...
	peticion.Header.Set("Accept", AcceptInformeDefinitivoRRHH)
	w := httptest.NewRecorder()
	h.ServeHTTP(w, peticion)
...
func TestConsultaRRHHInformeDefinitivoFallosSinPDFParcial(t *testing.T) {
	for _, caso := range []struct {
		nombre    string
		contenido []byte
		err       error
		...
		{"precondición", nil, ports.ErrInformeDefinitivoRRHHNoDisponible, 409},
		{"error render", []byte("%PDF-parcial"), errors.New("detalle interno no publicable"), 500},
		{"no PDF", []byte("no es PDF"), nil, 502},
		{"exceso tamaño", append([]byte("%PDF-"), make([]byte, MaximoPDFInformeDefinitivoRRHHBytes)...), nil, 502},
	} {
		t.Run(caso.nombre, func(t *testing.T) {
			c := &consultorDetalleRRHHPrueba{detalle: detalleInformeRRHHPrueba()}
			r := &renderizadorInformeRRHHPrueba{contenido: caso.contenido, err: caso.err}
			h, err := NuevoManejadorConsultaDetalleRRHH(c, r)
			if err != nil {
				t.Fatal(err)
			}
...
func TestConsultaRRHHInformeDefinitivoCancelacion(t *testing.T) {
	for _, duranteRender := range []bool{false, true} {
		...
		w := httptest.NewRecorder()
		h.ServeHTTP(w, peticionInformeRRHHPrueba().WithContext(ctx))
		cancelar()
