...
func TestConsultaRRHHBorradorDescargaTrasLectura(t *testing.T) {
	for _, caso := range []struct {
		...
		{ports.BorradorComunicacionCentro, AcceptComunicacionCentroRRHH, "comunicacion-centro-borrador.pdf"},
		{ports.BorradorContratoLaboral, AcceptContratoLaboralRRHH, "contrato-laboral-borrador.pdf"},
		{ports.BorradorNombramiento, AcceptNombramientoRRHH, "nombramiento-borrador.pdf"},
		{ports.BorradorCese, AcceptCeseRRHH, "cese-borrador.pdf"},
		{ports.BorradorModificacionNombramiento, AcceptModificacionNombramientoRRHH, "modificacion-nombramiento-borrador.pdf"},
	} {
...
func TestConsultaRRHHBorradorRechazosAntesDeRender(t *testing.T) {
	for _, caso := range []struct {
		...
		{"toma de posesión sin permiso", func(r *http.Request, c *consultorDetalleRRHHPrueba) {
		...
		{"comunicación al centro sin permiso", func(r *http.Request, c *consultorDetalleRRHHPrueba) {
			r.Header.Set("Accept", AcceptComunicacionCentroRRHH)
			c.err = application.ErrConsultaRRHHNoObservable
		}, 404, 1},
	...
	for _, accept := range []string{AcceptInformeDefinitivoRRHH, AcceptResolucionRRHH, AcceptDiligenciaRRHH, AcceptTomaPosesionRRHH, AcceptNotificacionRRHH, AcceptComunicacionCentroRRHH} {
