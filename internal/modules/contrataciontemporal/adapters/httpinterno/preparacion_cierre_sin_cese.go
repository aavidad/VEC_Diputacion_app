...
func NuevoManejadorPreparacionCierreSinCese(a AutoridadServidorCierreAdministrativo, e ports.LectorPreparacionCierreAdministrativo) (http.Handler, error) {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !rutaPreparacionCierreSinCeseExacta(r) {
			responderErrorCierreAdministrativo(w, r, nuevoErrorCierreAdministrativo(http.StatusNotFound, "recurso_no_encontrado"))
			return
		}
		if r.Method != http.MethodGet {
			w.Header().Set("Allow", http.MethodGet)
			responderErrorCierreAdministrativo(w, r, nuevoErrorCierreAdministrativo(http.StatusMethodNotAllowed, "metodo_no_permitido"))
			return
		}
		expediente, seguimiento, ok := consultaPreparacionCierreSinCese(r)
		if !ok {
			responderErrorCierreAdministrativo(w, r, nuevoErrorCierreAdministrativo(http.StatusBadRequest, "peticion_no_valida"))
			return
		}
		organizacion, err := a.ResolverOrganizacionCierreAdministrativo(r.Context())
		if err != nil {
			responderErrorPreparacionCierreSinCese(w, r, err)
			return
		}
...
func responderErrorPreparacionCierreSinCese(w http.ResponseWriter, peticion *http.Request, err error) {
	if errors.Is(err, ErrContextoCanalAusente) || errors.Is(err, ErrContextoCanalCaducado) {
		responderErrorCierreAdministrativo(w, peticion, nuevoErrorCierreAdministrativo(http.StatusUnauthorized, "autenticacion_requerida"), err)
		return
	}
	if errors.Is(err, ErrContextoCanalOrganizacionDenegada) || errors.Is(err, ports.ErrAutorizacionDenegada) || errors.Is(err, ports.ErrCierreAdministrativoDenegado) {
		responderErrorCierreAdministrativo(w, peticion, errorAccesoCierreAdministrativoDenegado, err)
		return
	}
	responderErrorCierreAdministrativo(w, peticion, errorServicioCierreAdministrativoNoDisponible, err)
}
