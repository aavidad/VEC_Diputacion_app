...
func TestNoIncorporacionHTTPConSusConflictos(t *testing.T) {
	...
	cuerpo := `{"expediente_ref":"expediente:prueba","version_esperada":7,"clave_idempotencia":"44444444-4444-4444-8444-444444444444","motivo_clave":"no_presentado","resolucion_ref":"resolucion:rrhh:1","resolucion_sha256":"` +
		strings.Repeat("b", 64) + `","resuelta_por":"per_segunda","fecha_notificacion":"2026-09-20","observaciones":""}`
	w := peticionSeguimiento(t, m, RutaNoIncorporaciones, cuerpo)
	if w.Code != http.StatusCreated || e.no.MotivoClave != "no_presentado" || e.no.ResueltaPor != "per_segunda" ||
