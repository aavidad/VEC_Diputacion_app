...
func (r *RegistroPropuestaFormalizacionPostgreSQL) ejecutar(ctx context.Context, m ports.MaterialPropuestaFormalizacion, destino any, validar func() error) error {
  ...
  set_config('idle_in_transaction_session_timeout','20s',true)`); err != nil {
	...
	funcion := "vec_contratacion_temporal.registrar_propuesta_formalizacion_v1"
	if m.Solicitud.VersionEsperada > 6 {
		funcion = "vec_contratacion_temporal.registrar_propuesta_formalizacion_v2"
	}
	var salida string
	err = tx.QueryRow(ctx, `SELECT `+funcion+`(
