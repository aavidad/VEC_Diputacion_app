...
func TestEjecucionesSeleccionO6PostgreSQLGoATerminal(t *testing.T) {
	...
	consultaTerminal, instanteConsulta := consultaTerminalSeleccionO6Prueba(t)
	...
	ausente, confirmada, err := adaptador.ResolverTerminal(ctx, consultaTerminal, instanteConsulta)
	...
	if _, _, err = adaptador.ResolverTerminal(ctx, consultaTerminal, instanteConsulta); err == nil {
		t.Fatal("el resolver autorizado expuso una ejecucion no confirmada")
	}
	indeterminada, err := adaptador.ConsultarEstado(ctx, solicitud)
	if err != nil || indeterminada.Situacion != ports.EjecucionSeleccionLlamamientoIndeterminada ||
		indeterminada.EfectoPosible != ports.EfectoPrepararOrdenSeleccionLlamamiento {
		t.Fatalf("estado indeterminado divergente: estado=%#v err=%v", indeterminada, err)
	}
	borrarEjecucionSeleccionO6Integracion(t, context.Background(), admin, solicitud.ClaveIdempotencia)
	estado, err = adaptador.Reservar(ctx, solicitud)
	if err != nil || estado.Situacion != ports.EjecucionSeleccionLlamamientoPropietaria {
	...
	reserva.ReservaRef = estado.ReservaRef
	estado = probarLeaseYFencingSeleccionO6Integracion(t, ctx, admin, adaptador, reserva)
	reserva.ReservaRef = estado.ReservaRef
	if err = adaptador.AbrirVentanaEfecto(ctx, reserva,
		ports.EfectoPrepararOrdenSeleccionLlamamiento); err != nil {
		t.Fatal(err)
	...
	terminal, confirmada, err := reiniciado.ResolverTerminal(ctx, consultaTerminal, instanteConsulta)
	...
	probarReplayAutorizadoSeleccionO6Integracion(
		t, ctx, poolReinicio, consultaTerminal, instanteConsulta,
	)
	...
	if err = reiniciado.Confirmar(ctx, reserva, recibo, artefacto); err == nil {
		t.Fatal("la confirmacion terminal repitio el efecto")
	}
}

func probarReplayAutorizadoSeleccionO6Integracion(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	...
	base := consultaTerminalSeleccionO6{
	...
	mutaciones := []func(*consultaTerminalSeleccionO6){
		func(v *consultaTerminalSeleccionO6) { v.OrganizacionRef += "-otra" },
		func(v *consultaTerminalSeleccionO6) { v.ExpedienteRef += "-otro" },
		func(v *consultaTerminalSeleccionO6) { v.VersionExpediente++ },
		func(v *consultaTerminalSeleccionO6) { v.CorrelacionRef += "-otra" },
		func(v *consultaTerminalSeleccionO6) { v.AutoridadSolicitante += "-otra" },
		func(v *consultaTerminalSeleccionO6) { v.Autorizacion.Referencia += "-otra" },
		func(v *consultaTerminalSeleccionO6) { v.Accion.Referencia += "-otra" },
		func(v *consultaTerminalSeleccionO6) { v.Recurso.Referencia += "-otro" },
		func(v *consultaTerminalSeleccionO6) { v.Finalidad.Referencia += "-otra" },
	}
	consultar := func(texto string, codigo string) {
		t.Helper()
		var campos [6]string
		err := pool.QueryRow(ctx, `SELECT * FROM `+funcionResolverTerminalSeleccionO6+
			`($1::uuid,$2::text)`, datos.ClaveIdempotencia, texto).Scan(
			&campos[0], &campos[1], &campos[2], &campos[3], &campos[4], &campos[5],
		)
		var fallo *pgconn.PgError
		if !errors.As(err, &fallo) || fallo.Code != codigo {
			t.Fatalf("replay no fallo cerrado: codigo=%s err=%v", codigo, err)
		}
	}
	for _, mutar := range mutaciones {
		mutada := base
		mutar(&mutada)
		consultar(string(debeJSONSeleccionO6Prueba(t, mutada)), "42501")
	}
	canon := string(debeJSONSeleccionO6Prueba(t, base))
	consultar(strings.Replace(canon, "{", `{"organizacion_ref":"organizacion:otra",`, 1), "22023")
	consultar(strings.Replace(canon,
		`"organizacion_ref":"`+datos.OrganizacionRef+`",`, "", 1), "22023")
}

func probarLeaseYFencingSeleccionO6Integracion(
	t *testing.T,
	ctx context.Context,
	admin *pgxpool.Pool,
	adaptador *EjecucionesSeleccionLlamamientoPostgreSQL,
	anterior ports.ReservaEjecucionSeleccionLlamamiento,
) ports.EstadoEjecucionSeleccionLlamamiento {
	t.Helper()
	_, err := admin.Exec(ctx, `UPDATE vec_contratacion_temporal.ejecucion_seleccion_llamamiento_o6
		   SET creada_en = date_trunc('microseconds', statement_timestamp() - interval '40 seconds'),
		       actualizada_en = date_trunc('microseconds', statement_timestamp() - interval '31 seconds'),
		       lease_hasta = date_trunc('microseconds', statement_timestamp() - interval '1 second')
		 WHERE clave_idempotencia = $1::uuid AND reserva_ref = $2`,
		anterior.Solicitud.ClaveIdempotencia, anterior.ReservaRef)
	if err != nil {
		t.Fatal(err)
	}
	nueva, err := adaptador.Reservar(ctx, anterior.Solicitud)
	if err != nil || nueva.Situacion != ports.EjecucionSeleccionLlamamientoPropietaria ||
		nueva.ReservaRef == anterior.ReservaRef || !referenciaReservaSeleccionO6Valida(nueva.ReservaRef) {
		t.Fatalf("reasignacion tras lease invalida: estado=%#v err=%v", nueva, err)
	}
	if err = adaptador.AbrirVentanaEfecto(ctx, anterior,
		ports.EfectoPrepararOrdenSeleccionLlamamiento); err == nil {
		t.Fatal("una ejecucion vencida cruzo el fencing del propietario nuevo")
	}
	var version int64
	var microsegundos int64
	if err = admin.QueryRow(ctx, `SELECT fencing_version,
		(extract(epoch FROM lease_hasta - actualizada_en) * 1000000)::bigint
		FROM vec_contratacion_temporal.ejecucion_seleccion_llamamiento_o6
		WHERE clave_idempotencia=$1::uuid`, anterior.Solicitud.ClaveIdempotencia).Scan(
		&version, &microsegundos,
	); err != nil || version != 2 || microsegundos <= 0 || microsegundos > 30000000 {
		t.Fatalf("lease/fencing durable divergente: version=%d us=%d err=%v",
			version, microsegundos, err)
	}
	return nueva
}
...
func probarACLSeleccionO6Integracion(t *testing.T, ctx context.Context, admin *pgxpool.Pool) {
	...
	principales := map[string]bool{
		"resolver_terminal_autorizado_seleccion_llamamiento_o6_v2": true,
	...
	filas, err := admin.Query(ctx, `
		SELECT funcion.proname, funcion.prosecdef, propietario.rolname,
		 ...
		 WHERE funcion.pronamespace = 'vec_contratacion_temporal'::regnamespace
		   AND funcion.proname LIKE '%seleccion_llamamiento_o6_v%'`,
	...
	if err = filas.Err(); err != nil || conteo != 22 {
	...
	_, err = conexion.Exec(ctx, `SELECT
		vec_contratacion_temporal.resolver_terminal_autorizado_seleccion_llamamiento_o6_v2(
		'30000000-0000-4000-8000-000000000001'::uuid, '{}'::text)`)
	var falloPG *pgconn.PgError
