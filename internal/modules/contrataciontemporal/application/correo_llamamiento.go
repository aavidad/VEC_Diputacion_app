...
func NuevoServicioCorreoLlamamiento(a ports.AutorizadorDespachoCorreoLlamamiento, r ports.ResolutorDestinoCorreoLlamamiento, registro ports.RegistroIntentosCorreoLlamamiento, t ports.TransportadorCorreoLlamamiento, reloj ports.Reloj) (*ServicioCorreoLlamamiento, error) {
	if dependenciaNula(a) || dependenciaNula(r) || dependenciaNula(registro) || dependenciaNula(t) || dependenciaNula(reloj) {
		return nil, ErrServicioCorreoLlamamientoInvalido
	}
	return &ServicioCorreoLlamamiento{a, r, registro, t, reloj}, nil
}
...
func (s *ServicioCorreoLlamamiento) Despachar(ctx context.Context, solicitud ports.SolicitudDespacharCorreoLlamamiento) (ports.ReservaIntentoCorreoLlamamiento, error) {
	...
	if s == nil || ctx == nil || dependenciaNula(s.autorizador) || dependenciaNula(s.resolutor) || dependenciaNula(s.registro) || dependenciaNula(s.transporte) || dependenciaNula(s.reloj) {
	...
	err = s.resolutor.ConDestinoCorreoLlamamiento(ctx, solicitud, capacidad, func(destino ports.DestinoCorreoLlamamiento) error {
		...
		mensaje := destinoMensajeCorreo(destino, reserva)
...
func destinoMensajeCorreo(d ports.DestinoCorreoLlamamiento, r ports.ReservaIntentoCorreoLlamamiento) ports.MensajeCorreoLlamamiento {
	m := d.MensajePlantilla()
