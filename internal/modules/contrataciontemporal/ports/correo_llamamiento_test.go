...
func TestCapacidadFinalizacionLigaReservaYCopiaSecreto(t *testing.T) {
	...
	secret := make([]byte, 32)
	secret[0] = 1
	c, e := NuevaCapacidadFinalizacionIntentoCorreoLlamamiento(r, secret)
	...
	if _, e = NuevaCapacidadFinalizacionIntentoCorreoLlamamiento(ReservaIntentoCorreoLlamamiento{}, make([]byte, 32)); e == nil {
		t.Fatal("cero")
	}
}
...
func TestCapacidadFinalizacionRechazaReservaOCredencialInvalida(t *testing.T) {
	...
	capacidad, err := NuevaCapacidadFinalizacionIntentoCorreoLlamamiento(base, secreto)
	...
	for _, caso := range []struct {
		...
		t.Run(caso.nombre, func(t *testing.T) {
			r := base
			caso.cambiar(&r)
			c, err := NuevaCapacidadFinalizacionIntentoCorreoLlamamiento(r, secreto)
	...
	for _, caso := range []struct {
		...
		t.Run(caso.nombre, func(t *testing.T) {
			c, err := NuevaCapacidadFinalizacionIntentoCorreoLlamamiento(base, caso.secreto)
			if err == nil || !c.EsCero() {
