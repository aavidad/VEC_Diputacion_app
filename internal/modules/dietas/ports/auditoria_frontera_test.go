...
func TestOrdenAuditoriaFronteraComisionMinimizaRutaYActor(t *testing.T) {
	...
	for _, cambio := range []func(*OrdenAuditoriaFronteraComision){
		...
		func(o *OrdenAuditoriaFronteraComision) { o.Accion = "pagar" },
...
func TestAuditoriaFronteraPrelecturaSoloEnCircuito(t *testing.T) {
	o := OrdenAuditoriaFronteraComision{CorrelacionRef: "corr_" + strings.Repeat("a", 32), Motivo: MotivoFronteraAccesoDenegado, Ruta: RutaAuditoriaFronteraCircuito, Accion: AccionFronteraPreleer, ActorRef: "desarrollo:actor_sintetico"}
	if err := o.Validar(); err != nil {
		t.Fatal(err)
	}
	o.Ruta = RutaAuditoriaFronteraDetalle
	if o.Validar() == nil {
		t.Fatal("prelectura aceptada en detalle de empleado")
	}
}
