...
func (s *ServicioAutorizacionRutasDietas) AutorizarYConsumirAccesoRutas(ctx context.Context, e dietasports.SolicitudAccesoRutasDietas) (dietasports.ReciboAccesoRutasDietas, error) {
	...
	decisionCanonica, errDecision := vecdomain.RepresentacionCanonicaDecisionAutorizacionV3(d)
	confirmacion, errConfirmacion := c.Datos()
	if errDecision != nil || errConfirmacion != nil || !bytes.Equal(m.DecisionCanonica(), decisionCanonica) ||
		!bytes.Equal(m.ContextoActorCanonico(), e.ResultadoContexto.RepresentacionCanonica) ||
		m.PersonaVersion() != e.ResultadoContexto.Contexto.Instantanea.PersonaVersion ||
		m.PerfilVersion() != e.ResultadoContexto.Contexto.Instantanea.PerfilVersion ||
		m.ResumenCapacidad().DecisionRef() != confirmacion.DecisionRef ||
		m.ResumenCapacidad().DecisionHuellaSHA256() != confirmacion.DecisionHuellaSHA256 {
		return dietasports.ReciboAccesoRutasDietas{}, dietasports.ErrAccesoRutasDietasDenegado
	}
